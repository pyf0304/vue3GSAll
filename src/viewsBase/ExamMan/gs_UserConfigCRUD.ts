...
//import $ from "jquery";
import {
  vgs_UserConfig_GetRecCountByCondCache,
  vgs_UserConfig_GetSubObjLstCache,
  vgs_UserConfig_GetObjByConfigIdAsync,
} from '@/ts/L3ForWApi/ExamMan/clsvgs_UserConfigWApi';
import {
...
import {
  gs_UserConfig_DelRecordAsync,
  gs_UserConfig_ReFreshCache,
  gs_UserConfig_GetObjLstByConfigIdLstAsync,
  gs_UserConfig_AddNewRecordAsync,
  gs_UserConfig_Delgs_UserConfigsAsync,
} from '@/ts/L3ForWApi/ExamMan/clsgs_UserConfigWApi';
import { clsvgs_UserConfigENEx } from '@/ts/L0Entity/ExamMan/clsvgs_UserConfigENEx';
import {
  BindTab_V,
  arrSelectedKeys,
  confirmDel,
...
import {
  vgs_UserConfigEx_FuncMapByFldName,
  vgs_UserConfigEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/ExamMan/clsvgs_UserConfigExWApi';
...
export abstract class gs_UserConfigCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (gs_UserConfigCRUD.sortvgs_UserConfigBy == '')
        gs_UserConfigCRUD.sortvgs_UserConfigBy = 'configTypeName Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vgs_UserConfigCache(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrvgs_UserConfigObjLst: Array<clsvgs_UserConfigEN>,
    ...
    const strFileName = Format('用户配置({0})导出.xlsx', clsvgs_UserConfigEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vgs_UserConfigCache() {
    const strThisFuncName = this.ExportExcel_vgs_UserConfigCache.name;
    if (gs_UserConfigCRUD.sortvgs_UserConfigBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvgs_UserConfigBy)为空,请检查!(In BindGv_vgs_UserConfigCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvgs_UserConfigCond = await this.Combinevgs_UserConfigConditionObj4ExportExcel();
    let arrvgs_UserConfigObjLst: Array<clsvgs_UserConfigEN> = [];
    try {
      this.recCount = await vgs_UserConfig_GetRecCountByCondCache(objvgs_UserConfigCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvgs_UserConfigCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvgs_UserConfigObjLst = await vgs_UserConfig_GetSubObjLstCache(objvgs_UserConfigCond);
    } catch (e) {
    ...
    try {
      ...
      arrvgs_UserConfigObjLst = arrvgs_UserConfigObjLst.sort(this.SortFunExportExcel);
  ...
  public async SelectRecord(lngConfigId: number) {
    ...
    try {
      const objgs_UserConfigEN = await vgs_UserConfig_GetObjByConfigIdAsync(lngConfigId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vgs_UserConfigCache();
  }
  ...
  public async Combinevgs_UserConfigConditionObj(): Promise<clsvgs_UserConfigEN> {
    ...
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    objvgs_UserConfigCond.whereCond = strWhereCond;
    return objvgs_UserConfigCond;
  }
  ...
  public async Combinevgs_UserConfigConditionObj4ExportExcel(): Promise<clsvgs_UserConfigEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    const strWhereCond = ' 1 = 1 ';
    const objvgs_UserConfigCond = new clsvgs_UserConfigENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    objvgs_UserConfigCond.whereCond = strWhereCond;
    return objvgs_UserConfigCond;
  }
   ...
   **/
  public async BindTab_vgs_UserConfig(
  ...
  public async ExtendFldFuncMap(
    arrvgs_UserConfigExObjLst: Array<clsvgs_UserConfigENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vgs_UserConfigCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vgs_UserConfigCache.name;
    if (gs_UserConfigCRUD.sortvgs_UserConfigBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvgs_UserConfigBy)为空,请检查!(In BindGv_vgs_UserConfigCache)',
      );
    ...
    const objvgs_UserConfigCond = await this.Combinevgs_UserConfigConditionObj();
    const strWhereCond = JSON.stringify(objvgs_UserConfigCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvgs_UserConfigExObjLst: Array<clsvgs_UserConfigENEx> = [];
    try {
      this.recCount = await vgs_UserConfig_GetRecCountByCondCache(objvgs_UserConfigCond);
      if (this.recCount == 0) {
      ...
      if (gs_UserConfigCRUD.sortFunStatic != undefined) {
        strSortFun = gs_UserConfigCRUD.sortFunStatic(gs_UserConfigCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvgs_UserConfigExObjLst = await vgs_UserConfigEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    try {
      await this.BindTab_vgs_UserConfig(divList, arrvgs_UserConfigExObjLst);
      //console.log("完成BindGv_vgs_UserConfigCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvgs_UserConfigEN, b: clsvgs_UserConfigEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      gs_UserConfigCRUD.ascOrDesc4SortFun,
    ...
    gs_UserConfigCRUD.sortFunStatic = sortFun;
    await this.BindGv_vgs_UserConfigCache(this.divList);
  }
  ...
  public async DelMultiRecord(arrConfigId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const lngConfigId of arrConfigId) {
          gs_UserConfig_ReFreshCache();
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public Showgs_UserConfigObj(divContainer: HTMLDivElement, objgs_UserConfig: clsgs_UserConfigEN) {
    if (divContainer == null) {
