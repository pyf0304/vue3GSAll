...
import {
  ...
  gs_Tags_GetObjLstByTagsIdLstAsync,
  gs_Tags_GetMaxStrIdAsync,
  gs_Tags_AddNewRecordAsync,
  gs_Tags_Delgs_TagssAsync,
...
export abstract class gs_TagsCRUD implements clsOperateList {
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (gs_TagsCRUD.sortgs_TagsBy == '') gs_TagsCRUD.sortgs_TagsBy = 'tagsContent Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_gs_Tags4Func(this.divList);
    } catch (e) {
  ...
  public CombineData(arrgs_TagsObjLst: Array<clsgs_TagsEN>, arrDataColumn: Array<clsDataColumn>) {
    ...
    const strFileName = Format('标注({0})导出.xlsx', clsgs_TagsEN._CurrTabName);
  ...
  public async ExportExcel_gs_Tags() {
    ...
    try {
      ...
      arrgs_TagsObjLst = arrgs_TagsObjLst.sort(this.SortFunExportExcel);
  ...
  public async SetDdl_TagsTypeIdInDiv() {
    await gs_TagsType_BindDdl_TagsTypeIdInDivCache(this.divQuery, 'ddlTagsTypeId_q'); //
  }
  ...
  public async BindDdl4QueryRegion() {
    const IdCurrEduClsStatic = gs_TagsCRUD.IdCurrEduClsStatic; //静态变量;//静态变量
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_gs_Tags();
  }
  ...
  public async Combinegs_TagsCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(Combinegs_TagsCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async Combinegs_TagsConditionObj(): Promise<clsgs_TagsEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objgs_TagsCond = new clsgs_TagsEN();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.tagsContent_q != '') {
        strWhereCond += Format(
        ...
        objgs_TagsCond.SetCondFldValue(clsgs_TagsEN.con_TagsContent, this.tagsContent_q, 'like');
      }
      if (this.paperId_q != '' && this.paperId_q != '0') {
        strWhereCond += Format(" And {0} = '{1}'", clsgs_TagsEN.con_PaperId, this.paperId_q);
        objgs_TagsCond.SetCondFldValue(clsgs_TagsEN.con_PaperId, this.paperId_q, '=');
      }
      if (this.tagsTypeId_q != '' && this.tagsTypeId_q != '0') {
        strWhereCond += Format(" And {0} = '{1}'", clsgs_TagsEN.con_TagsTypeId, this.tagsTypeId_q);
        objgs_TagsCond.SetCondFldValue(clsgs_TagsEN.con_TagsTypeId, this.tagsTypeId_q, '=');
      }
    } catch (objException) {
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(Combinegs_TagsConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objgs_TagsCond.whereCond = strWhereCond;
    return objgs_TagsCond;
  }
  ...
  public async Combinegs_TagsConditionObj4ExportExcel(): Promise<clsgs_TagsEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objgs_TagsCond = new clsgs_TagsENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.tagsContent_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(Combinegs_TagsConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
   ...
   **/
  public async BindTab_gs_Tags(
    divContainer: HTMLDivElement,
    arrgs_TagsObjLst: Array<clsgs_TagsEN>,
  ) {
    ...
    await BindTab(divDataLst, arrgs_TagsObjLst, arrDataColumn, clsgs_TagsEN.con_TagsId, this);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsgs_TagsEN, b: clsgs_TagsEN): number {
  ...
  public async DelMultiRecord(arrTagsId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strTagsId of arrTagsId) {
          //gs_Tags_ReFreshCache();
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public Showgs_TagsObj(divContainer: HTMLDivElement, objgs_Tags: clsgs_TagsEN) {
    if (divContainer == null) {
  ...
  public get tagsContent_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtTagsContent_q');
    if (strValue == undefined) return '';
  ...
  public set tagsContent_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtTagsContent_q', value);
  }
  /**
   * 标注类型ID (Used In CombineCondition())
   **/
  public get tagsTypeId_q(): string {
    const strValue = GetSelectValueInDivObj(this.divQuery, 'ddlTagsTypeId_q');
    if (strValue == undefined) return '';
    else if (strValue == '0') return '';
    else return strValue;
  }
  ...
  public set tagsTypeId_q(value: string) {
    SetSelectValueByIdInDivObj(this.divQuery, 'ddlTagsTypeId_q', value);
  }
  /**
