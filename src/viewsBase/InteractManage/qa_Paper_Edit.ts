...
import {
  ...
  qa_Paper_CheckPropertyNew,
  qa_Paper_AddNewRecordWithMaxIdAsync,
  qa_Paper_ReFreshCache,
  qa_Paper_GetUniCondStr,
  qa_Paper_IsExistRecordAsync,
  qa_Paper_GetUniCondStr4Update,
  qa_Paper_IsExistAsync,
  qa_Paper_GetObjByQaPaperIdAsync,
  qa_Paper_CheckProperty4Update,
  qa_Paper_UpdateRecordAsync,
...
export abstract class qa_Paper_Edit {
  ...
  public static objPageEdit: qa_Paper_Edit;
  public static objPageEdit2: qa_Paper_Edit;
  public static objPageEdit3: qa_Paper_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList) {
    ...
    if (qa_Paper_Edit.SetPageEdit(this, 1) == true) return;
    if (qa_Paper_Edit.SetPageEdit(this, 2) == true) return;
    if (qa_Paper_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (qa_Paper_Edit.objPageEdit == null) {
          qa_Paper_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = qa_Paper_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (qa_Paper_Edit.objPageEdit2 == null) {
          qa_Paper_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = qa_Paper_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            qa_Paper_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (qa_Paper_Edit.objPageEdit3 == null) {
          qa_Paper_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = qa_Paper_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            qa_Paper_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (qa_Paper_Edit.objPageEdit != null) {
      const strClassNameOld = qa_Paper_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return qa_Paper_Edit.objPageEdit;
    }
    if (qa_Paper_Edit.objPageEdit2 != null) {
      const strClassNameOld = qa_Paper_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return qa_Paper_Edit.objPageEdit2;
    }
    if (qa_Paper_Edit.objPageEdit3 != null) {
      const strClassNameOld = qa_Paper_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return qa_Paper_Edit.objPageEdit3;
    }
    return null;
  }
  ...
  public HideDialog_qa_Paper() {
    if (qa_Paper_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      qa_Paper_Edit.EditObj.hideDialog();
    }
  }
   ...
   **/
  public async ShowDialog_qa_Paper(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_qa_Paper.name;
    if (qa_Paper_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (qa_Paper_Edit.EditObj == null) {
        const strMsg = Format(
      ...
      await qa_Paper_Edit.EditObj.showDialog();
    }
    this.divEdit = qa_Paper_Edit.EditObj.$refs.refDivEdit;
    if (this.divEdit == null) {
      if (qa_Paper_Edit.times4TestShowDialog < 2) {
        qa_Paper_Edit.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_qa_Paper(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      qa_Paper_Edit.times4TestShowDialog = 0;
    }
    if (strOp === 'Add' || strOp === 'AddWithMaxId') {
      this.btnSubmitqa_Paper = '确认添加';
      this.btnCancelqa_Paper = '取消添加';
    } else if (strOp === 'Update') {
      this.btnSubmitqa_Paper = '确认修改';
      this.btnCancelqa_Paper = '取消修改';
    }
    return true;
  }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (qa_Paper_Edit.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (qa_Paper_Edit.EditObj.dialogVisible == false) {
        const strMsg = Format(
  ...
  public async BindDdl4EditRegionInDiv() {
    const IdCurrEduClsStatic = qa_Paper_Edit.IdCurrEduClsStatic; //静态变量;//静态变量
  ...
  public async btnSubmit_Click() {
    ...
    const strCommandText: string = this.btnSubmitqa_Paper;
    try {
        ...
        case '确认添加':
          ...
          if (this.opType == 'AddWithMaxId') {
            ...
            if (IsNullOrEmpty(returnKeyId) == false) {
              ...
              this.iShowList.BindGvCache(clsqa_PaperEN._CurrTabName, returnKeyId);
            }
          } else {
            ...
            if (returnBool == true) {
              ...
              this.iShowList.BindGvCache(clsqa_PaperEN._CurrTabName, this.keyId);
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecord() {
    ...
    this.Clear();
    //wucqa_PaperB1.qaPaperId = qa_PaperGetMaxStrId_S();
    try {
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.qaPaperId = await qa_Paper_GetMaxStrIdAsync();
    try {
      const returnString = await qa_Paper_GetMaxStrIdAsync();
      if (returnString == '') {
        const strInfo = Format('获取表qa_Paper的最大关键字为空,不成功,请检查!');
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objqa_PaperEN);
      ...
      if (returnBool == true) {
        qa_Paper_ReFreshCache(qa_Paper_Edit.IdCurrEduClsCache);
   ...
   **/
  public async CheckUniCond4Add(objqa_PaperEN: clsqa_PaperEN): Promise<boolean> {
    const strUniquenessCondition = qa_Paper_GetUniCondStr(objqa_PaperEN);
   ...
   **/
  public async CheckUniCond4Update(objqa_PaperEN: clsqa_PaperEN): Promise<boolean> {
    const strUniquenessCondition = qa_Paper_GetUniCondStr4Update(objqa_PaperEN);
    const bolIsExistCondition = await qa_Paper_IsExistRecordAsync(strUniquenessCondition);
    if (bolIsExistCondition == true) {
      const strMsg = Format(
        '不能满足唯一性条件。满足条件：{0}的记录已经存在!',
   ...
   * @param strQaPaperId: 表记录的关键字,显示该表关键字的内容
   **/
  public async ShowData(strQaPaperId: string) {
    ...
    try {
      const returnBool = await qa_Paper_IsExistAsync(strQaPaperId);
      if (returnBool == false) {
        const strInfo = Format('关键字:[{0}] 的记录不存在!', strQaPaperId);
    ...
    try {
      const objqa_PaperENConst = await qa_Paper_GetObjByQaPaperIdAsync(strQaPaperId);
      if (objqa_PaperENConst == null) {
        const strMsg = Format(
      ...
      objqa_PaperEN = objqa_PaperENConst;
    } catch (e) {
      const strMsg = Format(
   ...
   **/
  public async GetDataFromqa_PaperClass(pobjqa_PaperEN: clsqa_PaperEN) {
   ...
   **/
  public async UpdateRecord(strQaPaperId: string): Promise<boolean> {
    const strThisFuncName = this.UpdateRecord.name;
    this.keyId = strQaPaperId;
    try {
      const objqa_PaperEN = await qa_Paper_GetObjByQaPaperIdAsync(strQaPaperId);
      ...
      await this.GetDataFromqa_PaperClass(objqa_PaperEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    const objqa_PaperEN = new clsqa_PaperEN();
    objqa_PaperEN.SetQaPaperId(this.keyId);
    await this.PutDataToqa_PaperClass(objqa_PaperEN);
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objqa_PaperEN);
      if (bolIsExistCond == false) {
        return false;
      }
   ...
   **/
  public set btnCancelqa_Paper(value: string) {
    qa_Paper_Edit.EditObj.SetButtonText('btnCancelqa_Paper', value);
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitqa_Paper(): string {
    const strValue = qa_Paper_Edit.EditObj.GetButtonText('btnSubmitqa_Paper');
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitqa_Paper(value: string) {
    qa_Paper_Edit.EditObj.SetButtonText('btnSubmitqa_Paper', value);
  }
  /**
   * 是否公开 (Used In Clear())
   **/
