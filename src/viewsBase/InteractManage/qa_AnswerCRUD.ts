...
//import $ from "jquery";
import {
  vqa_Answer_GetRecCountByCondCache,
  vqa_Answer_GetSubObjLstCache,
...
import {
  ...
  qa_Answer_GetObjLstByAnswerIdLstAsync,
  qa_Answer_GetMaxStrIdAsync,
  qa_Answer_AddNewRecordAsync,
  qa_Answer_Delqa_AnswersAsync,
...
import {
  vqa_AnswerEx_FuncMapByFldName,
  vqa_AnswerEx_GetObjExLstByPagerCache,
...
export abstract class qa_AnswerCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (qa_AnswerCRUD.sortvqa_AnswerBy == '')
        qa_AnswerCRUD.sortvqa_AnswerBy = 'answerContent Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vqa_AnswerCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('答疑回答({0})导出.xlsx', clsvqa_AnswerEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vqa_AnswerCache() {
    const strThisFuncName = this.ExportExcel_vqa_AnswerCache.name;
    if (qa_AnswerCRUD.sortvqa_AnswerBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvqa_AnswerBy)为空,请检查!(In BindGv_vqa_AnswerCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvqa_AnswerCond = await this.Combinevqa_AnswerConditionObj4ExportExcel();
    objvqa_AnswerCond.SetCondFldValue(clsvqa_AnswerEN.con_TopicId, qa_AnswerCRUD.TopicIdCache, '=');
    let arrvqa_AnswerObjLst: Array<clsvqa_AnswerEN> = [];
    try {
      this.recCount = await vqa_Answer_GetRecCountByCondCache(
        objvqa_AnswerCond,
        qa_AnswerCRUD.TopicIdCache,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvqa_AnswerCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvqa_AnswerObjLst = await vqa_Answer_GetSubObjLstCache(
        objvqa_AnswerCond,
        qa_AnswerCRUD.TopicIdCache,
      );
    } catch (e) {
    ...
    if (arrvqa_AnswerObjLst.length == 0) {
      const strKey = Format('{0}_{1}', clsqa_AnswerEN._CurrTabName, qa_AnswerCRUD.TopicIdCache);
    ...
    try {
      ...
      arrvqa_AnswerObjLst = arrvqa_AnswerObjLst.sort(this.SortFunExportExcel);
  ...
  public async DelRecord(strAnswerId: string) {
    ...
    try {
      ...
      if (returnInt > 0) {
        qa_Answer_ReFreshCache(qa_AnswerCRUD.TopicIdCache);
  ...
  public async SelectRecord(strAnswerId: string) {
    ...
    try {
      const objqa_AnswerEN = await vqa_Answer_GetObjByAnswerIdAsync(strAnswerId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vqa_AnswerCache();
  }
  ...
  public async Combinevqa_AnswerCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(Combineqa_AnswerCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async Combinevqa_AnswerConditionObj(): Promise<clsvqa_AnswerEN> {
    ...
    try {
      if (this.questionsId_q != '') {
        ...
        objvqa_AnswerCond.SetCondFldValue(
          clsvqa_AnswerEN.con_QuestionsId,
          this.questionsId_q,
          'like',
        );
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(Combineqa_AnswerConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvqa_AnswerCond.whereCond = strWhereCond;
    return objvqa_AnswerCond;
  }
  ...
  public async Combinevqa_AnswerConditionObj4ExportExcel(): Promise<clsvqa_AnswerEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvqa_AnswerCond = new clsvqa_AnswerENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.questionsId_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(Combineqa_AnswerConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async ExtendFldFuncMap(
    arrvqa_AnswerExObjLst: Array<clsvqa_AnswerENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vqa_AnswerCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vqa_AnswerCache.name;
    if (qa_AnswerCRUD.sortvqa_AnswerBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvqa_AnswerBy)为空,请检查!(In BindGv_vqa_AnswerCache)',
      );
    ...
    const objvqa_AnswerCond = await this.Combinevqa_AnswerConditionObj();
    const strWhereCond = JSON.stringify(objvqa_AnswerCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvqa_AnswerExObjLst: Array<clsvqa_AnswerENEx> = [];
    try {
      this.recCount = await vqa_Answer_GetRecCountByCondCache(
        objvqa_AnswerCond,
        qa_AnswerCRUD.TopicIdCache,
      ...
      if (qa_AnswerCRUD.sortFunStatic != undefined) {
        strSortFun = qa_AnswerCRUD.sortFunStatic(qa_AnswerCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvqa_AnswerExObjLst = await vqa_AnswerEx_GetObjExLstByPagerCache(
        objPagerPara,
        qa_AnswerCRUD.TopicIdCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_vqa_Answer(divList, arrvqa_AnswerExObjLst);
      //console.log("完成BindGv_vqa_AnswerCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvqa_AnswerEN, b: clsvqa_AnswerEN): number {
  ...
  public async DelMultiRecord(arrAnswerId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strAnswerId of arrAnswerId) {
          qa_Answer_ReFreshCache(qa_AnswerCRUD.TopicIdCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public Showqa_AnswerObj(divContainer: HTMLDivElement, objqa_Answer: clsqa_AnswerEN) {
    if (divContainer == null) {
