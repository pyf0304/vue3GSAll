...
//import $ from "jquery";
import {
  vqa_Questions_GetRecCountByCondCache,
  vqa_Questions_GetSubObjLstCache,
...
import {
  ...
  qa_Questions_GoBottomAsync,
  qa_Questions_DownMoveAsync,
  qa_Questions_UpMoveAsync,
  qa_Questions_GoTopAsync,
  qa_Questions_DelRecordAsync,
  qa_Questions_GetObjLstByQuestionsIdLstAsync,
  qa_Questions_GetMaxStrIdAsync,
  qa_Questions_AddNewRecordAsync,
  qa_Questions_Delqa_QuestionssAsync,
...
import {
  vqa_QuestionsEx_FuncMapByFldName,
  vqa_QuestionsEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/InteractManage/clsvqa_QuestionsExWApi';
...
export abstract class qa_QuestionsCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (qa_QuestionsCRUD.sortvqa_QuestionsBy == '')
        qa_QuestionsCRUD.sortvqa_QuestionsBy = 'questionsContent Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vqa_QuestionsCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('答疑提问表({0})导出.xlsx', clsvqa_QuestionsEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vqa_QuestionsCache() {
    const strThisFuncName = this.ExportExcel_vqa_QuestionsCache.name;
    if (qa_QuestionsCRUD.sortvqa_QuestionsBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvqa_QuestionsBy)为空,请检查!(In BindGv_vqa_QuestionsCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvqa_QuestionsCond = await this.Combinevqa_QuestionsConditionObj4ExportExcel();
    let arrvqa_QuestionsObjLst: Array<clsvqa_QuestionsEN> = [];
    try {
      this.recCount = await vqa_Questions_GetRecCountByCondCache(objvqa_QuestionsCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvqa_QuestionsCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvqa_QuestionsObjLst = await vqa_Questions_GetSubObjLstCache(objvqa_QuestionsCond);
    } catch (e) {
    ...
    try {
      ...
      arrvqa_QuestionsObjLst = arrvqa_QuestionsObjLst.sort(this.SortFunExportExcel);
  ...
  public async btnGoBottum_Click() {
    ...
    try {
      ...
      await qa_Questions_GoBottomAsync(objOrderByData);
      qa_Questions_ReFreshCache();
    } catch (e) {
  ...
  public async btnGoTop_Click() {
    ...
    try {
      const objOrderByData: clsOrderByData = new clsOrderByData();
      objOrderByData.KeyIdLst = arrKeyIds;
      await qa_Questions_GoTopAsync(objOrderByData);
      qa_Questions_ReFreshCache();
    } catch (e) {
  ...
  public async SelectRecord(strQuestionsId: string) {
    ...
    try {
      const objqa_QuestionsEN = await vqa_Questions_GetObjByQuestionsIdAsync(strQuestionsId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vqa_QuestionsCache();
  }
  ...
  public async Combinevqa_QuestionsCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(Combineqa_QuestionsCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async Combinevqa_QuestionsConditionObj(): Promise<clsvqa_QuestionsEN> {
    ...
    try {
      if (this.qaPaperId_q != '') {
        ...
        objvqa_QuestionsCond.SetCondFldValue(
          clsvqa_QuestionsEN.con_QaPaperId,
          this.qaPaperId_q,
          'like',
        );
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(Combineqa_QuestionsConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvqa_QuestionsCond.whereCond = strWhereCond;
    return objvqa_QuestionsCond;
  }
  ...
  public async Combinevqa_QuestionsConditionObj4ExportExcel(): Promise<clsvqa_QuestionsEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvqa_QuestionsCond = new clsvqa_QuestionsENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.qaPaperId_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(Combineqa_QuestionsConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async ExtendFldFuncMap(
    arrvqa_QuestionsExObjLst: Array<clsvqa_QuestionsENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vqa_QuestionsCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vqa_QuestionsCache.name;
    if (qa_QuestionsCRUD.sortvqa_QuestionsBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvqa_QuestionsBy)为空,请检查!(In BindGv_vqa_QuestionsCache)',
      );
    ...
    const objvqa_QuestionsCond = await this.Combinevqa_QuestionsConditionObj();
    const strWhereCond = JSON.stringify(objvqa_QuestionsCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvqa_QuestionsExObjLst: Array<clsvqa_QuestionsENEx> = [];
    try {
      this.recCount = await vqa_Questions_GetRecCountByCondCache(objvqa_QuestionsCond);
      if (this.recCount == 0) {
      ...
      if (qa_QuestionsCRUD.sortFunStatic != undefined) {
        strSortFun = qa_QuestionsCRUD.sortFunStatic(qa_QuestionsCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvqa_QuestionsExObjLst = await vqa_QuestionsEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    try {
      await this.BindTab_vqa_Questions(divList, arrvqa_QuestionsExObjLst);
      //console.log("完成BindGv_vqa_QuestionsCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvqa_QuestionsEN, b: clsvqa_QuestionsEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      qa_QuestionsCRUD.ascOrDesc4SortFun,
    ...
    qa_QuestionsCRUD.sortFunStatic = sortFun;
    await this.BindGv_vqa_QuestionsCache(this.divList);
  }
  ...
  public async CopyRecord(arrQuestionsId: Array<string>) {
    ...
    try {
      const arrqa_QuestionsObjLst = await qa_Questions_GetObjLstByQuestionsIdLstAsync(
        arrQuestionsId,
      );
  ...
  public async DelMultiRecord(arrQuestionsId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strQuestionsId of arrQuestionsId) {
          qa_Questions_ReFreshCache();
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public Showqa_QuestionsObj(divContainer: HTMLDivElement, objqa_Questions: clsqa_QuestionsEN) {
    if (divContainer == null) {
