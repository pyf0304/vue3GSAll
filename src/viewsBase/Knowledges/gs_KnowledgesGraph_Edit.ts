...
import {
  ...
  gs_KnowledgesGraph_GetUniCondStr,
  gs_KnowledgesGraph_IsExistRecordAsync,
  gs_KnowledgesGraph_GetUniCondStr4Update,
...
export abstract class gs_KnowledgesGraph_Edit {
  ...
  public HideDialog_gs_KnowledgesGraph() {
    if (gs_KnowledgesGraph_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      gs_KnowledgesGraph_Edit.EditObj.hideDialog();
    }
  }
  ...
  public async ShowDialog_gs_KnowledgesGraph(strOp: string): Promise<boolean> {
    ...
    if (gs_KnowledgesGraph_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (gs_KnowledgesGraph_Edit.EditObj == null) {
        const strMsg = Format(
      ...
      await gs_KnowledgesGraph_Edit.EditObj.showDialog();
    }
    this.divEdit = gs_KnowledgesGraph_Edit.EditObj.$refs.refDivEdit;
    if (this.divEdit == null) {
      if (gs_KnowledgesGraph_Edit.times4TestShowDialog < 2) {
        gs_KnowledgesGraph_Edit.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_gs_KnowledgesGraph(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      gs_KnowledgesGraph_Edit.times4TestShowDialog = 0;
    }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (gs_KnowledgesGraph_Edit.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (gs_KnowledgesGraph_Edit.EditObj.dialogVisible == false) {
        const strMsg = Format(
  ...
  public async SetDdl_CourseIdInDiv() {
    await cc_Course_BindDdl_CourseIdInDivCache(this.divEdit, 'ddlCourseId'); //
  }
        ...
        case '确认添加':
          ...
          if (this.opType == 'AddWithMaxId') {
            ...
            if (IsNullOrEmpty(returnKeyId) == false) {
              ...
              this.iShowList.BindGvCache(clsgs_KnowledgesGraphEN._CurrTabName, returnKeyId);
            }
          } else {
            ...
            if (returnBool == true) {
              ...
              this.iShowList.BindGvCache(clsgs_KnowledgesGraphEN._CurrTabName, this.keyId);
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      ...
      let returnBool = false;
      const returnKeyId = await gs_KnowledgesGraph_AddNewRecordWithMaxIdAsync(
      ...
      if (returnBool == true) {
        gs_KnowledgesGraph_ReFreshCache(gs_KnowledgesGraph_Edit.CourseIdCache);
  ...
  public async ShowData(strKnowledgeGraphId: string) {
    ...
    try {
      const objgs_KnowledgesGraphENConst = await gs_KnowledgesGraph_GetObjByKnowledgeGraphIdAsync(
        strKnowledgeGraphId,
      );
  ...
  public async UpdateRecord(strKnowledgeGraphId: string): Promise<boolean> {
    ...
    try {
      ...
      await this.GetDataFromgs_KnowledgesGraphClass(objgs_KnowledgesGraphEN);
  ...
  public set btnCancelgs_KnowledgesGraph(value: string) {
    gs_KnowledgesGraph_Edit.EditObj.SetButtonText('btnCancelgs_KnowledgesGraph', value);
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitgs_KnowledgesGraph(): string {
    const strValue = gs_KnowledgesGraph_Edit.EditObj.GetButtonText('btnSubmitgs_KnowledgesGraph');
    return strValue;
  }
  /**
  ...
  public set btnSubmitgs_KnowledgesGraph(value: string) {
    gs_KnowledgesGraph_Edit.EditObj.SetButtonText('btnSubmitgs_KnowledgesGraph', value);
  }
  /**
   * 课程Id (Used In Clear())
   **/
  ...
  public set knowledgeGraphName(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtKnowledgeGraphName', value);
  }
  /**
   * 连连看图名 (Used In PutDataToClass())
   **/
  public get knowledgeGraphName(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtKnowledgeGraphName');
    if (strValue == undefined) return '';
