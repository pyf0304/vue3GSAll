...
//import $ from "jquery";
import {
  vPaperSubViewpointAppraise_GetRecCountByCondCache,
  vPaperSubViewpointAppraise_GetSubObjLstCache,
  vPaperSubViewpointAppraise_GetObjByPaperSubViewpointAppraiseIdAsync,
} from '@/ts/L3ForWApi/GradEduPaper/clsvPaperSubViewpointAppraiseWApi';
import {
...
import {
  PaperSubViewpointAppraise_DelRecordAsync,
  PaperSubViewpointAppraise_ReFreshCache,
  PaperSubViewpointAppraise_GetObjLstByPaperSubViewpointAppraiseIdLstAsync,
  PaperSubViewpointAppraise_AddNewRecordAsync,
  PaperSubViewpointAppraise_DelPaperSubViewpointAppraisesAsync,
...
import { clsvPaperSubViewpointAppraiseENEx } from '@/ts/L0Entity/GradEduPaper/clsvPaperSubViewpointAppraiseENEx';
import {
  BindTab_V,
  arrSelectedKeys,
  confirmDel,
...
import {
  vPaperSubViewpointAppraiseEx_FuncMapByFldName,
  vPaperSubViewpointAppraiseEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduPaper/clsvPaperSubViewpointAppraiseExWApi';
...
export abstract class PaperSubViewpointAppraiseCRUD implements clsOperateList {
  ...
  public static objPageCRUD: PaperSubViewpointAppraiseCRUD;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  public static ascOrDesc4SortFun = 'Asc';
  public static sortvPaperSubViewpointAppraiseBy = '';
  constructor(divLayout: HTMLDivElement) {
    PaperSubViewpointAppraiseCRUD.objPageCRUD = this;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (PaperSubViewpointAppraiseCRUD.sortvPaperSubViewpointAppraiseBy == '')
        PaperSubViewpointAppraiseCRUD.sortvPaperSubViewpointAppraiseBy = 'appraiseScore Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vPaperSubViewpointAppraiseCache(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrvPaperSubViewpointAppraiseObjLst: Array<clsvPaperSubViewpointAppraiseEN>,
   ...
   **/
  public async ExportExcel_vPaperSubViewpointAppraiseCache() {
    const strThisFuncName = this.ExportExcel_vPaperSubViewpointAppraiseCache.name;
    if (PaperSubViewpointAppraiseCRUD.sortvPaperSubViewpointAppraiseBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvPaperSubViewpointAppraiseBy)为空,请检查!(In BindGv_vPaperSubViewpointAppraiseCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvPaperSubViewpointAppraiseCond =
      await this.CombinevPaperSubViewpointAppraiseConditionObj4ExportExcel();
    let arrvPaperSubViewpointAppraiseObjLst: Array<clsvPaperSubViewpointAppraiseEN> = [];
    try {
      this.recCount = await vPaperSubViewpointAppraise_GetRecCountByCondCache(
        objvPaperSubViewpointAppraiseCond,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvPaperSubViewpointAppraiseCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvPaperSubViewpointAppraiseObjLst = await vPaperSubViewpointAppraise_GetSubObjLstCache(
        objvPaperSubViewpointAppraiseCond,
      );
    } catch (e) {
    ...
    try {
      ...
      arrvPaperSubViewpointAppraiseObjLst = arrvPaperSubViewpointAppraiseObjLst.sort(
  ...
  public async DelRecord(lngPaperSubViewpointAppraiseId: number) {
    ...
    try {
      const returnInt = await PaperSubViewpointAppraise_DelRecordAsync(
  ...
  public async SelectRecord(lngPaperSubViewpointAppraiseId: number) {
    ...
    try {
      const objPaperSubViewpointAppraiseEN =
        await vPaperSubViewpointAppraise_GetObjByPaperSubViewpointAppraiseIdAsync(
          lngPaperSubViewpointAppraiseId,
        );
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vPaperSubViewpointAppraiseCache();
  }
  ...
  public async CombinevPaperSubViewpointAppraiseCondition(): Promise<string> {
    ...
    try {
      if (this.updDate_q != '') {
        strWhereCond += Format(
          " And {0} like '% {1}%'",
          clsvPaperSubViewpointAppraiseEN.con_UpdDate,
          this.updDate_q,
        );
      }
    } catch (objException) {
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombinePaperSubViewpointAppraiseCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinevPaperSubViewpointAppraiseConditionObj(): Promise<clsvPaperSubViewpointAppraiseEN> {
    ...
    try {
      if (this.updDate_q != '') {
        ...
        objvPaperSubViewpointAppraiseCond.SetCondFldValue(
          clsvPaperSubViewpointAppraiseEN.con_UpdDate,
          this.updDate_q,
          'like',
        );
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombinePaperSubViewpointAppraiseConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvPaperSubViewpointAppraiseCond.whereCond = strWhereCond;
    return objvPaperSubViewpointAppraiseCond;
  }
  ...
  public async CombinevPaperSubViewpointAppraiseConditionObj4ExportExcel(): Promise<clsvPaperSubViewpointAppraiseEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvPaperSubViewpointAppraiseCond = new clsvPaperSubViewpointAppraiseENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.updDate_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombinePaperSubViewpointAppraiseConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
   ...
   **/
  public async BindTab_vPaperSubViewpointAppraise(
    ...
    await BindTab_V(
      divDataLst,
      arrvPaperSubViewpointAppraiseObjLst,
      arrDataColumn,
      clsvPaperSubViewpointAppraiseEN.con_PaperSubViewpointAppraiseId,
      this,
    );
  ...
  public async ExtendFldFuncMap(
    arrvPaperSubViewpointAppraiseExObjLst: Array<clsvPaperSubViewpointAppraiseENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vPaperSubViewpointAppraiseCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vPaperSubViewpointAppraiseCache.name;
    if (PaperSubViewpointAppraiseCRUD.sortvPaperSubViewpointAppraiseBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvPaperSubViewpointAppraiseBy)为空,请检查!(In BindGv_vPaperSubViewpointAppraiseCache)',
      );
    ...
    const objvPaperSubViewpointAppraiseCond =
      await this.CombinevPaperSubViewpointAppraiseConditionObj();
    const strWhereCond = JSON.stringify(objvPaperSubViewpointAppraiseCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvPaperSubViewpointAppraiseExObjLst: Array<clsvPaperSubViewpointAppraiseENEx> = [];
    try {
      this.recCount = await vPaperSubViewpointAppraise_GetRecCountByCondCache(
        objvPaperSubViewpointAppraiseCond,
      );
      ...
      if (PaperSubViewpointAppraiseCRUD.sortFunStatic != undefined) {
        strSortFun = PaperSubViewpointAppraiseCRUD.sortFunStatic(
          PaperSubViewpointAppraiseCRUD.ascOrDesc4SortFun,
        );
      }
      ...
      arrvPaperSubViewpointAppraiseExObjLst =
        await vPaperSubViewpointAppraiseEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    try {
      await this.BindTab_vPaperSubViewpointAppraise(divList, arrvPaperSubViewpointAppraiseExObjLst);
      //console.log("完成BindGv_vPaperSubViewpointAppraiseCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
  ...
  public SortFunExportExcel(
    a: clsvPaperSubViewpointAppraiseEN,
    b: clsvPaperSubViewpointAppraiseEN,
  ): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    PaperSubViewpointAppraiseCRUD.sortFunStatic = sortFun;
    await this.BindGv_vPaperSubViewpointAppraiseCache(this.divList);
  }
  ...
  public async CopyRecord(arrPaperSubViewpointAppraiseId: Array<string>) {
    ...
    try {
      const arrPaperSubViewpointAppraiseObjLst =
        await PaperSubViewpointAppraise_GetObjLstByPaperSubViewpointAppraiseIdLstAsync(
          arrPaperSubViewpointAppraiseId,
        );
  ...
  public async DelMultiRecord(arrPaperSubViewpointAppraiseId: Array<string>) {
    ...
    try {
      const returnInt = await PaperSubViewpointAppraise_DelPaperSubViewpointAppraisesAsync(
        arrPaperSubViewpointAppraiseId,
      );
      if (returnInt > 0) {
        for (const lngPaperSubViewpointAppraiseId of arrPaperSubViewpointAppraiseId) {
          PaperSubViewpointAppraise_ReFreshCache();
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public ShowPaperSubViewpointAppraiseObj(
    divContainer: HTMLDivElement,
    objPaperSubViewpointAppraise: clsPaperSubViewpointAppraiseEN,
  ) {
    if (divContainer == null) {
