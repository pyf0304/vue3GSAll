...
import {
  JournalSubject_GetRecCountByCondCache,
  JournalSubject_GetSubObjLstCache,
  JournalSubject_DelRecordAsync,
  JournalSubject_ReFreshCache,
  JournalSubject_GetObjByJournalSubjectIdAsync,
  JournalSubject_GetObjLstByJournalSubjectIdLstAsync,
  JournalSubject_GetMaxStrIdAsync,
  JournalSubject_AddNewRecordAsync,
  JournalSubject_DelJournalSubjectsAsync,
} from '@/ts/L3ForWApi/GradEduPaper/clsJournalSubjectWApi';
import {
  JournalSubjectEx_CopyToEx,
  JournalSubjectEx_FuncMapByFldName,
  JournalSubjectEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduPaper/clsJournalSubjectExWApi';
...
export abstract class JournalSubjectCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (JournalSubjectCRUD.sortJournalSubjectBy == '')
        JournalSubjectCRUD.sortJournalSubjectBy = 'journalSubjectId Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_JournalSubject4Func(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrJournalSubjectObjLst: Array<clsJournalSubjectEN>,
    ...
    const strFileName = Format('期刊学科({0})导出.xlsx', clsJournalSubjectEN._CurrTabName);
  ...
  public CombineData4Func(
    arrJournalSubjectExObjLst: Array<clsJournalSubjectENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
  ...
  public async ExportExcel_JournalSubject4Func() {
    ...
    if (JournalSubjectCRUD.sortJournalSubjectBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortJournalSubjectBy)为空,请检查!(In BindGv_JournalSubjectCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objJournalSubjectCond = await this.CombineJournalSubjectConditionObj4ExportExcel();
    let arrJournalSubjectObjLst: Array<clsJournalSubjectEN> = [];
    let arrJournalSubjectExObjLst: Array<clsJournalSubjectENEx> = [];
    try {
      this.recCount = await JournalSubject_GetRecCountByCondCache(objJournalSubjectCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objJournalSubjectCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrJournalSubjectObjLst = await JournalSubject_GetSubObjLstCache(objJournalSubjectCond);
    ...
    try {
      ...
      arrJournalSubjectObjLst = arrJournalSubjectObjLst.sort(this.SortFunExportExcel);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_JournalSubject4Func();
  }
  ...
  public async CombineJournalSubjectCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombineJournalSubjectCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombineJournalSubjectConditionObj(): Promise<clsJournalSubjectEN> {
    ...
    try {
      if (this.journalSubjectId_q != '') {
        ...
        objJournalSubjectCond.SetCondFldValue(
          clsJournalSubjectEN.con_JournalSubjectId,
          this.journalSubjectId_q,
          'like',
        );
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombineJournalSubjectConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objJournalSubjectCond.whereCond = strWhereCond;
    return objJournalSubjectCond;
  }
  ...
  public async CombineJournalSubjectConditionObj4ExportExcel(): Promise<clsJournalSubjectEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objJournalSubjectCond = new clsJournalSubjectENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.journalSubjectId_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombineJournalSubjectConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
   ...
   **/
  public async BindTab_JournalSubject(
   ...
   **/
  public async BindTab_JournalSubject4Func(
    divContainer: HTMLDivElement,
    arrJournalSubjectExObjLst: Array<clsJournalSubjectENEx>,
  ) {
   ...
   **/
  public async BindGv_JournalSubjectCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_JournalSubjectCache.name;
    if (JournalSubjectCRUD.sortJournalSubjectBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortJournalSubjectBy)为空,请检查!(In BindGv_JournalSubjectCache)',
      );
    ...
    const objJournalSubjectCond = await this.CombineJournalSubjectConditionObj();
    const strWhereCond = JSON.stringify(objJournalSubjectCond);
    ...
    try {
      ...
      if (JournalSubjectCRUD.sortFunStatic != undefined) {
        strSortFun = JournalSubjectCRUD.sortFunStatic(JournalSubjectCRUD.ascOrDesc4SortFun);
      }
      ...
      arrJournalSubjectExObjLst = await JournalSubjectEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
    ...
    try {
      await this.BindTab_JournalSubject(divList, arrJournalSubjectExObjLst);
      //console.log("完成BindGv_JournalSubjectCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   **/
  public async BindGv_JournalSubject4Func(divList: HTMLDivElement) {
    ...
    try {
      await this.BindTab_JournalSubject4Func(divList, arrJournalSubjectExObjLst);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsJournalSubjectEN, b: clsJournalSubjectEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      JournalSubjectCRUD.ascOrDesc4SortFun,
    ...
    JournalSubjectCRUD.sortFunStatic = sortFun;
    await this.BindGv_JournalSubject4Func(this.divList);
  }
  ...
  public async CopyRecord(arrJournalSubjectId: Array<string>) {
    ...
    try {
      const arrJournalSubjectObjLst = await JournalSubject_GetObjLstByJournalSubjectIdLstAsync(
        arrJournalSubjectId,
      );
  ...
  public async DelMultiRecord(arrJournalSubjectId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strJournalSubjectId of arrJournalSubjectId) {
          JournalSubject_ReFreshCache();
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public ShowJournalSubjectObj(
    divContainer: HTMLDivElement,
    objJournalSubject: clsJournalSubjectEN,
  ) {
    if (divContainer == null) {
  ...
  public get journalSubjectCode_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtJournalSubjectCode_q');
    if (strValue == undefined) return '';
  ...
  public set journalSubjectCode_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtJournalSubjectCode_q', value);
  }
  /**
   * 期刊学科Id (Used In CombineCondition())
   **/
  public get journalSubjectId_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtJournalSubjectId_q');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set journalSubjectId_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtJournalSubjectId_q', value);
  }
  /**
   * 期刊学科名称 (Used In CombineCondition())
   **/
  public get journalSubjectName_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtJournalSubjectName_q');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set journalSubjectName_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtJournalSubjectName_q', value);
  }
  /**
