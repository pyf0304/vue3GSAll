...
//import $ from "jquery";
import {
  gs_PaperAttention_GetRecCountByCondCache,
  gs_PaperAttention_GetSubObjLstCache,
  gs_PaperAttention_DelRecordAsync,
  gs_PaperAttention_ReFreshCache,
  gs_PaperAttention_GetObjByPaperAttentionIdAsync,
  gs_PaperAttention_GetObjLstByPaperAttentionIdLstAsync,
  gs_PaperAttention_AddNewRecordAsync,
  gs_PaperAttention_Delgs_PaperAttentionsAsync,
...
import { clsgs_PaperAttentionENEx } from '@/ts/L0Entity/GradEduPaper/clsgs_PaperAttentionENEx';
import {
  BindTab,
  arrSelectedKeys,
  confirmDel,
...
import {
  gs_PaperAttentionEx_FuncMapByFldName,
  gs_PaperAttentionEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduPaper/clsgs_PaperAttentionExWApi';
...
export abstract class gs_PaperAttentionCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (gs_PaperAttentionCRUD.sortgs_PaperAttentionBy == '')
        gs_PaperAttentionCRUD.sortgs_PaperAttentionBy = 'paperAttentionId Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_gs_PaperAttentionCache(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrgs_PaperAttentionObjLst: Array<clsgs_PaperAttentionEN>,
    ...
    const strFileName = Format('论文关注({0})导出.xlsx', clsgs_PaperAttentionEN._CurrTabName);
   ...
   **/
  public async ExportExcel_gs_PaperAttentionCache() {
    const strThisFuncName = this.ExportExcel_gs_PaperAttentionCache.name;
    if (gs_PaperAttentionCRUD.sortgs_PaperAttentionBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortgs_PaperAttentionBy)为空,请检查!(In BindGv_gs_PaperAttentionCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objgs_PaperAttentionCond = await this.Combinegs_PaperAttentionConditionObj4ExportExcel();
    objgs_PaperAttentionCond.SetCondFldValue(
      clsgs_PaperAttentionEN.con_UserId,
      gs_PaperAttentionCRUD.UserIdCache,
      '=',
    );
    let arrgs_PaperAttentionObjLst: Array<clsgs_PaperAttentionEN> = [];
    try {
      this.recCount = await gs_PaperAttention_GetRecCountByCondCache(
        objgs_PaperAttentionCond,
        gs_PaperAttentionCRUD.UserIdCache,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objgs_PaperAttentionCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrgs_PaperAttentionObjLst = await gs_PaperAttention_GetSubObjLstCache(
        objgs_PaperAttentionCond,
        gs_PaperAttentionCRUD.UserIdCache,
      );
    } catch (e) {
    ...
    try {
      ...
      arrgs_PaperAttentionObjLst = arrgs_PaperAttentionObjLst.sort(this.SortFunExportExcel);
  ...
  public async BindDdl4QueryRegion() {
    const IdCurrEduClsStatic = gs_PaperAttentionCRUD.IdCurrEduClsStatic; //静态变量;//静态变量
    const strUserId = userStore.getUserId; //Session存储、local存储

    await this.SetDdl_PaperIdInDiv(IdCurrEduClsStatic); //查询区域

    await this.SetDdl_PaperGroupIdInDiv(strUserId); //查询区域
  }
  ...
  public async DelRecord(lngPaperAttentionId: number) {
    ...
    try {
      ...
      if (returnInt > 0) {
        gs_PaperAttention_ReFreshCache(gs_PaperAttentionCRUD.UserIdCache);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_gs_PaperAttentionCache();
  }
  ...
  public async Combinegs_PaperAttentionCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(Combinegs_PaperAttentionCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async Combinegs_PaperAttentionConditionObj(): Promise<clsgs_PaperAttentionEN> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(Combinegs_PaperAttentionConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objgs_PaperAttentionCond.whereCond = strWhereCond;
    return objgs_PaperAttentionCond;
  }
  ...
  public async Combinegs_PaperAttentionConditionObj4ExportExcel(): Promise<clsgs_PaperAttentionEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objgs_PaperAttentionCond = new clsgs_PaperAttentionENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.paperId_q != '' && this.paperId_q != '0') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(Combinegs_PaperAttentionConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
   ...
   **/
  public async BindTab_gs_PaperAttention(
  ...
  public async ExtendFldFuncMap(
    arrgs_PaperAttentionExObjLst: Array<clsgs_PaperAttentionENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_gs_PaperAttentionCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_gs_PaperAttentionCache.name;
    if (gs_PaperAttentionCRUD.sortgs_PaperAttentionBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortgs_PaperAttentionBy)为空,请检查!(In BindGv_gs_PaperAttentionCache)',
      );
    ...
    const objgs_PaperAttentionCond = await this.Combinegs_PaperAttentionConditionObj();
    const strWhereCond = JSON.stringify(objgs_PaperAttentionCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrgs_PaperAttentionExObjLst: Array<clsgs_PaperAttentionENEx> = [];
    try {
      this.recCount = await gs_PaperAttention_GetRecCountByCondCache(
        objgs_PaperAttentionCond,
        gs_PaperAttentionCRUD.UserIdCache,
      ...
      if (gs_PaperAttentionCRUD.sortFunStatic != undefined) {
        strSortFun = gs_PaperAttentionCRUD.sortFunStatic(gs_PaperAttentionCRUD.ascOrDesc4SortFun);
      }
      ...
      arrgs_PaperAttentionExObjLst = await gs_PaperAttentionEx_GetObjExLstByPagerCache(
        objPagerPara,
        gs_PaperAttentionCRUD.UserIdCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_gs_PaperAttention(divList, arrgs_PaperAttentionExObjLst);
      //console.log("完成BindGv_gs_PaperAttentionCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsgs_PaperAttentionEN, b: clsgs_PaperAttentionEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      gs_PaperAttentionCRUD.ascOrDesc4SortFun,
    ...
    gs_PaperAttentionCRUD.sortFunStatic = sortFun;
    await this.BindGv_gs_PaperAttentionCache(this.divList);
  }
  ...
  public async CopyRecord(arrPaperAttentionId: Array<string>) {
    ...
    try {
      const arrgs_PaperAttentionObjLst =
        await gs_PaperAttention_GetObjLstByPaperAttentionIdLstAsync(arrPaperAttentionId);
  ...
  public async DelMultiRecord(arrPaperAttentionId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const lngPaperAttentionId of arrPaperAttentionId) {
          gs_PaperAttention_ReFreshCache(gs_PaperAttentionCRUD.UserIdCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public Showgs_PaperAttentionObj(
    divContainer: HTMLDivElement,
  ...
  public get paperGroupId_q(): string {
    const strValue = GetSelectValueInDivObj(this.divQuery, 'ddlPaperGroupId_q');
  ...
  public set paperGroupId_q(value: string) {
    SetSelectValueByIdInDivObj(this.divQuery, 'ddlPaperGroupId_q', value);
  }
  /**
