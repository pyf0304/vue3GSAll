...
import {
  ...
  Paper_GetObjLstByPaperIdLstAsync,
...
import { XzMajorDirection_BindDdl_MajorDirectionIdByIdXzMajorInDivCache } from '@/ts/L3ForWApi/BaseInfo/clsXzMajorDirectionWApi';
import {
...
export abstract class PaperCRUD implements clsOperateList {
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (PaperCRUD.sortPaperBy == '') PaperCRUD.sortPaperBy = 'paperTitle Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_Paper(this.divList);
    } catch (e) {
  ...
  public CombineData(arrPaperObjLst: Array<clsPaperEN>, arrDataColumn: Array<clsDataColumn>) {
    ...
    const strFileName = Format('论文表({0})导出.xlsx', clsPaperEN._CurrTabName);
  ...
  public async ExportExcel_Paper() {
    ...
    if (PaperCRUD.sortPaperBy == null) {
      const strMsg = Format('在显示列表时,排序字段(sortPaperBy)为空,请检查!(In BindGv_PaperCache)');
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    ...
    try {
      ...
      arrPaperObjLst = arrPaperObjLst.sort(this.SortFunExportExcel);
  ...
  public async SetDdl_LiteratureTypeIdInDiv() {
    await LiteratureType_BindDdl_LiteratureTypeIdInDivCache(this.divQuery, 'ddlLiteratureTypeId_q'); //
  }
  ...
  public async SetDdl_MajorDirectionIdInDivInFeature(strIdXzMajor: string) {
    if (IsNullOrEmpty(strIdXzMajor) == true) {
      const strMsg = Format(
        '参数:[strIdXzMajor]不能为空!(In .SetDdl_MajorDirectionIdInDivInFeature)',
      );
      console.error(strMsg);
      throw strMsg;
    }
    if (strIdXzMajor.length != 8) {
      const strMsg = Format(
        '缓存分类变量:[strIdXzMajor]的长度:[{0}]不正确!(.SetDdl_MajorDirectionIdInDivInFeature)',
        strIdXzMajor.length,
      );
      console.error(strMsg);
      throw strMsg;
   ...
   **/
  public async btnSetMajorDirectionId_Click() {
    const strThisFuncName = this.btnSetMajorDirectionId_Click.name;
    try {
      const arrKeyIds = GetCheckedKeyIdsInDivObj(this.divList);
      if (arrKeyIds.length == 0) {
        alert(`请选择需要设置研究方向Id的${this.thisTabName}记录!`);
        return '';
      }
      const strMajorDirectionId = GetSelectValueInDivObj(
        this.divFunction,
        'ddlMajorDirectionId_SetValue_SetFldValue',
      );
      ...
      //console.log('strMajorDirectionId=' + strMajorDirectionId);
      //console.log('arrKeyIds=');
      //console.log(arrKeyIds);
      await this.SetMajorDirectionId(arrKeyIds, strMajorDirectionId);
      await this.BindGv_Paper(this.divList);
    } catch (e) {
  ...
  public async BindDdl4FeatureRegion() {
    const IdXzMajorStatic = PaperCRUD.IdXzMajorStatic; //静态变量;//静态变量

    await this.SetDdl_MajorDirectionIdInDivInFeature(IdXzMajorStatic); //功能区域
  }
  ...
  public async CombinePaperCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombinePaperCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinePaperConditionObj(): Promise<clsPaperEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objPaperCond = new clsPaperEN();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.paperTitle_q != '') {
        strWhereCond += Format(
        ...
        objPaperCond.SetCondFldValue(clsPaperEN.con_PaperTitle, this.paperTitle_q, 'like');
      }
      if (this.updDate_q != '') {
        strWhereCond += Format(" And {0} like '% {1}%'", clsPaperEN.con_UpdDate, this.updDate_q);
        objPaperCond.SetCondFldValue(clsPaperEN.con_UpdDate, this.updDate_q, 'like');
      }
      if (this.isQuotethesis_q == true) {
        strWhereCond += Format(" And {0} = '1'", clsPaperEN.con_IsQuotethesis);
        objPaperCond.SetCondFldValue(clsPaperEN.con_IsQuotethesis, true, '=');
      } else {
        strWhereCond += Format(" And {0} = '0'", clsPaperEN.con_IsQuotethesis);
        objPaperCond.SetCondFldValue(clsPaperEN.con_IsQuotethesis, false, '=');
      }
      if (this.literatureTypeId_q != '' && this.literatureTypeId_q != '0') {
        strWhereCond += Format(
          " And {0} = '{1}'",
        ...
        objPaperCond.SetCondFldValue(clsPaperEN.con_LiteratureTypeId, this.literatureTypeId_q, '=');
      }
    } catch (objException) {
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombinePaperConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objPaperCond.whereCond = strWhereCond;
    return objPaperCond;
  }
  ...
  public async CombinePaperConditionObj4ExportExcel(): Promise<clsPaperEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objPaperCond = new clsPaperENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.paperTitle_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombinePaperConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
   ...
   **/
  public async BindTab_Paper(divContainer: HTMLDivElement, arrPaperObjLst: Array<clsPaperEN>) {
    ...
    await BindTab(divDataLst, arrPaperObjLst, arrDataColumn, clsPaperEN.con_PaperId, this);
  ...
  public async BindGv_Paper(divList: HTMLDivElement) {
    ...
    try {
      await this.BindTab_Paper(divList, arrPaperExObjLst);
      //console.log("完成BindGv_Paper!");
    } catch (e) {
      //console.trace();
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsPaperEN, b: clsPaperEN): number {
   ...
   **/
  public async SetMajorDirectionId(arrPaperId: Array<string>, strMajorDirectionId: string) {
    const strThisFuncName = this.SetMajorDirectionId.name;
    ...
    try {
      ...
      for (const objInFor of arrPaperObjLst) {
        ...
        ObjectAssign(objPaperEN, objInFor);
        objPaperEN.SetPaperId(objInFor.paperId);
        //objPaperEN.SetMajorDirectionId(strMajorDirectionId);
        let returnBool = false;
        try {
          objPaperEN.sfUpdFldSetStr = objPaperEN.updFldString; //设置哪些字段被修改(脏字段)
          returnBool = await Paper_UpdateRecordAsync(objPaperEN);
        } catch (e) {
          const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
          console.error(strMsg);
          throw strMsg;
  ...
  public get literatureTypeId_q(): string {
    const strValue = GetSelectValueInDivObj(this.divQuery, 'ddlLiteratureTypeId_q');
  ...
  public set literatureTypeId_q(value: string) {
    SetSelectValueByIdInDivObj(this.divQuery, 'ddlLiteratureTypeId_q', value);
  }
  /**
