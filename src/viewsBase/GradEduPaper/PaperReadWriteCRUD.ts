...
import {
  ...
  vPaperReadWrite_GetSubObjLstCache,
  vPaperReadWrite_GetObjByPaperRWIdAsync,
...
import {
  PaperReadWrite_DelRecordAsync,
  PaperReadWrite_ReFreshCache,
  PaperReadWrite_GetObjLstByPaperRWIdLstAsync,
  PaperReadWrite_GetMaxStrIdAsync,
  PaperReadWrite_AddNewRecordAsync,
  PaperReadWrite_DelPaperReadWritesAsync,
...
import {
  vPaperReadWriteEx_FuncMapByFldName,
  vPaperReadWriteEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduPaper/clsvPaperReadWriteExWApi';
...
export abstract class PaperReadWriteCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (PaperReadWriteCRUD.sortvPaperReadWriteBy == '')
        PaperReadWriteCRUD.sortvPaperReadWriteBy = 'paperTitle Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vPaperReadWriteCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('论文读写表({0})导出.xlsx', clsvPaperReadWriteEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vPaperReadWriteCache() {
    const strThisFuncName = this.ExportExcel_vPaperReadWriteCache.name;
    if (PaperReadWriteCRUD.sortvPaperReadWriteBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvPaperReadWriteBy)为空,请检查!(In BindGv_vPaperReadWriteCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvPaperReadWriteCond = await this.CombinevPaperReadWriteConditionObj4ExportExcel();
    objvPaperReadWriteCond.SetCondFldValue(
      clsvPaperReadWriteEN.con_IdCurrEduCls,
      PaperReadWriteCRUD.IdCurrEduClsCache,
      '=',
    );
    let arrvPaperReadWriteObjLst: Array<clsvPaperReadWriteEN> = [];
    try {
      this.recCount = await vPaperReadWrite_GetRecCountByCondCache(
        objvPaperReadWriteCond,
        PaperReadWriteCRUD.IdCurrEduClsCache,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvPaperReadWriteCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvPaperReadWriteObjLst = await vPaperReadWrite_GetSubObjLstCache(
        objvPaperReadWriteCond,
        PaperReadWriteCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    if (arrvPaperReadWriteObjLst.length == 0) {
      const strKey = Format(
        '{0}_{1}',
        clsPaperReadWriteEN._CurrTabName,
        PaperReadWriteCRUD.IdCurrEduClsCache,
      );
    ...
    try {
      ...
      arrvPaperReadWriteObjLst = arrvPaperReadWriteObjLst.sort(this.SortFunExportExcel);
  ...
  public async SetDdl_OperationTypeIdInDiv() {
    await OperationType_BindDdl_OperationTypeIdInDivCache(this.divQuery, 'ddlOperationTypeId_q'); //
  }
  ...
  public async DelRecord(strPaperRWId: string) {
    ...
    try {
      ...
      if (returnInt > 0) {
        PaperReadWrite_ReFreshCache(PaperReadWriteCRUD.IdCurrEduClsCache);
  ...
  public async SelectRecord(strPaperRWId: string) {
    ...
    try {
      const objPaperReadWriteEN = await vPaperReadWrite_GetObjByPaperRWIdAsync(strPaperRWId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vPaperReadWriteCache();
  }
  ...
  public async CombinevPaperReadWriteCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombinePaperReadWriteCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinevPaperReadWriteConditionObj(): Promise<clsvPaperReadWriteEN> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombinePaperReadWriteConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvPaperReadWriteCond.whereCond = strWhereCond;
    return objvPaperReadWriteCond;
  }
  ...
  public async CombinevPaperReadWriteConditionObj4ExportExcel(): Promise<clsvPaperReadWriteEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvPaperReadWriteCond = new clsvPaperReadWriteENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.paperTitle_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombinePaperReadWriteConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async ExtendFldFuncMap(
    arrvPaperReadWriteExObjLst: Array<clsvPaperReadWriteENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vPaperReadWriteCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vPaperReadWriteCache.name;
    if (PaperReadWriteCRUD.sortvPaperReadWriteBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvPaperReadWriteBy)为空,请检查!(In BindGv_vPaperReadWriteCache)',
      );
    ...
    const objvPaperReadWriteCond = await this.CombinevPaperReadWriteConditionObj();
    const strWhereCond = JSON.stringify(objvPaperReadWriteCond);
    ...
    try {
      ...
      if (PaperReadWriteCRUD.sortFunStatic != undefined) {
        strSortFun = PaperReadWriteCRUD.sortFunStatic(PaperReadWriteCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvPaperReadWriteExObjLst = await vPaperReadWriteEx_GetObjExLstByPagerCache(
        objPagerPara,
        PaperReadWriteCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_vPaperReadWrite(divList, arrvPaperReadWriteExObjLst);
      //console.log("完成BindGv_vPaperReadWriteCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvPaperReadWriteEN, b: clsvPaperReadWriteEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      PaperReadWriteCRUD.ascOrDesc4SortFun,
    ...
    PaperReadWriteCRUD.sortFunStatic = sortFun;
    await this.BindGv_vPaperReadWriteCache(this.divList);
  }
  ...
  public async CopyRecord(arrPaperRWId: Array<string>) {
    ...
    try {
      const arrPaperReadWriteObjLst = await PaperReadWrite_GetObjLstByPaperRWIdLstAsync(
        arrPaperRWId,
      );
  ...
  public async DelMultiRecord(arrPaperRWId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strPaperRWId of arrPaperRWId) {
          PaperReadWrite_ReFreshCache(PaperReadWriteCRUD.IdCurrEduClsCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
