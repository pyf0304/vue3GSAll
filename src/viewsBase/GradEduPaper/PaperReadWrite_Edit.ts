...
import { OperationType_BindDdl_OperationTypeIdInDivCache } from '@/ts/L3ForWApi/SysPara/clsOperationTypeWApi';
import {
...
import {
  ...
  PaperReadWrite_CheckPropertyNew,
  PaperReadWrite_AddNewRecordWithMaxIdAsync,
  PaperReadWrite_ReFreshCache,
  PaperReadWrite_GetUniCondStr,
  PaperReadWrite_IsExistRecordAsync,
  PaperReadWrite_GetUniCondStr4Update,
  PaperReadWrite_IsExistAsync,
  PaperReadWrite_GetObjByPaperRWIdAsync,
  PaperReadWrite_CheckProperty4Update,
...
export abstract class PaperReadWrite_Edit {
  ...
  public static objPageEdit: PaperReadWrite_Edit;
  public static objPageEdit2: PaperReadWrite_Edit;
  public static objPageEdit3: PaperReadWrite_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList) {
    ...
    if (PaperReadWrite_Edit.SetPageEdit(this, 1) == true) return;
    if (PaperReadWrite_Edit.SetPageEdit(this, 2) == true) return;
    if (PaperReadWrite_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (PaperReadWrite_Edit.objPageEdit == null) {
          PaperReadWrite_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = PaperReadWrite_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (PaperReadWrite_Edit.objPageEdit2 == null) {
          PaperReadWrite_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = PaperReadWrite_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            PaperReadWrite_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (PaperReadWrite_Edit.objPageEdit3 == null) {
          PaperReadWrite_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = PaperReadWrite_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            PaperReadWrite_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (PaperReadWrite_Edit.objPageEdit != null) {
      const strClassNameOld = PaperReadWrite_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return PaperReadWrite_Edit.objPageEdit;
    }
    if (PaperReadWrite_Edit.objPageEdit2 != null) {
      const strClassNameOld = PaperReadWrite_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return PaperReadWrite_Edit.objPageEdit2;
    }
    if (PaperReadWrite_Edit.objPageEdit3 != null) {
      const strClassNameOld = PaperReadWrite_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return PaperReadWrite_Edit.objPageEdit3;
    }
    return null;
  }
  ...
  public HideDialog_PaperReadWrite() {
    if (PaperReadWrite_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      PaperReadWrite_Edit.EditObj.hideDialog();
    }
  }
   ...
   **/
  public async ShowDialog_PaperReadWrite(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_PaperReadWrite.name;
    if (PaperReadWrite_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (PaperReadWrite_Edit.EditObj == null) {
        const strMsg = Format(
      ...
      await PaperReadWrite_Edit.EditObj.showDialog();
    }
    this.divEdit = PaperReadWrite_Edit.EditObj.$refs.refDivEdit;
    if (this.divEdit == null) {
      if (PaperReadWrite_Edit.times4TestShowDialog < 2) {
        PaperReadWrite_Edit.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_PaperReadWrite(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      PaperReadWrite_Edit.times4TestShowDialog = 0;
    }
    if (strOp === 'Add' || strOp === 'AddWithMaxId') {
      this.btnSubmitPaperReadWrite = '确认添加';
      this.btnCancelPaperReadWrite = '取消添加';
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (PaperReadWrite_Edit.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (PaperReadWrite_Edit.EditObj.dialogVisible == false) {
        const strMsg = Format(
  ...
  public async btnAddNewRecord_Click() {
    ...
    try {
      this.opType = 'Add';
      const bolIsSuccess = await this.ShowDialog_PaperReadWrite(this.opType);
  ...
  public async BindDdl4EditRegionInDiv() {
    const IdCurrEduClsStatic = PaperReadWrite_Edit.IdCurrEduClsStatic; //静态变量;//静态变量
  ...
  public async SetDdl_OperationTypeIdInDiv() {
    await OperationType_BindDdl_OperationTypeIdInDivCache(this.divEdit, 'ddlOperationTypeId'); //
  }
        ...
        case '确认添加':
          ...
          if (this.opType == 'AddWithMaxId') {
            ...
            if (IsNullOrEmpty(returnKeyId) == false) {
              ...
              this.iShowList.BindGvCache(clsPaperReadWriteEN._CurrTabName, returnKeyId);
            }
          } else {
            ...
            if (returnBool == true) {
              ...
              this.iShowList.BindGvCache(clsPaperReadWriteEN._CurrTabName, this.keyId);
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecord() {
    ...
    this.Clear();
    //wucPaperReadWriteB1.paperRWId = PaperReadWriteGetMaxStrId_S();
    try {
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.paperRWId = await PaperReadWrite_GetMaxStrIdAsync();
    try {
      const returnString = await PaperReadWrite_GetMaxStrIdAsync();
      if (returnString == '') {
        const strInfo = Format('获取表PaperReadWrite的最大关键字为空,不成功,请检查!');
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objPaperReadWriteEN);
      ...
      if (returnBool == true) {
        PaperReadWrite_ReFreshCache(PaperReadWrite_Edit.IdCurrEduClsCache);
   ...
   **/
  public async CheckUniCond4Add(objPaperReadWriteEN: clsPaperReadWriteEN): Promise<boolean> {
    const strUniquenessCondition = PaperReadWrite_GetUniCondStr(objPaperReadWriteEN);
   ...
   **/
  public async CheckUniCond4Update(objPaperReadWriteEN: clsPaperReadWriteEN): Promise<boolean> {
    const strUniquenessCondition = PaperReadWrite_GetUniCondStr4Update(objPaperReadWriteEN);
    const bolIsExistCondition = await PaperReadWrite_IsExistRecordAsync(strUniquenessCondition);
    if (bolIsExistCondition == true) {
      const strMsg = Format(
        '不能满足唯一性条件。满足条件：{0}的记录已经存在!',
  ...
  public async ShowData(strPaperRWId: string) {
    ...
    try {
      const objPaperReadWriteENConst = await PaperReadWrite_GetObjByPaperRWIdAsync(strPaperRWId);
      if (objPaperReadWriteENConst == null) {
        const strMsg = Format(
      ...
      objPaperReadWriteEN = objPaperReadWriteENConst;
    } catch (e) {
      const strMsg = Format(
   ...
   **/
  public async GetDataFromPaperReadWriteClass(pobjPaperReadWriteEN: clsPaperReadWriteEN) {
  ...
  public async UpdateRecord(strPaperRWId: string): Promise<boolean> {
    ...
    try {
      ...
      await this.GetDataFromPaperReadWriteClass(objPaperReadWriteEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objPaperReadWriteEN);
      if (bolIsExistCond == false) {
        return false;
      }
   ...
   **/
  public set btnCancelPaperReadWrite(value: string) {
    PaperReadWrite_Edit.EditObj.SetButtonText('btnCancelPaperReadWrite', value);
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitPaperReadWrite(): string {
    const strValue = PaperReadWrite_Edit.EditObj.GetButtonText('btnSubmitPaperReadWrite');
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitPaperReadWrite(value: string) {
    PaperReadWrite_Edit.EditObj.SetButtonText('btnSubmitPaperReadWrite', value);
  }
  /**
   * 备注 (Used In Clear())
   **/
  ...
  public set readerId(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtReaderId', value);
  }
  /**
   * 阅读者Id (Used In PutDataToClass())
   **/
  public get readerId(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtReaderId');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
