/**
 * 类名:gs_ReflectLog_Detail(界面:gs_ReflectLogCRUD)
 * 表名:gs_ReflectLog(01120702)
 * 版本:2024.03.11.1(服务器:WIN-SRV103-116)
 * 日期:2024/03/17 14:04:49
 * 生成者:
 工程名称:问卷调查(0112)
 ...
 * 编程语言:TypeScript
 **/
import { gs_ReflectLog_GetObjBymIdAsync } from '@/ts/L3ForWApi/GradEduTopic/clsgs_ReflectLogWApi';
import { ObjectAssign } from '@/ts/PubFun/clsCommFunc4Web';
import { gs_ReflectLogEx_FuncMapByFldName } from '@/ts/L3ForWApiExShare/GradEduTopic/clsgs_ReflectLogExWApi';
...
/* gs_ReflectLog_Detail 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_DetailCS_TS4TypeScript:GeneCode)
*/
export abstract class gs_ReflectLog_Detail {
  ...
  public HideDialog_gs_ReflectLog() {
    if (gs_ReflectLog_Detail.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      gs_ReflectLog_Detail.DetailObj.hideDialog();
    }
  }
  ...
  public async ShowDialog_gs_ReflectLog(strOp: string): Promise<boolean> {
    ...
    if (gs_ReflectLog_Detail.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (gs_ReflectLog_Detail.DetailObj == null) {
        const strMsg = Format(
      ...
      await gs_ReflectLog_Detail.DetailObj.showDialog();
    }
    this.divDetail = gs_ReflectLog_Detail.DetailObj.$refs.refDivDetail;
    if (this.divDetail == null) {
      if (gs_ReflectLog_Detail.times4TestShowDialog < 2) {
        gs_ReflectLog_Detail.times4TestShowDialog++;
        setTimeout(() => {
      ...
      gs_ReflectLog_Detail.times4TestShowDialog = 0;
    }
    if (strOp === 'Detail') {
      this.btnCancelgs_ReflectLog = '关闭';
    }
    return true;
  }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (gs_ReflectLog_Detail.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (gs_ReflectLog_Detail.DetailObj.dialogVisible == false) {
        const strMsg = Format(
...
*/
  public async btnDetailRecordInTab_Click(strKeyId: string) {
    ...
    this.opType = 'Detail';
    const bolIsSuccess = await this.ShowDialog_gs_ReflectLog('Detail');
...
*/
  public async DetailRecord4Func(lngmId: number) {
    ...
    try {
      ...
      const objgs_ReflectLogENEx = new clsgs_ReflectLogENEx();
      ObjectAssign(objgs_ReflectLogENEx, objgs_ReflectLogEN);
      await gs_ReflectLogEx_FuncMapByFldName(
        clsgs_ReflectLogENEx.con_CreateDateSim,
        objgs_ReflectLogENEx,
      );
      await gs_ReflectLogEx_FuncMapByFldName(
        clsgs_ReflectLogENEx.con_DateTimeSim,
        objgs_ReflectLogENEx,
      );
      this.ShowDetailDataFromgs_ReflectLogClass4Func(objgs_ReflectLogENEx);
...
*/
  public ShowDetailDataFromgs_ReflectLogClass4Func(pobjgs_ReflectLogENEx: clsgs_ReflectLogENEx) {
    this.userId_d = pobjgs_ReflectLogENEx.userId; // 用户ID
    this.reflectLogName_d = pobjgs_ReflectLogENEx.reflectLogName; // 标题
    this.reflectLogContent_d = pobjgs_ReflectLogENEx.reflectLogContent; // 内容
    this.createDateSim_d = pobjgs_ReflectLogENEx.createDateSim; // 简化日期时间
    this.dateTimeSim_d = pobjgs_ReflectLogENEx.dateTimeSim; // 简化日期时间
    this.memo_d = pobjgs_ReflectLogENEx.memo; // 备注
  }
  /**
   * 设置取消按钮的标题(Used In DetailRecord())
   **/
  public set btnCancelgs_ReflectLog(value: string) {
    gs_ReflectLog_Detail.DetailObj.SetButtonText('btnCancelgs_ReflectLog', value);
  }
  /**
   * 建立日期 (Used In ShowDetailDataFromClass4Func())
   **/
  public set createDateSim_d(value: string) {
    SetLabelHtmlByIdInDivObj(this.divDetail, 'lblCreateDateSim_d', value);
  }
  /**
  ...
  /**
   * 内容 (Used In ShowDetailDataFromClass4Func())
   **/
  public set reflectLogContent_d(value: string) {
    SetLabelHtmlByIdInDivObj(this.divDetail, 'lblReflectLogContent_d', value);
  }
  /**
   * 标题 (Used In ShowDetailDataFromClass4Func())
   **/
  public set reflectLogName_d(value: string) {
    SetLabelHtmlByIdInDivObj(this.divDetail, 'lblReflectLogName_d', value);
  }
  /**
