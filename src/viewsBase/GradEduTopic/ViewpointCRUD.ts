...
//import $ from "jquery";
import {
  vViewpoint_GetRecCountByCondCache,
  vViewpoint_GetSubObjLstCache,
...
import {
  ...
  Viewpoint_GetObjLstByViewpointIdLstAsync,
...
import {
  vViewpointEx_FuncMapByFldName,
  vViewpointEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsvViewpointExWApi';
...
export abstract class ViewpointCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (ViewpointCRUD.sortvViewpointBy == '')
        ViewpointCRUD.sortvViewpointBy = 'viewpointName Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vViewpointCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('观点表({0})导出.xlsx', clsvViewpointEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vViewpointCache() {
    const strThisFuncName = this.ExportExcel_vViewpointCache.name;
    if (ViewpointCRUD.sortvViewpointBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvViewpointBy)为空,请检查!(In BindGv_vViewpointCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvViewpointCond = await this.CombinevViewpointConditionObj4ExportExcel();
    objvViewpointCond.SetCondFldValue(
      clsvViewpointEN.con_IdCurrEduCls,
      ViewpointCRUD.IdCurrEduClsCache,
      '=',
    );
    let arrvViewpointObjLst: Array<clsvViewpointEN> = [];
    try {
      this.recCount = await vViewpoint_GetRecCountByCondCache(
        objvViewpointCond,
        ViewpointCRUD.IdCurrEduClsCache,
      );
      if (this.recCount == 0) {
      ...
      arrvViewpointObjLst = await vViewpoint_GetSubObjLstCache(
        objvViewpointCond,
        ViewpointCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      ...
      arrvViewpointObjLst = arrvViewpointObjLst.sort(this.SortFunExportExcel);
  ...
  public async DelRecord(strViewpointId: string) {
    ...
    try {
      ...
      if (returnInt > 0) {
        Viewpoint_ReFreshCache(ViewpointCRUD.IdCurrEduClsCache);
  ...
  public async SelectRecord(strViewpointId: string) {
    ...
    try {
      const objViewpointEN = await vViewpoint_GetObjByViewpointIdAsync(strViewpointId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vViewpointCache();
  }
    ...
    try {
      ...
      if (this.vPProposePeople_q != '') {
        strWhereCond += Format(
          ...
          this.vPProposePeople_q,
        );
      }
    } catch (objException) {
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombineViewpointCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinevViewpointConditionObj(): Promise<clsvViewpointEN> {
    ...
    try {
      ...
      if (this.reason_q != '') {
        strWhereCond += Format(" And {0} like '% {1}%'", clsvViewpointEN.con_Reason, this.reason_q);
        objvViewpointCond.SetCondFldValue(clsvViewpointEN.con_Reason, this.reason_q, 'like');
      }
      if (this.vPProposePeople_q != '') {
        strWhereCond += Format(
          " And {0} like '% {1}%'",
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombineViewpointConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvViewpointCond.whereCond = strWhereCond;
    return objvViewpointCond;
  }
  ...
  public async CombinevViewpointConditionObj4ExportExcel(): Promise<clsvViewpointEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvViewpointCond = new clsvViewpointENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.viewpointName_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombineViewpointConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async ExtendFldFuncMap(
    arrvViewpointExObjLst: Array<clsvViewpointENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vViewpointCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vViewpointCache.name;
    if (ViewpointCRUD.sortvViewpointBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvViewpointBy)为空,请检查!(In BindGv_vViewpointCache)',
      );
    ...
    const objvViewpointCond = await this.CombinevViewpointConditionObj();
    const strWhereCond = JSON.stringify(objvViewpointCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvViewpointExObjLst: Array<clsvViewpointENEx> = [];
    try {
      this.recCount = await vViewpoint_GetRecCountByCondCache(
        objvViewpointCond,
        ViewpointCRUD.IdCurrEduClsCache,
      ...
      if (ViewpointCRUD.sortFunStatic != undefined) {
        strSortFun = ViewpointCRUD.sortFunStatic(ViewpointCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvViewpointExObjLst = await vViewpointEx_GetObjExLstByPagerCache(
        objPagerPara,
        ViewpointCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_vViewpoint(divList, arrvViewpointExObjLst);
      //console.log("完成BindGv_vViewpointCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvViewpointEN, b: clsvViewpointEN): number {
  ...
  public async DelMultiRecord(arrViewpointId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strViewpointId of arrViewpointId) {
          Viewpoint_ReFreshCache(ViewpointCRUD.IdCurrEduClsCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
  ...
  public get reason_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtReason_q');
  ...
  public set reason_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtReason_q', value);
  }
  /**
  ...
  public get vPProposePeople_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtVPProposePeople_q');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set vPProposePeople_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtVPProposePeople_q', value);
  }
}
