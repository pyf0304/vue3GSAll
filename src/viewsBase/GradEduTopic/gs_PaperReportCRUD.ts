...
//import $ from "jquery";
import {
  vgs_PaperReport_GetRecCountByCondCache,
  vgs_PaperReport_GetSubObjLstCache,
  vgs_PaperReport_GetObjByReportIdAsync,
} from '@/ts/L3ForWApi/GradEduTopic/clsvgs_PaperReportWApi';
import {
...
import {
  ...
  gs_PaperReport_GetObjLstByReportIdLstAsync,
  gs_PaperReport_GetMaxStrIdAsync,
  gs_PaperReport_AddNewRecordAsync,
  gs_PaperReport_Delgs_PaperReportsAsync,
} from '@/ts/L3ForWApi/GradEduTopic/clsgs_PaperReportWApi';
import { clsvgs_PaperReportENEx } from '@/ts/L0Entity/GradEduTopic/clsvgs_PaperReportENEx';
import {
  BindTab_V,
  arrSelectedKeys,
  confirmDel,
...
import {
  vgs_PaperReportEx_FuncMapByFldName,
  vgs_PaperReportEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsvgs_PaperReportExWApi';
...
export abstract class gs_PaperReportCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (gs_PaperReportCRUD.sortvgs_PaperReportBy == '')
        gs_PaperReportCRUD.sortvgs_PaperReportBy = 'updDate Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vgs_PaperReportCache(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrvgs_PaperReportObjLst: Array<clsvgs_PaperReportEN>,
    ...
    const strFileName = Format('论文汇报表({0})导出.xlsx', clsvgs_PaperReportEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vgs_PaperReportCache() {
    const strThisFuncName = this.ExportExcel_vgs_PaperReportCache.name;
    if (gs_PaperReportCRUD.sortvgs_PaperReportBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvgs_PaperReportBy)为空,请检查!(In BindGv_vgs_PaperReportCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvgs_PaperReportCond = await this.Combinevgs_PaperReportConditionObj4ExportExcel();
    let arrvgs_PaperReportObjLst: Array<clsvgs_PaperReportEN> = [];
    try {
      this.recCount = await vgs_PaperReport_GetRecCountByCondCache(objvgs_PaperReportCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvgs_PaperReportCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvgs_PaperReportObjLst = await vgs_PaperReport_GetSubObjLstCache(objvgs_PaperReportCond);
    } catch (e) {
    ...
    try {
      ...
      arrvgs_PaperReportObjLst = arrvgs_PaperReportObjLst.sort(this.SortFunExportExcel);
  ...
  public async SelectRecord(strReportId: string) {
    ...
    try {
      const objgs_PaperReportEN = await vgs_PaperReport_GetObjByReportIdAsync(strReportId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vgs_PaperReportCache();
  }
  ...
  public async Combinevgs_PaperReportConditionObj(): Promise<clsvgs_PaperReportEN> {
    ...
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    objvgs_PaperReportCond.whereCond = strWhereCond;
    return objvgs_PaperReportCond;
  }
  ...
  public async Combinevgs_PaperReportConditionObj4ExportExcel(): Promise<clsvgs_PaperReportEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    const strWhereCond = ' 1 = 1 ';
    const objvgs_PaperReportCond = new clsvgs_PaperReportENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    objvgs_PaperReportCond.whereCond = strWhereCond;
    return objvgs_PaperReportCond;
  }
   ...
   **/
  public async BindTab_vgs_PaperReport(
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      },
      {
        ...
        funcName: (strKeyId: string, strText: string) => {
          ...
          btn1.onclick = function () {
            gs_PaperReportCRUD.vuebtn_Click('UpdateRecordInTab', strKeyId);
          };
          return btn1;
        },
      },
      {
        ...
        funcName: (strKeyId: string, strText: string) => {
          ...
          btn1.className = 'btn btn-outline-info btn-sm';
          btn1.onclick = function () {
            gs_PaperReportCRUD.vuebtn_Click('DelRecordInTab', strKeyId);
          };
          return btn1;
        },
      },
  ...
  public async ExtendFldFuncMap(
    arrvgs_PaperReportExObjLst: Array<clsvgs_PaperReportENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vgs_PaperReportCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vgs_PaperReportCache.name;
    if (gs_PaperReportCRUD.sortvgs_PaperReportBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvgs_PaperReportBy)为空,请检查!(In BindGv_vgs_PaperReportCache)',
      );
    ...
    const objvgs_PaperReportCond = await this.Combinevgs_PaperReportConditionObj();
    const strWhereCond = JSON.stringify(objvgs_PaperReportCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvgs_PaperReportExObjLst: Array<clsvgs_PaperReportENEx> = [];
    try {
      this.recCount = await vgs_PaperReport_GetRecCountByCondCache(objvgs_PaperReportCond);
      if (this.recCount == 0) {
      ...
      if (gs_PaperReportCRUD.sortFunStatic != undefined) {
        strSortFun = gs_PaperReportCRUD.sortFunStatic(gs_PaperReportCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvgs_PaperReportExObjLst = await vgs_PaperReportEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    try {
      await this.BindTab_vgs_PaperReport(divList, arrvgs_PaperReportExObjLst);
      //console.log("完成BindGv_vgs_PaperReportCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvgs_PaperReportEN, b: clsvgs_PaperReportEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      gs_PaperReportCRUD.ascOrDesc4SortFun,
    ...
    gs_PaperReportCRUD.sortFunStatic = sortFun;
    await this.BindGv_vgs_PaperReportCache(this.divList);
  }
  ...
  public async DelMultiRecord(arrReportId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strReportId of arrReportId) {
          gs_PaperReport_ReFreshCache();
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public Showgs_PaperReportObj(
    divContainer: HTMLDivElement,
    objgs_PaperReport: clsgs_PaperReportEN,
  ) {
    if (divContainer == null) {
