...
import {
  ...
  SysSkill_CheckPropertyNew,
  SysSkill_AddNewRecordWithMaxIdAsync,
  SysSkill_ReFreshCache,
  SysSkill_GetUniCondStr,
  SysSkill_IsExistRecordAsync,
  SysSkill_GetUniCondStr4Update,
  SysSkill_IsExistAsync,
  SysSkill_GetObjBySkillIdAsync,
  SysSkill_CheckProperty4Update,
...
export abstract class SysSkill_Edit {
  ...
  public static objPageEdit: SysSkill_Edit;
  public static objPageEdit2: SysSkill_Edit;
  public static objPageEdit3: SysSkill_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList) {
    ...
    if (SysSkill_Edit.SetPageEdit(this, 1) == true) return;
    if (SysSkill_Edit.SetPageEdit(this, 2) == true) return;
    if (SysSkill_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (SysSkill_Edit.objPageEdit == null) {
          SysSkill_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = SysSkill_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (SysSkill_Edit.objPageEdit2 == null) {
          SysSkill_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = SysSkill_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            SysSkill_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (SysSkill_Edit.objPageEdit3 == null) {
          SysSkill_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = SysSkill_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            SysSkill_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (SysSkill_Edit.objPageEdit != null) {
      const strClassNameOld = SysSkill_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return SysSkill_Edit.objPageEdit;
    }
    if (SysSkill_Edit.objPageEdit2 != null) {
      const strClassNameOld = SysSkill_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return SysSkill_Edit.objPageEdit2;
    }
    if (SysSkill_Edit.objPageEdit3 != null) {
      const strClassNameOld = SysSkill_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return SysSkill_Edit.objPageEdit3;
    }
    return null;
  }
  ...
  public HideDialog_SysSkill() {
    if (SysSkill_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      SysSkill_Edit.EditObj.hideDialog();
    }
  }
   ...
   **/
  public async ShowDialog_SysSkill(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_SysSkill.name;
    if (SysSkill_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (SysSkill_Edit.EditObj == null) {
        const strMsg = Format(
      ...
      await SysSkill_Edit.EditObj.showDialog();
    }
    this.divEdit = SysSkill_Edit.EditObj.$refs.refDivEdit;
    if (this.divEdit == null) {
      if (SysSkill_Edit.times4TestShowDialog < 2) {
        SysSkill_Edit.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_SysSkill(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      SysSkill_Edit.times4TestShowDialog = 0;
    }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (SysSkill_Edit.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (SysSkill_Edit.EditObj.dialogVisible == false) {
        const strMsg = Format(
  ...
  public async btnSubmit_Click() {
    ...
    const strCommandText: string = this.btnSubmitSysSkill;
    try {
        ...
        case '确认添加':
          ...
          if (this.opType == 'AddWithMaxId') {
            ...
            if (IsNullOrEmpty(returnKeyId) == false) {
              ...
              this.iShowList.BindGvCache(clsSysSkillEN._CurrTabName, returnKeyId);
            }
          } else {
            ...
            if (returnBool == true) {
              ...
              this.iShowList.BindGvCache(clsSysSkillEN._CurrTabName, this.keyId);
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecord() {
    ...
    this.Clear();
    //wucSysSkillB1.skillId = SysSkillGetMaxStrId_S();
    try {
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.skillId = await SysSkill_GetMaxStrIdAsync();
    try {
      const returnString = await SysSkill_GetMaxStrIdAsync();
      if (returnString == '') {
        const strInfo = Format('获取表SysSkill的最大关键字为空,不成功,请检查!');
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objSysSkillEN);
      ...
      if (returnBool == true) {
        SysSkill_ReFreshCache(SysSkill_Edit.IdCurrEduClsCache);
   ...
   **/
  public async CheckUniCond4Add(objSysSkillEN: clsSysSkillEN): Promise<boolean> {
    const strUniquenessCondition = SysSkill_GetUniCondStr(objSysSkillEN);
   ...
   **/
  public async CheckUniCond4Update(objSysSkillEN: clsSysSkillEN): Promise<boolean> {
    const strUniquenessCondition = SysSkill_GetUniCondStr4Update(objSysSkillEN);
    const bolIsExistCondition = await SysSkill_IsExistRecordAsync(strUniquenessCondition);
    if (bolIsExistCondition == true) {
      const strMsg = Format(
        '不能满足唯一性条件。满足条件：{0}的记录已经存在!',
  ...
  public async ShowData(strSkillId: string) {
    ...
    try {
      const objSysSkillENConst = await SysSkill_GetObjBySkillIdAsync(strSkillId);
      if (objSysSkillENConst == null) {
        const strMsg = Format(
      ...
      objSysSkillEN = objSysSkillENConst;
    } catch (e) {
      const strMsg = Format(
  ...
  public async UpdateRecord(strSkillId: string): Promise<boolean> {
    ...
    try {
      ...
      await this.GetDataFromSysSkillClass(objSysSkillEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objSysSkillEN);
      if (bolIsExistCond == false) {
        return false;
      }
   ...
   **/
  public set btnCancelSysSkill(value: string) {
    SysSkill_Edit.EditObj.SetButtonText('btnCancelSysSkill', value);
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitSysSkill(): string {
    const strValue = SysSkill_Edit.EditObj.GetButtonText('btnSubmitSysSkill');
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitSysSkill(value: string) {
    SysSkill_Edit.EditObj.SetButtonText('btnSubmitSysSkill', value);
  }
  /**
   * 引用Id (Used In Clear())
   **/
  public set citationId(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtCitationId', value);
  }
  /**
   * 引用Id (Used In PutDataToClass())
   **/
  public get citationId(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtCitationId');
  ...
  public set process(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtProcess', value);
  }
  /**
   * 实施过程 (Used In PutDataToClass())
   **/
  public get process(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtProcess');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set skillName(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtSkillName', value);
  }
  /**
   * 技能名称 (Used In PutDataToClass())
   **/
  public get skillName(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtSkillName');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
