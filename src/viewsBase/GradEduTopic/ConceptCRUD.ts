...
import {
  ...
  Concept_GetSubObjLstCache,
  Concept_DelRecordAsync,
  Concept_ReFreshCache,
  Concept_GetObjByConceptIdAsync,
  Concept_GetObjLstByConceptIdLstAsync,
  Concept_GetMaxStrIdAsync,
  Concept_AddNewRecordAsync,
  Concept_DelConceptsAsync,
...
import {
  ConceptEx_FuncMapByFldName,
  ConceptEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsConceptExWApi';
...
export abstract class ConceptCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (ConceptCRUD.sortConceptBy == '') ConceptCRUD.sortConceptBy = 'conceptContent Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_ConceptCache(this.divList);
    } catch (e) {
  ...
  public CombineData(arrConceptObjLst: Array<clsConceptEN>, arrDataColumn: Array<clsDataColumn>) {
    ...
    const strFileName = Format('观点概念({0})导出.xlsx', clsConceptEN._CurrTabName);
   ...
   **/
  public async ExportExcel_ConceptCache() {
    const strThisFuncName = this.ExportExcel_ConceptCache.name;
    if (ConceptCRUD.sortConceptBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortConceptBy)为空,请检查!(In BindGv_ConceptCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objConceptCond = await this.CombineConceptConditionObj4ExportExcel();
    objConceptCond.SetCondFldValue(
      clsConceptEN.con_IdCurrEduCls,
      ConceptCRUD.IdCurrEduClsCache,
      '=',
    );
    let arrConceptObjLst: Array<clsConceptEN> = [];
    try {
      this.recCount = await Concept_GetRecCountByCondCache(
        objConceptCond,
        ConceptCRUD.IdCurrEduClsCache,
      );
      if (this.recCount == 0) {
      ...
      arrConceptObjLst = await Concept_GetSubObjLstCache(
        objConceptCond,
        ConceptCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    if (arrConceptObjLst.length == 0) {
      const strKey = Format('{0}_{1}', clsConceptEN._CurrTabName, ConceptCRUD.IdCurrEduClsCache);
    ...
    try {
      ...
      arrConceptObjLst = arrConceptObjLst.sort(this.SortFunExportExcel);
  ...
  public async DelRecord(strConceptId: string) {
    ...
    try {
      ...
      if (returnInt > 0) {
        Concept_ReFreshCache(ConceptCRUD.IdCurrEduClsCache);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_ConceptCache();
  }
  ...
  public async CombineConceptCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombineConceptCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombineConceptConditionObj(): Promise<clsConceptEN> {
    ...
    try {
      if (this.conceptName_q != '') {
        ...
        objConceptCond.SetCondFldValue(clsConceptEN.con_ConceptName, this.conceptName_q, 'like');
      }
      if (this.isSubmit_q == true) {
        strWhereCond += Format(" And {0} = '1'", clsConceptEN.con_IsSubmit);
        objConceptCond.SetCondFldValue(clsConceptEN.con_IsSubmit, true, '=');
      } else {
        strWhereCond += Format(" And {0} = '0'", clsConceptEN.con_IsSubmit);
        objConceptCond.SetCondFldValue(clsConceptEN.con_IsSubmit, false, '=');
      }
    } catch (objException) {
      const strMsg: string = Format(
  ...
  public async CombineConceptConditionObj4ExportExcel(): Promise<clsConceptEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objConceptCond = new clsConceptENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.conceptName_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombineConceptConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async BindTab_Concept(
    ...
    await BindTab(divDataLst, arrConceptObjLst, arrDataColumn, clsConceptEN.con_ConceptId, this);
  ...
  public async ExtendFldFuncMap(
    arrConceptExObjLst: Array<clsConceptENEx>,
  ...
  public async BindGv_ConceptCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_ConceptCache.name;
    if (ConceptCRUD.sortConceptBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortConceptBy)为空,请检查!(In BindGv_ConceptCache)',
      );
    ...
    const objConceptCond = await this.CombineConceptConditionObj();
    const strWhereCond = JSON.stringify(objConceptCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrConceptExObjLst: Array<clsConceptENEx> = [];
    try {
      this.recCount = await Concept_GetRecCountByCondCache(
        objConceptCond,
        ConceptCRUD.IdCurrEduClsCache,
      ...
      if (this.recCount == 0) {
        ...
        lblMsg.innerHTML = Format('根据条件:[{0}]获取的对象列表数为0!', objConceptCond.whereCond);
        if (divDataLst != null) {
      ...
      if (ConceptCRUD.sortFunStatic != undefined) {
        strSortFun = ConceptCRUD.sortFunStatic(ConceptCRUD.ascOrDesc4SortFun);
      }
      ...
      arrConceptExObjLst = await ConceptEx_GetObjExLstByPagerCache(
        objPagerPara,
        ConceptCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_Concept(divList, arrConceptExObjLst);
      //console.log("完成BindGv_ConceptCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsConceptEN, b: clsConceptEN): number {
  ...
  public async DelMultiRecord(arrConceptId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strConceptId of arrConceptId) {
          Concept_ReFreshCache(ConceptCRUD.IdCurrEduClsCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public ShowConceptObj(divContainer: HTMLDivElement, objConcept: clsConceptEN) {
    if (divContainer == null) {
