...
import { clsgs_TeacherTaskENEx } from '@/ts/L0Entity/GradEduTopic/clsgs_TeacherTaskENEx';
import {
  BindTab,
  arrSelectedKeys,
  confirmDel,
...
import {
  gs_TeacherTaskEx_FuncMapByFldName,
  gs_TeacherTaskEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsgs_TeacherTaskExWApi';
...
export abstract class gs_TeacherTaskCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (gs_TeacherTaskCRUD.sortgs_TeacherTaskBy == '')
        gs_TeacherTaskCRUD.sortgs_TeacherTaskBy = 'missionStatement Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_gs_TeacherTaskCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('教师任务({0})导出.xlsx', clsgs_TeacherTaskEN._CurrTabName);
   ...
   **/
  public async ExportExcel_gs_TeacherTaskCache() {
    const strThisFuncName = this.ExportExcel_gs_TeacherTaskCache.name;
    if (gs_TeacherTaskCRUD.sortgs_TeacherTaskBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortgs_TeacherTaskBy)为空,请检查!(In BindGv_gs_TeacherTaskCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objgs_TeacherTaskCond = await this.Combinegs_TeacherTaskConditionObj4ExportExcel();
    let arrgs_TeacherTaskObjLst: Array<clsgs_TeacherTaskEN> = [];
    try {
      this.recCount = await gs_TeacherTask_GetRecCountByCondCache(objgs_TeacherTaskCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objgs_TeacherTaskCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrgs_TeacherTaskObjLst = await gs_TeacherTask_GetSubObjLstCache(objgs_TeacherTaskCond);
    } catch (e) {
    ...
    try {
      ...
      arrgs_TeacherTaskObjLst = arrgs_TeacherTaskObjLst.sort(this.SortFunExportExcel);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_gs_TeacherTaskCache();
  }
  ...
  public async Combinegs_TeacherTaskCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(Combinegs_TeacherTaskCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async Combinegs_TeacherTaskConditionObj(): Promise<clsgs_TeacherTaskEN> {
    ...
    try {
      if (this.missionStatement_q != '') {
        ...
        objgs_TeacherTaskCond.SetCondFldValue(
          clsgs_TeacherTaskEN.con_MissionStatement,
          this.missionStatement_q,
          'like',
        );
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(Combinegs_TeacherTaskConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objgs_TeacherTaskCond.whereCond = strWhereCond;
    return objgs_TeacherTaskCond;
  }
  ...
  public async Combinegs_TeacherTaskConditionObj4ExportExcel(): Promise<clsgs_TeacherTaskEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objgs_TeacherTaskCond = new clsgs_TeacherTaskENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.missionStatement_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(Combinegs_TeacherTaskConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async ExtendFldFuncMap(
    arrgs_TeacherTaskExObjLst: Array<clsgs_TeacherTaskENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
  ...
  public async BindGv_gs_TeacherTaskCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_gs_TeacherTaskCache.name;
    if (gs_TeacherTaskCRUD.sortgs_TeacherTaskBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortgs_TeacherTaskBy)为空,请检查!(In BindGv_gs_TeacherTaskCache)',
      );
    ...
    const objgs_TeacherTaskCond = await this.Combinegs_TeacherTaskConditionObj();
    const strWhereCond = JSON.stringify(objgs_TeacherTaskCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrgs_TeacherTaskExObjLst: Array<clsgs_TeacherTaskENEx> = [];
    try {
      this.recCount = await gs_TeacherTask_GetRecCountByCondCache(objgs_TeacherTaskCond);
      if (this.recCount == 0) {
      ...
      if (gs_TeacherTaskCRUD.sortFunStatic != undefined) {
        strSortFun = gs_TeacherTaskCRUD.sortFunStatic(gs_TeacherTaskCRUD.ascOrDesc4SortFun);
      }
      ...
      arrgs_TeacherTaskExObjLst = await gs_TeacherTaskEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    try {
      await this.BindTab_gs_TeacherTask(divList, arrgs_TeacherTaskExObjLst);
      //console.log("完成BindGv_gs_TeacherTaskCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsgs_TeacherTaskEN, b: clsgs_TeacherTaskEN): number {
  ...
  public get missionStatement_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtMissionStatement_q');
  ...
  public set missionStatement_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtMissionStatement_q', value);
  }
  /**
