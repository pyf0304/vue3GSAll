...
//import $ from "jquery";
import {
  vRTPaperRela_GetRecCountByCondCache,
  vRTPaperRela_GetSubObjLstCache,
...
import {
  ...
  RTPaperRela_GetObjLstBymIdLstAsync,
  RTPaperRela_AddNewRecordAsync,
  RTPaperRela_DelRTPaperRelasAsync,
...
import {
  vRTPaperRelaEx_FuncMapByFldName,
  vRTPaperRelaEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsvRTPaperRelaExWApi';
...
export abstract class RTPaperRelaCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (RTPaperRelaCRUD.sortvRTPaperRelaBy == '')
        RTPaperRelaCRUD.sortvRTPaperRelaBy = 'updDate Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vRTPaperRelaCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('RTPaperRela({0})导出.xlsx', clsvRTPaperRelaEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vRTPaperRelaCache() {
    const strThisFuncName = this.ExportExcel_vRTPaperRelaCache.name;
    if (RTPaperRelaCRUD.sortvRTPaperRelaBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvRTPaperRelaBy)为空,请检查!(In BindGv_vRTPaperRelaCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvRTPaperRelaCond = await this.CombinevRTPaperRelaConditionObj4ExportExcel();
    let arrvRTPaperRelaObjLst: Array<clsvRTPaperRelaEN> = [];
    try {
      this.recCount = await vRTPaperRela_GetRecCountByCondCache(objvRTPaperRelaCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvRTPaperRelaCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvRTPaperRelaObjLst = await vRTPaperRela_GetSubObjLstCache(objvRTPaperRelaCond);
    } catch (e) {
    ...
    try {
      ...
      arrvRTPaperRelaObjLst = arrvRTPaperRelaObjLst.sort(this.SortFunExportExcel);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vRTPaperRelaCache();
  }
  ...
  public async CombinevRTPaperRelaCondition(): Promise<string> {
    ...
    try {
      if (this.topicName_q != '') {
        strWhereCond += Format(
          " And {0} like '% {1}%'",
          clsvRTPaperRelaEN.con_TopicName,
          this.topicName_q,
        );
      }
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombineRTPaperRelaCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinevRTPaperRelaConditionObj(): Promise<clsvRTPaperRelaEN> {
    ...
    try {
      if (this.topicName_q != '') {
        ...
        objvRTPaperRelaCond.SetCondFldValue(
          clsvRTPaperRelaEN.con_TopicName,
          this.topicName_q,
          'like',
        );
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombineRTPaperRelaConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvRTPaperRelaCond.whereCond = strWhereCond;
    return objvRTPaperRelaCond;
  }
  ...
  public async CombinevRTPaperRelaConditionObj4ExportExcel(): Promise<clsvRTPaperRelaEN> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombineRTPaperRelaConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async ExtendFldFuncMap(
    arrvRTPaperRelaExObjLst: Array<clsvRTPaperRelaENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vRTPaperRelaCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vRTPaperRelaCache.name;
    if (RTPaperRelaCRUD.sortvRTPaperRelaBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvRTPaperRelaBy)为空,请检查!(In BindGv_vRTPaperRelaCache)',
      );
    ...
    const objvRTPaperRelaCond = await this.CombinevRTPaperRelaConditionObj();
    const strWhereCond = JSON.stringify(objvRTPaperRelaCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvRTPaperRelaExObjLst: Array<clsvRTPaperRelaENEx> = [];
    try {
      this.recCount = await vRTPaperRela_GetRecCountByCondCache(objvRTPaperRelaCond);
      if (this.recCount == 0) {
      ...
      if (RTPaperRelaCRUD.sortFunStatic != undefined) {
        strSortFun = RTPaperRelaCRUD.sortFunStatic(RTPaperRelaCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvRTPaperRelaExObjLst = await vRTPaperRelaEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    try {
      await this.BindTab_vRTPaperRela(divList, arrvRTPaperRelaExObjLst);
      //console.log("完成BindGv_vRTPaperRelaCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvRTPaperRelaEN, b: clsvRTPaperRelaEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      RTPaperRelaCRUD.ascOrDesc4SortFun,
    ...
    RTPaperRelaCRUD.sortFunStatic = sortFun;
    await this.BindGv_vRTPaperRelaCache(this.divList);
  }
   ...
   **/
  public ShowRTPaperRelaObj(divContainer: HTMLDivElement, objRTPaperRela: clsRTPaperRelaEN) {
    if (divContainer == null) {
