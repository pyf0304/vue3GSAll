...
//import $ from "jquery";
import {
  gs_TopicRole_GetRecCountByCondCache,
  gs_TopicRole_GetSubObjLstCache,
  gs_TopicRole_DelRecordAsync,
  gs_TopicRole_ReFreshCache,
  gs_TopicRole_GetObjByTopicRoleIdAsync,
  gs_TopicRole_GetObjLstByTopicRoleIdLstAsync,
  gs_TopicRole_GetMaxStrIdAsync,
  gs_TopicRole_AddNewRecordAsync,
  gs_TopicRole_Delgs_TopicRolesAsync,
...
import {
  gs_TopicRoleEx_FuncMapByFldName,
  gs_TopicRoleEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsgs_TopicRoleExWApi';
...
export abstract class gs_TopicRoleCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (gs_TopicRoleCRUD.sortgs_TopicRoleBy == '')
        gs_TopicRoleCRUD.sortgs_TopicRoleBy = 'topicRoleId Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_gs_TopicRoleCache(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrgs_TopicRoleObjLst: Array<clsgs_TopicRoleEN>,
    ...
    const strFileName = Format('主题权限表({0})导出.xlsx', clsgs_TopicRoleEN._CurrTabName);
   ...
   **/
  public async ExportExcel_gs_TopicRoleCache() {
    const strThisFuncName = this.ExportExcel_gs_TopicRoleCache.name;
    if (gs_TopicRoleCRUD.sortgs_TopicRoleBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortgs_TopicRoleBy)为空,请检查!(In BindGv_gs_TopicRoleCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objgs_TopicRoleCond = await this.Combinegs_TopicRoleConditionObj4ExportExcel();
    let arrgs_TopicRoleObjLst: Array<clsgs_TopicRoleEN> = [];
    try {
      this.recCount = await gs_TopicRole_GetRecCountByCondCache(objgs_TopicRoleCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objgs_TopicRoleCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrgs_TopicRoleObjLst = await gs_TopicRole_GetSubObjLstCache(objgs_TopicRoleCond);
    } catch (e) {
    ...
    try {
      ...
      arrgs_TopicRoleObjLst = arrgs_TopicRoleObjLst.sort(this.SortFunExportExcel);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_gs_TopicRoleCache();
  }
  ...
  public async Combinegs_TopicRoleConditionObj(): Promise<clsgs_TopicRoleEN> {
    ...
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    objgs_TopicRoleCond.whereCond = strWhereCond;
    return objgs_TopicRoleCond;
  }
  ...
  public async Combinegs_TopicRoleConditionObj4ExportExcel(): Promise<clsgs_TopicRoleEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    const strWhereCond = ' 1 = 1 ';
    const objgs_TopicRoleCond = new clsgs_TopicRoleENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    objgs_TopicRoleCond.whereCond = strWhereCond;
    return objgs_TopicRoleCond;
  }
   ...
   **/
  public async BindTab_gs_TopicRole(
  ...
  public async ExtendFldFuncMap(
    arrgs_TopicRoleExObjLst: Array<clsgs_TopicRoleENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_gs_TopicRoleCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_gs_TopicRoleCache.name;
    if (gs_TopicRoleCRUD.sortgs_TopicRoleBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortgs_TopicRoleBy)为空,请检查!(In BindGv_gs_TopicRoleCache)',
      );
    ...
    const objgs_TopicRoleCond = await this.Combinegs_TopicRoleConditionObj();
    const strWhereCond = JSON.stringify(objgs_TopicRoleCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrgs_TopicRoleExObjLst: Array<clsgs_TopicRoleENEx> = [];
    try {
      this.recCount = await gs_TopicRole_GetRecCountByCondCache(objgs_TopicRoleCond);
      if (this.recCount == 0) {
      ...
      if (gs_TopicRoleCRUD.sortFunStatic != undefined) {
        strSortFun = gs_TopicRoleCRUD.sortFunStatic(gs_TopicRoleCRUD.ascOrDesc4SortFun);
      }
      ...
      arrgs_TopicRoleExObjLst = await gs_TopicRoleEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
    ...
    try {
      await this.BindTab_gs_TopicRole(divList, arrgs_TopicRoleExObjLst);
      //console.log("完成BindGv_gs_TopicRoleCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsgs_TopicRoleEN, b: clsgs_TopicRoleEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      gs_TopicRoleCRUD.ascOrDesc4SortFun,
    ...
    gs_TopicRoleCRUD.sortFunStatic = sortFun;
    await this.BindGv_gs_TopicRoleCache(this.divList);
  }
  ...
  public async CopyRecord(arrTopicRoleId: Array<string>) {
    ...
    try {
      const arrgs_TopicRoleObjLst = await gs_TopicRole_GetObjLstByTopicRoleIdLstAsync(
        arrTopicRoleId,
      );
  ...
  public async DelMultiRecord(arrTopicRoleId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strTopicRoleId of arrTopicRoleId) {
          gs_TopicRole_ReFreshCache();
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public Showgs_TopicRoleObj(divContainer: HTMLDivElement, objgs_TopicRole: clsgs_TopicRoleEN) {
    if (divContainer == null) {
