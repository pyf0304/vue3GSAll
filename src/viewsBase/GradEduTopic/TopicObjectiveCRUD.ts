...
import {
  ...
  vTopicObjective_GetSubObjLstCache,
  vTopicObjective_GetObjByTopicObjectiveIdAsync,
...
import {
  ...
  TopicObjective_GetObjLstByTopicObjectiveIdLstAsync,
...
import {
  vTopicObjectiveEx_FuncMapByFldName,
  vTopicObjectiveEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsvTopicObjectiveExWApi';
...
export abstract class TopicObjectiveCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (TopicObjectiveCRUD.sortvTopicObjectiveBy == '')
        TopicObjectiveCRUD.sortvTopicObjectiveBy = 'paperTitle Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vTopicObjectiveCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('主题客观表({0})导出.xlsx', clsvTopicObjectiveEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vTopicObjectiveCache() {
    const strThisFuncName = this.ExportExcel_vTopicObjectiveCache.name;
    if (TopicObjectiveCRUD.sortvTopicObjectiveBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvTopicObjectiveBy)为空,请检查!(In BindGv_vTopicObjectiveCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvTopicObjectiveCond = await this.CombinevTopicObjectiveConditionObj4ExportExcel();
    objvTopicObjectiveCond.SetCondFldValue(
      clsvTopicObjectiveEN.con_IdCurrEduCls,
      TopicObjectiveCRUD.IdCurrEduClsCache,
      '=',
    );
    let arrvTopicObjectiveObjLst: Array<clsvTopicObjectiveEN> = [];
    try {
      this.recCount = await vTopicObjective_GetRecCountByCondCache(
        objvTopicObjectiveCond,
        TopicObjectiveCRUD.IdCurrEduClsCache,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvTopicObjectiveCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvTopicObjectiveObjLst = await vTopicObjective_GetSubObjLstCache(
        objvTopicObjectiveCond,
        TopicObjectiveCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    if (arrvTopicObjectiveObjLst.length == 0) {
      const strKey = Format(
        '{0}_{1}',
        clsTopicObjectiveEN._CurrTabName,
        TopicObjectiveCRUD.IdCurrEduClsCache,
      );
    ...
    try {
      ...
      arrvTopicObjectiveObjLst = arrvTopicObjectiveObjLst.sort(this.SortFunExportExcel);
  ...
  public async DelRecord(strTopicObjectiveId: string) {
    ...
    try {
      ...
      if (returnInt > 0) {
        TopicObjective_ReFreshCache(TopicObjectiveCRUD.IdCurrEduClsCache);
  ...
  public async SelectRecord(strTopicObjectiveId: string) {
    ...
    try {
      const objTopicObjectiveEN = await vTopicObjective_GetObjByTopicObjectiveIdAsync(
        strTopicObjectiveId,
      );
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vTopicObjectiveCache();
  }
  ...
  public async CombinevTopicObjectiveCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombineTopicObjectiveCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinevTopicObjectiveConditionObj(): Promise<clsvTopicObjectiveEN> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombineTopicObjectiveConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvTopicObjectiveCond.whereCond = strWhereCond;
    return objvTopicObjectiveCond;
  }
  ...
  public async CombinevTopicObjectiveConditionObj4ExportExcel(): Promise<clsvTopicObjectiveEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvTopicObjectiveCond = new clsvTopicObjectiveENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.objectiveName_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombineTopicObjectiveConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async BindTab_vTopicObjective(
    ...
    await BindTab_V(
      ...
      arrDataColumn,
      clsvTopicObjectiveEN.con_TopicObjectiveId,
      this,
    );
  ...
  public async ExtendFldFuncMap(
    arrvTopicObjectiveExObjLst: Array<clsvTopicObjectiveENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
  ...
  public async BindGv_vTopicObjectiveCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vTopicObjectiveCache.name;
    if (TopicObjectiveCRUD.sortvTopicObjectiveBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvTopicObjectiveBy)为空,请检查!(In BindGv_vTopicObjectiveCache)',
      );
    ...
    const objvTopicObjectiveCond = await this.CombinevTopicObjectiveConditionObj();
    const strWhereCond = JSON.stringify(objvTopicObjectiveCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvTopicObjectiveExObjLst: Array<clsvTopicObjectiveENEx> = [];
    try {
      this.recCount = await vTopicObjective_GetRecCountByCondCache(
        objvTopicObjectiveCond,
        TopicObjectiveCRUD.IdCurrEduClsCache,
      ...
      if (TopicObjectiveCRUD.sortFunStatic != undefined) {
        strSortFun = TopicObjectiveCRUD.sortFunStatic(TopicObjectiveCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvTopicObjectiveExObjLst = await vTopicObjectiveEx_GetObjExLstByPagerCache(
        objPagerPara,
        TopicObjectiveCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_vTopicObjective(divList, arrvTopicObjectiveExObjLst);
      //console.log("完成BindGv_vTopicObjectiveCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvTopicObjectiveEN, b: clsvTopicObjectiveEN): number {
  ...
  public async CopyRecord(arrTopicObjectiveId: Array<string>) {
    ...
    try {
      const arrTopicObjectiveObjLst = await TopicObjective_GetObjLstByTopicObjectiveIdLstAsync(
        arrTopicObjectiveId,
      );
  ...
  public async DelMultiRecord(arrTopicObjectiveId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strTopicObjectiveId of arrTopicObjectiveId) {
          TopicObjective_ReFreshCache(TopicObjectiveCRUD.IdCurrEduClsCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
