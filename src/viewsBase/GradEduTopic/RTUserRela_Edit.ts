...
import {
  RTUserRela_CheckPropertyNew,
  RTUserRela_AddNewRecordAsync,
  RTUserRela_ReFreshCache,
  RTUserRela_GetUniCondStr,
  RTUserRela_IsExistRecordAsync,
  RTUserRela_GetUniCondStr4Update,
  RTUserRela_IsExistAsync,
  RTUserRela_GetObjBymIdAsync,
  RTUserRela_CheckProperty4Update,
...
export abstract class RTUserRela_Edit {
  ...
  public static objPageEdit: RTUserRela_Edit;
  public static objPageEdit2: RTUserRela_Edit;
  public static objPageEdit3: RTUserRela_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList) {
    ...
    if (RTUserRela_Edit.SetPageEdit(this, 1) == true) return;
    if (RTUserRela_Edit.SetPageEdit(this, 2) == true) return;
    if (RTUserRela_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (RTUserRela_Edit.objPageEdit == null) {
          RTUserRela_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = RTUserRela_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (RTUserRela_Edit.objPageEdit2 == null) {
          RTUserRela_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = RTUserRela_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            RTUserRela_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (RTUserRela_Edit.objPageEdit3 == null) {
          RTUserRela_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = RTUserRela_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            RTUserRela_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (RTUserRela_Edit.objPageEdit != null) {
      const strClassNameOld = RTUserRela_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return RTUserRela_Edit.objPageEdit;
    }
    if (RTUserRela_Edit.objPageEdit2 != null) {
      const strClassNameOld = RTUserRela_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return RTUserRela_Edit.objPageEdit2;
    }
    if (RTUserRela_Edit.objPageEdit3 != null) {
      const strClassNameOld = RTUserRela_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return RTUserRela_Edit.objPageEdit3;
    }
    return null;
  }
  ...
  public HideDialog_RTUserRela() {
    if (RTUserRela_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      RTUserRela_Edit.EditObj.hideDialog();
    }
  }
   ...
   **/
  public async ShowDialog_RTUserRela(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_RTUserRela.name;
    if (RTUserRela_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (RTUserRela_Edit.EditObj == null) {
        const strMsg = Format(
      ...
      await RTUserRela_Edit.EditObj.showDialog();
    }
    this.divEdit = RTUserRela_Edit.EditObj.$refs.refDivEdit;
    if (this.divEdit == null) {
      if (RTUserRela_Edit.times4TestShowDialog < 2) {
        RTUserRela_Edit.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_RTUserRela(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      RTUserRela_Edit.times4TestShowDialog = 0;
    }
    if (strOp === 'Add' || strOp === 'AddWithMaxId') {
      this.btnSubmitRTUserRela = '确认添加';
      this.btnCancelRTUserRela = '取消添加';
    } else if (strOp === 'Update') {
      this.btnSubmitRTUserRela = '确认修改';
      this.btnCancelRTUserRela = '取消修改';
    }
    return true;
  }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (RTUserRela_Edit.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (RTUserRela_Edit.EditObj.dialogVisible == false) {
        const strMsg = Format(
  ...
  public async btnAddNewRecord_Click() {
    ...
    try {
      this.opType = 'Add';
      const bolIsSuccess = await this.ShowDialog_RTUserRela(this.opType);
  ...
  public async BindDdl4EditRegionInDiv() {
    const IdCurrEduClsCache = RTUserRela_Edit.IdCurrEduClsCache; //缓存分类变量;//在switch中未找到相关类型: tsCache(in AGC.PureClassEx.FuncParaType:GetTsTypeStr)
  ...
  public async btnSubmit_Click() {
    ...
    const strCommandText: string = this.btnSubmitRTUserRela;
    try {
      ...
      switch (strCommandText) {
        ...
        case '确认添加':
          ...
          if (this.opType == 'AddWithMaxId') {
            ...
            //{
            //RTUserRela_Edit.EditObj.hideDialog();
            //this.iShowList.BindGvCache(clsRTUserRelaEN._CurrTabName, "");
            //}
          } else {
            ...
            if (returnBool == true) {
              ...
              this.iShowList.BindGvCache(clsRTUserRelaEN._CurrTabName, this.keyId);
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecord() {
    ...
    this.Clear();
    //wucRTUserRelaB1.mId = RTUserRelaGetMaxStrId_S();
  }
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.mId = await RTUserRela_GetMaxStrIdAsync();
  }
  ...
  public async PutDataToRTUserRelaClass(pobjRTUserRelaEN: clsRTUserRelaEN) {
    ...
    pobjRTUserRelaEN.SetUpdUser(userStore.getUserId); // 修改人
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objRTUserRelaEN);
      ...
      if (returnBool == true) {
        RTUserRela_ReFreshCache(RTUserRela_Edit.UserIdCache);
   ...
   **/
  public async CheckUniCond4Add(objRTUserRelaEN: clsRTUserRelaEN): Promise<boolean> {
    const strUniquenessCondition = RTUserRela_GetUniCondStr(objRTUserRelaEN);
   ...
   **/
  public async CheckUniCond4Update(objRTUserRelaEN: clsRTUserRelaEN): Promise<boolean> {
    const strUniquenessCondition = RTUserRela_GetUniCondStr4Update(objRTUserRelaEN);
    const bolIsExistCondition = await RTUserRela_IsExistRecordAsync(strUniquenessCondition);
    if (bolIsExistCondition == true) {
      const strMsg = Format(
        '不能满足唯一性条件。满足条件：{0}的记录已经存在!',
  ...
  public async ShowData(lngmId: number) {
    ...
    try {
      const objRTUserRelaENConst = await RTUserRela_GetObjBymIdAsync(lngmId);
      if (objRTUserRelaENConst == null) {
        const strMsg = Format(
      ...
      objRTUserRelaEN = objRTUserRelaENConst;
    } catch (e) {
      const strMsg = Format(
   ...
   **/
  public async GetDataFromRTUserRelaClass(pobjRTUserRelaEN: clsRTUserRelaEN) {
  ...
  public async UpdateRecord(lngmId: number): Promise<boolean> {
    ...
    try {
      ...
      await this.GetDataFromRTUserRelaClass(objRTUserRelaEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objRTUserRelaEN);
      if (bolIsExistCond == false) {
        return false;
      }
   ...
   **/
  public set btnCancelRTUserRela(value: string) {
    RTUserRela_Edit.EditObj.SetButtonText('btnCancelRTUserRela', value);
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitRTUserRela(): string {
    const strValue = RTUserRela_Edit.EditObj.GetButtonText('btnSubmitRTUserRela');
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitRTUserRela(value: string) {
    RTUserRela_Edit.EditObj.SetButtonText('btnSubmitRTUserRela', value);
  }
  /**
   * 备注 (Used In Clear())
   **/
