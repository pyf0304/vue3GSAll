...
import {
  ...
  TopicObjective_CheckPropertyNew,
  TopicObjective_AddNewRecordWithMaxIdAsync,
  TopicObjective_ReFreshCache,
  TopicObjective_GetUniCondStr,
  TopicObjective_IsExistRecordAsync,
  TopicObjective_GetUniCondStr4Update,
  TopicObjective_IsExistAsync,
  TopicObjective_GetObjByTopicObjectiveIdAsync,
  TopicObjective_CheckProperty4Update,
...
export abstract class TopicObjective_Edit {
  ...
  public static objPageEdit: TopicObjective_Edit;
  public static objPageEdit2: TopicObjective_Edit;
  public static objPageEdit3: TopicObjective_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList) {
    ...
    if (TopicObjective_Edit.SetPageEdit(this, 1) == true) return;
    if (TopicObjective_Edit.SetPageEdit(this, 2) == true) return;
    if (TopicObjective_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (TopicObjective_Edit.objPageEdit == null) {
          TopicObjective_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = TopicObjective_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (TopicObjective_Edit.objPageEdit2 == null) {
          TopicObjective_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = TopicObjective_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            TopicObjective_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (TopicObjective_Edit.objPageEdit3 == null) {
          TopicObjective_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = TopicObjective_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            TopicObjective_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (TopicObjective_Edit.objPageEdit != null) {
      const strClassNameOld = TopicObjective_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return TopicObjective_Edit.objPageEdit;
    }
    if (TopicObjective_Edit.objPageEdit2 != null) {
      const strClassNameOld = TopicObjective_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return TopicObjective_Edit.objPageEdit2;
    }
    if (TopicObjective_Edit.objPageEdit3 != null) {
      const strClassNameOld = TopicObjective_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return TopicObjective_Edit.objPageEdit3;
    }
    return null;
  }
  ...
  public HideDialog_TopicObjective() {
    if (TopicObjective_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      TopicObjective_Edit.EditObj.hideDialog();
    }
  }
   ...
   **/
  public async ShowDialog_TopicObjective(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_TopicObjective.name;
    if (TopicObjective_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (TopicObjective_Edit.EditObj == null) {
        const strMsg = Format(
      ...
      await TopicObjective_Edit.EditObj.showDialog();
    }
    this.divEdit = TopicObjective_Edit.EditObj.$refs.refDivEdit;
    if (this.divEdit == null) {
      if (TopicObjective_Edit.times4TestShowDialog < 2) {
        TopicObjective_Edit.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_TopicObjective(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      TopicObjective_Edit.times4TestShowDialog = 0;
    }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (TopicObjective_Edit.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (TopicObjective_Edit.EditObj.dialogVisible == false) {
        const strMsg = Format(
  ...
  public async btnAddNewRecord_Click() {
    ...
    try {
      this.opType = 'Add';
      const bolIsSuccess = await this.ShowDialog_TopicObjective(this.opType);
        ...
        case '确认添加':
          ...
          if (this.opType == 'AddWithMaxId') {
            ...
            if (IsNullOrEmpty(returnKeyId) == false) {
              ...
              this.iShowList.BindGvCache(clsTopicObjectiveEN._CurrTabName, returnKeyId);
            }
          } else {
            ...
            if (returnBool == true) {
              ...
              this.iShowList.BindGvCache(clsTopicObjectiveEN._CurrTabName, this.keyId);
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecord() {
    ...
    this.Clear();
    //wucTopicObjectiveB1.topicObjectiveId = TopicObjectiveGetMaxStrId_S();
    try {
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.topicObjectiveId = await TopicObjective_GetMaxStrIdAsync();
    try {
      const returnString = await TopicObjective_GetMaxStrIdAsync();
      if (returnString == '') {
        const strInfo = Format('获取表TopicObjective的最大关键字为空,不成功,请检查!');
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objTopicObjectiveEN);
      ...
      if (returnBool == true) {
        TopicObjective_ReFreshCache(TopicObjective_Edit.IdCurrEduClsCache);
   ...
   **/
  public async CheckUniCond4Add(objTopicObjectiveEN: clsTopicObjectiveEN): Promise<boolean> {
    const strUniquenessCondition = TopicObjective_GetUniCondStr(objTopicObjectiveEN);
   ...
   **/
  public async CheckUniCond4Update(objTopicObjectiveEN: clsTopicObjectiveEN): Promise<boolean> {
    const strUniquenessCondition = TopicObjective_GetUniCondStr4Update(objTopicObjectiveEN);
    const bolIsExistCondition = await TopicObjective_IsExistRecordAsync(strUniquenessCondition);
    if (bolIsExistCondition == true) {
      const strMsg = Format(
        '不能满足唯一性条件。满足条件：{0}的记录已经存在!',
  ...
  public async ShowData(strTopicObjectiveId: string) {
    ...
    try {
      const objTopicObjectiveENConst = await TopicObjective_GetObjByTopicObjectiveIdAsync(
        strTopicObjectiveId,
      );
      if (objTopicObjectiveENConst == null) {
        const strMsg = Format(
      ...
      objTopicObjectiveEN = objTopicObjectiveENConst;
    } catch (e) {
      const strMsg = Format(
  ...
  public async UpdateRecord(strTopicObjectiveId: string): Promise<boolean> {
    ...
    try {
      ...
      await this.GetDataFromTopicObjectiveClass(objTopicObjectiveEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objTopicObjectiveEN);
      if (bolIsExistCond == false) {
        return false;
      }
   ...
   **/
  public set btnCancelTopicObjective(value: string) {
    TopicObjective_Edit.EditObj.SetButtonText('btnCancelTopicObjective', value);
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitTopicObjective(): string {
    const strValue = TopicObjective_Edit.EditObj.GetButtonText('btnSubmitTopicObjective');
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitTopicObjective(value: string) {
    TopicObjective_Edit.EditObj.SetButtonText('btnSubmitTopicObjective', value);
  }
  /**
   * 结论 (Used In Clear())
   **/
  ...
  public set objectiveContent(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtObjectiveContent', value);
  }
  /**
   * 客观内容 (Used In PutDataToClass())
   **/
  public get objectiveContent(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtObjectiveContent');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set objectiveName(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtObjectiveName', value);
  }
  /**
   * 客观名称 (Used In PutDataToClass())
   **/
  public get objectiveName(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtObjectiveName');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set objectiveType(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtObjectiveType', value);
  }
  /**
   * 客观类型 (Used In PutDataToClass())
   **/
  public get objectiveType(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtObjectiveType');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set sourceId(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtSourceId', value);
  }
  /**
   * 来源Id (Used In PutDataToClass())
   **/
  public get sourceId(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtSourceId');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
