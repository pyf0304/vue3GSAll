...
import {
  ...
  vgs_ResearchPlan_GetSubObjLstCache,
  vgs_ResearchPlan_GetObjByPlanIdAsync,
...
import {
  ...
  gs_ResearchPlan_GetObjLstByPlanIdLstAsync,
...
import {
  vgs_ResearchPlanEx_FuncMapByFldName,
  vgs_ResearchPlanEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTopic/clsvgs_ResearchPlanExWApi';
...
export abstract class gs_ResearchPlanCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (gs_ResearchPlanCRUD.sortvgs_ResearchPlanBy == '')
        gs_ResearchPlanCRUD.sortvgs_ResearchPlanBy = 'topicName Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vgs_ResearchPlanCache(this.divList);
    } catch (e) {
  ...
  public CombineData(
    ...
    const strFileName = Format('主题研究计划({0})导出.xlsx', clsvgs_ResearchPlanEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vgs_ResearchPlanCache() {
    const strThisFuncName = this.ExportExcel_vgs_ResearchPlanCache.name;
    if (gs_ResearchPlanCRUD.sortvgs_ResearchPlanBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvgs_ResearchPlanBy)为空,请检查!(In BindGv_vgs_ResearchPlanCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvgs_ResearchPlanCond = await this.Combinevgs_ResearchPlanConditionObj4ExportExcel();
    objvgs_ResearchPlanCond.SetCondFldValue(
      clsvgs_ResearchPlanEN.con_IdCurrEduCls,
      gs_ResearchPlanCRUD.IdCurrEduClsCache,
      '=',
    );
    let arrvgs_ResearchPlanObjLst: Array<clsvgs_ResearchPlanEN> = [];
    try {
      this.recCount = await vgs_ResearchPlan_GetRecCountByCondCache(
        objvgs_ResearchPlanCond,
        gs_ResearchPlanCRUD.IdCurrEduClsCache,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvgs_ResearchPlanCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvgs_ResearchPlanObjLst = await vgs_ResearchPlan_GetSubObjLstCache(
        objvgs_ResearchPlanCond,
        gs_ResearchPlanCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    if (arrvgs_ResearchPlanObjLst.length == 0) {
      const strKey = Format(
        '{0}_{1}',
        clsgs_ResearchPlanEN._CurrTabName,
        gs_ResearchPlanCRUD.IdCurrEduClsCache,
      );
    ...
    try {
      ...
      arrvgs_ResearchPlanObjLst = arrvgs_ResearchPlanObjLst.sort(this.SortFunExportExcel);
  ...
  public async SetDdl_StatusIdInDiv() {
    await gs_TopicTaskStatus_BindDdl_StatusIdInDivCache(this.divQuery, 'ddlStatusId_q'); //
  }
  ...
  public async DelRecord(strPlanId: string) {
    ...
    try {
      ...
      if (returnInt > 0) {
        gs_ResearchPlan_ReFreshCache(gs_ResearchPlanCRUD.IdCurrEduClsCache);
  ...
  public async SelectRecord(strPlanId: string) {
    ...
    try {
      const objgs_ResearchPlanEN = await vgs_ResearchPlan_GetObjByPlanIdAsync(strPlanId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vgs_ResearchPlanCache();
  }
  ...
  public async Combinevgs_ResearchPlanCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(Combinegs_ResearchPlanCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async Combinevgs_ResearchPlanConditionObj(): Promise<clsvgs_ResearchPlanEN> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(Combinegs_ResearchPlanConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvgs_ResearchPlanCond.whereCond = strWhereCond;
    return objvgs_ResearchPlanCond;
  }
  ...
  public async Combinevgs_ResearchPlanConditionObj4ExportExcel(): Promise<clsvgs_ResearchPlanEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvgs_ResearchPlanCond = new clsvgs_ResearchPlanENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.topicName_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(Combinegs_ResearchPlanConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async ExtendFldFuncMap(
    arrvgs_ResearchPlanExObjLst: Array<clsvgs_ResearchPlanENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
  ...
  public async BindGv_vgs_ResearchPlanCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vgs_ResearchPlanCache.name;
    if (gs_ResearchPlanCRUD.sortvgs_ResearchPlanBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvgs_ResearchPlanBy)为空,请检查!(In BindGv_vgs_ResearchPlanCache)',
      );
    ...
    const objvgs_ResearchPlanCond = await this.Combinevgs_ResearchPlanConditionObj();
    const strWhereCond = JSON.stringify(objvgs_ResearchPlanCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvgs_ResearchPlanExObjLst: Array<clsvgs_ResearchPlanENEx> = [];
    try {
      this.recCount = await vgs_ResearchPlan_GetRecCountByCondCache(
        objvgs_ResearchPlanCond,
        gs_ResearchPlanCRUD.IdCurrEduClsCache,
      ...
      if (gs_ResearchPlanCRUD.sortFunStatic != undefined) {
        strSortFun = gs_ResearchPlanCRUD.sortFunStatic(gs_ResearchPlanCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvgs_ResearchPlanExObjLst = await vgs_ResearchPlanEx_GetObjExLstByPagerCache(
        objPagerPara,
        gs_ResearchPlanCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_vgs_ResearchPlan(divList, arrvgs_ResearchPlanExObjLst);
      //console.log("完成BindGv_vgs_ResearchPlanCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvgs_ResearchPlanEN, b: clsvgs_ResearchPlanEN): number {
  ...
  public async DelMultiRecord(arrPlanId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strPlanId of arrPlanId) {
          gs_ResearchPlan_ReFreshCache(gs_ResearchPlanCRUD.IdCurrEduClsCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
  ...
  public get statusId_q(): string {
    const strValue = GetSelectValueInDivObj(this.divQuery, 'ddlStatusId_q');
  ...
  public set statusId_q(value: string) {
    SetSelectValueByIdInDivObj(this.divQuery, 'ddlStatusId_q', value);
  }
  /**
