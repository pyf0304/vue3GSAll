...
import {
  SysScoreWeight_CheckPropertyNew,
  SysScoreWeight_IsExistAsync,
  SysScoreWeight_AddNewRecordAsync,
  SysScoreWeight_ReFreshCache,
  SysScoreWeight_AddNewRecordWithMaxIdAsync,
  SysScoreWeight_GetObjByScoreWeightIdAsync,
  SysScoreWeight_CheckProperty4Update,
  SysScoreWeight_UpdateRecordAsync,
...
export abstract class SysScoreWeight_Edit {
  ...
  public static objPageEdit: SysScoreWeight_Edit;
  public static objPageEdit2: SysScoreWeight_Edit;
  public static objPageEdit3: SysScoreWeight_Edit;
  ...
  constructor(strClassName: string, objShowList: IShowList) {
    ...
    if (SysScoreWeight_Edit.SetPageEdit(this, 1) == true) return;
    if (SysScoreWeight_Edit.SetPageEdit(this, 2) == true) return;
    if (SysScoreWeight_Edit.SetPageEdit(this, 3) == true) return;
  }
  public static SetPageEdit(objDataLst: any, intIndex: number): boolean {
    const strNewClassName = objDataLst.className;
    switch (intIndex) {
      case 1:
        if (SysScoreWeight_Edit.objPageEdit == null) {
          SysScoreWeight_Edit.objPageEdit = objDataLst;
          return true;
        } else {
          const strClassNameOld = SysScoreWeight_Edit.objPageEdit.className;
        ...
        break;
      case 2:
        if (SysScoreWeight_Edit.objPageEdit2 == null) {
          SysScoreWeight_Edit.objPageEdit2 = objDataLst;
          return true;
        } else {
          const strClassNameOld = SysScoreWeight_Edit.objPageEdit2.className;
          if (strClassNameOld == strNewClassName) {
            SysScoreWeight_Edit.objPageEdit2 = objDataLst;
            return true;
          } else return false;
        ...
        break;
      case 3:
        if (SysScoreWeight_Edit.objPageEdit3 == null) {
          SysScoreWeight_Edit.objPageEdit3 = objDataLst;
          return true;
        } else {
          const strClassNameOld = SysScoreWeight_Edit.objPageEdit3.className;
          if (strClassNameOld == strNewClassName) {
            SysScoreWeight_Edit.objPageEdit3 = objDataLst;
            return true;
          } else return false;
  ...
  public static GetPageEditObj(strClassName: string): any {
    if (SysScoreWeight_Edit.objPageEdit != null) {
      const strClassNameOld = SysScoreWeight_Edit.objPageEdit.className;
      if (strClassNameOld == strClassName) return SysScoreWeight_Edit.objPageEdit;
    }
    if (SysScoreWeight_Edit.objPageEdit2 != null) {
      const strClassNameOld = SysScoreWeight_Edit.objPageEdit2.className;
      if (strClassNameOld == strClassName) return SysScoreWeight_Edit.objPageEdit2;
    }
    if (SysScoreWeight_Edit.objPageEdit3 != null) {
      const strClassNameOld = SysScoreWeight_Edit.objPageEdit3.className;
      if (strClassNameOld == strClassName) return SysScoreWeight_Edit.objPageEdit3;
    }
    return null;
  }
  ...
  public HideDialog_SysScoreWeight() {
    if (SysScoreWeight_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      SysScoreWeight_Edit.EditObj.hideDialog();
    }
  }
   ...
   **/
  public async ShowDialog_SysScoreWeight(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_SysScoreWeight.name;
    if (SysScoreWeight_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (SysScoreWeight_Edit.EditObj == null) {
        const strMsg = Format(
      ...
      await SysScoreWeight_Edit.EditObj.showDialog();
    }
    this.divEdit = SysScoreWeight_Edit.EditObj.$refs.refDivEdit;
    if (this.divEdit == null) {
      if (SysScoreWeight_Edit.times4TestShowDialog < 2) {
        SysScoreWeight_Edit.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_SysScoreWeight(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      SysScoreWeight_Edit.times4TestShowDialog = 0;
    }
    if (strOp === 'Add' || strOp === 'AddWithMaxId') {
      this.btnSubmitSysScoreWeight = '确认添加';
      this.btnCancelSysScoreWeight = '取消添加';
    } else if (strOp === 'Update') {
      this.btnSubmitSysScoreWeight = '确认修改';
      this.btnCancelSysScoreWeight = '取消修改';
    }
    return true;
  }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    if (SysScoreWeight_Edit.strPageDispModeId != enumPageDispMode.PopupBox_01) {
      ...
      if (SysScoreWeight_Edit.EditObj.dialogVisible == false) {
        const strMsg = Format(
  ...
  public async btnAddNewRecord_Click() {
    ...
    try {
      this.opType = 'Add';
      const bolIsSuccess = await this.ShowDialog_SysScoreWeight(this.opType);
  ...
  public async BindDdl4EditRegionInDiv() {
    const CourseIdStatic = SysScoreWeight_Edit.CourseIdStatic; //静态变量;//静态变量
  ...
  public async btnSubmit_Click() {
    ...
    const strCommandText: string = this.btnSubmitSysScoreWeight;
    try {
        ...
        case '确认添加':
          ...
          if (this.opType == 'AddWithMaxId') {
            ...
            if (IsNullOrEmpty(returnKeyId) == false) {
              ...
              this.iShowList.BindGvCache(clsSysScoreWeightEN._CurrTabName, returnKeyId);
            }
          } else {
            ...
            if (returnBool == true) {
              ...
              this.iShowList.BindGvCache(clsSysScoreWeightEN._CurrTabName, this.keyId);
            }
          }
          break;
        case '确认修改':
  ...
  public async AddNewRecord() {
    ...
    this.Clear();
    //wucSysScoreWeightB1.scoreWeightId = SysScoreWeightGetMaxStrId_S();
  }
  ...
  public async AddNewRecordWithMaxId() {
    ...
    //this.scoreWeightId = await SysScoreWeight_GetMaxStrIdAsync();
  }
  ...
  public async PutDataToSysScoreWeightClass(pobjSysScoreWeightEN: clsSysScoreWeightEN) {
    ...
    pobjSysScoreWeightEN.SetIdCurrEduCls(this.idCurrEduCls); // 当前教学班流水号
  ...
  public async ShowData(strScoreWeightId: string) {
    ...
    try {
      const objSysScoreWeightENConst = await SysScoreWeight_GetObjByScoreWeightIdAsync(
        strScoreWeightId,
      );
      if (objSysScoreWeightENConst == null) {
        const strMsg = Format(
      ...
      objSysScoreWeightEN = objSysScoreWeightENConst;
    } catch (e) {
      const strMsg = Format(
   ...
   **/
  public async GetDataFromSysScoreWeightClass(pobjSysScoreWeightEN: clsSysScoreWeightEN) {
    ...
    this.idCurrEduCls = pobjSysScoreWeightEN.idCurrEduCls; // 当前教学班流水号
  ...
  public async UpdateRecord(strScoreWeightId: string): Promise<boolean> {
    ...
    try {
      ...
      await this.GetDataFromSysScoreWeightClass(objSysScoreWeightEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    const objSysScoreWeightEN = new clsSysScoreWeightEN();
    objSysScoreWeightEN.SetScoreWeightId(this.keyId);
    await this.PutDataToSysScoreWeightClass(objSysScoreWeightEN);
   ...
   **/
  public set btnCancelSysScoreWeight(value: string) {
    SysScoreWeight_Edit.EditObj.SetButtonText('btnCancelSysScoreWeight', value);
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitSysScoreWeight(): string {
    const strValue = SysScoreWeight_Edit.EditObj.GetButtonText('btnSubmitSysScoreWeight');
    return strValue;
  }
  /**
   * 设置确定按钮的标题(Used In AddNewRecord())
   **/
  public set btnSubmitSysScoreWeight(value: string) {
    SysScoreWeight_Edit.EditObj.SetButtonText('btnSubmitSysScoreWeight', value);
  }
  /**
  ...
  public set scoreWeightId(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtScoreWeightId', value);
  }
  /**
   * 分数权重Id (Used In PutDataToClass())
   **/
  public get scoreWeightId(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtScoreWeightId');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
  /**
  ...
  public set scoreWeightValue(value: string) {
    SetInputValueInDivObj(this.divEdit, 'txtScoreWeightValue', value);
  }
  /**
   * 分数权重值 (Used In PutDataToClass())
   **/
  public get scoreWeightValue(): string {
    const strValue = GetInputValueInDivObj(this.divEdit, 'txtScoreWeightValue');
    if (strValue == undefined) return '';
    else return strValue.toString();
  }
}
