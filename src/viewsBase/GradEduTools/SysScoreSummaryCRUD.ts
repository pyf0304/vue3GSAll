...
//import $ from "jquery";
import {
  vSysScoreSummary_GetRecCountByCondCache,
  vSysScoreSummary_GetSubObjLstCache,
  vSysScoreSummary_GetObjBymIdAsync,
...
import {
  SysScoreSummary_DelRecordAsync,
  SysScoreSummary_ReFreshCache,
  SysScoreSummary_GetObjLstBymIdLstAsync,
  SysScoreSummary_AddNewRecordAsync,
  SysScoreSummary_DelSysScoreSummarysAsync,
...
import {
  vSysScoreSummaryEx_FuncMapByFldName,
  vSysScoreSummaryEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTools/clsvSysScoreSummaryExWApi';
...
export abstract class SysScoreSummaryCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (SysScoreSummaryCRUD.sortvSysScoreSummaryBy == '')
        SysScoreSummaryCRUD.sortvSysScoreSummaryBy = 'schoolYear Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vSysScoreSummaryCache(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrvSysScoreSummaryObjLst: Array<clsvSysScoreSummaryEN>,
    ...
    const strFileName = Format('系统分数汇总表({0})导出.xlsx', clsvSysScoreSummaryEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vSysScoreSummaryCache() {
    const strThisFuncName = this.ExportExcel_vSysScoreSummaryCache.name;
    if (SysScoreSummaryCRUD.sortvSysScoreSummaryBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvSysScoreSummaryBy)为空,请检查!(In BindGv_vSysScoreSummaryCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvSysScoreSummaryCond = await this.CombinevSysScoreSummaryConditionObj4ExportExcel();
    objvSysScoreSummaryCond.SetCondFldValue(
      clsvSysScoreSummaryEN.con_IdCurrEduCls,
      SysScoreSummaryCRUD.IdCurrEduClsCache,
      '=',
    );
    let arrvSysScoreSummaryObjLst: Array<clsvSysScoreSummaryEN> = [];
    try {
      this.recCount = await vSysScoreSummary_GetRecCountByCondCache(
        objvSysScoreSummaryCond,
        SysScoreSummaryCRUD.IdCurrEduClsCache,
      );
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objvSysScoreSummaryCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      arrvSysScoreSummaryObjLst = await vSysScoreSummary_GetSubObjLstCache(
        objvSysScoreSummaryCond,
        SysScoreSummaryCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    if (arrvSysScoreSummaryObjLst.length == 0) {
      const strKey = Format(
        '{0}_{1}',
        clsSysScoreSummaryEN._CurrTabName,
        SysScoreSummaryCRUD.IdCurrEduClsCache,
      );
    ...
    try {
      ...
      arrvSysScoreSummaryObjLst = arrvSysScoreSummaryObjLst.sort(this.SortFunExportExcel);
  ...
  public async SetDdl_ScoreTypeIdInDiv() {
    await SysScoreType_BindDdl_ScoreTypeIdInDivCache(this.divQuery, 'ddlScoreTypeId_q'); //
  }
  ...
  public async DelRecord(lngmId: number) {
    ...
    try {
      ...
      if (returnInt > 0) {
        SysScoreSummary_ReFreshCache(SysScoreSummaryCRUD.IdCurrEduClsCache);
  ...
  public async SelectRecord(lngmId: number) {
    ...
    try {
      const objSysScoreSummaryEN = await vSysScoreSummary_GetObjBymIdAsync(lngmId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vSysScoreSummaryCache();
  }
  ...
  public async CombinevSysScoreSummaryCondition(): Promise<string> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombineSysScoreSummaryCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinevSysScoreSummaryConditionObj(): Promise<clsvSysScoreSummaryEN> {
    ...
    try {
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombineSysScoreSummaryConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvSysScoreSummaryCond.whereCond = strWhereCond;
    return objvSysScoreSummaryCond;
  }
  ...
  public async CombinevSysScoreSummaryConditionObj4ExportExcel(): Promise<clsvSysScoreSummaryEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvSysScoreSummaryCond = new clsvSysScoreSummaryENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.userName_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombineSysScoreSummaryConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
   ...
   **/
  public async BindTab_vSysScoreSummary(
  ...
  public async ExtendFldFuncMap(
    arrvSysScoreSummaryExObjLst: Array<clsvSysScoreSummaryENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vSysScoreSummaryCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vSysScoreSummaryCache.name;
    if (SysScoreSummaryCRUD.sortvSysScoreSummaryBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvSysScoreSummaryBy)为空,请检查!(In BindGv_vSysScoreSummaryCache)',
      );
    ...
    const objvSysScoreSummaryCond = await this.CombinevSysScoreSummaryConditionObj();
    const strWhereCond = JSON.stringify(objvSysScoreSummaryCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvSysScoreSummaryExObjLst: Array<clsvSysScoreSummaryENEx> = [];
    try {
      this.recCount = await vSysScoreSummary_GetRecCountByCondCache(
        objvSysScoreSummaryCond,
        SysScoreSummaryCRUD.IdCurrEduClsCache,
      ...
      if (SysScoreSummaryCRUD.sortFunStatic != undefined) {
        strSortFun = SysScoreSummaryCRUD.sortFunStatic(SysScoreSummaryCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvSysScoreSummaryExObjLst = await vSysScoreSummaryEx_GetObjExLstByPagerCache(
        objPagerPara,
        SysScoreSummaryCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_vSysScoreSummary(divList, arrvSysScoreSummaryExObjLst);
      //console.log("完成BindGv_vSysScoreSummaryCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvSysScoreSummaryEN, b: clsvSysScoreSummaryEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      SysScoreSummaryCRUD.ascOrDesc4SortFun,
    ...
    SysScoreSummaryCRUD.sortFunStatic = sortFun;
    await this.BindGv_vSysScoreSummaryCache(this.divList);
  }
  ...
  public get scoreTypeId_q(): string {
    const strValue = GetSelectValueInDivObj(this.divQuery, 'ddlScoreTypeId_q');
    if (strValue == undefined) return '';
    else if (strValue == '0') return '';
    else return strValue;
  }
  ...
  public set scoreTypeId_q(value: string) {
    SetSelectValueByIdInDivObj(this.divQuery, 'ddlScoreTypeId_q', value);
  }
  /**
