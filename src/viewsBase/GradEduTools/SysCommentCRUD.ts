...
//import $ from "jquery";
import {
  vSysComment_GetRecCountByCondCache,
  vSysComment_GetSubObjLstCache,
  vSysComment_GetObjByCommentIdAsync,
...
import {
  ...
  SysComment_GetObjLstByCommentIdLstAsync,
  SysComment_GetMaxStrIdAsync,
  SysComment_AddNewRecordAsync,
  SysComment_DelSysCommentsAsync,
} from '@/ts/L3ForWApi/GradEduTools/clsSysCommentWApi';
import { clsvSysCommentEN } from '@/ts/L0Entity/GradEduTools/clsvSysCommentEN';
import { clsvSysCommentENEx } from '@/ts/L0Entity/GradEduTools/clsvSysCommentENEx';
import {
  BindTab_V,
  arrSelectedKeys,
  confirmDel,
...
import {
  vSysCommentEx_FuncMapByFldName,
  vSysCommentEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiEx/GradEduTools/clsvSysCommentExWApi';
...
export abstract class SysCommentCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      this.SetEventFunc();
      if (SysCommentCRUD.sortvSysCommentBy == '') SysCommentCRUD.sortvSysCommentBy = 'comment Asc';
      //2、显示无条件的表内容在GridView中
      await this.BindGv_vSysCommentCache(this.divList);
    } catch (e) {
   ...
   **/
  public CombineData(
    arrvSysCommentObjLst: Array<clsvSysCommentEN>,
    ...
    const strFileName = Format('系统评论表({0})导出.xlsx', clsvSysCommentEN._CurrTabName);
   ...
   **/
  public async ExportExcel_vSysCommentCache() {
    const strThisFuncName = this.ExportExcel_vSysCommentCache.name;
    if (SysCommentCRUD.sortvSysCommentBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvSysCommentBy)为空,请检查!(In BindGv_vSysCommentCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    const objvSysCommentCond = await this.CombinevSysCommentConditionObj4ExportExcel();
    objvSysCommentCond.SetCondFldValue(
      clsvSysCommentEN.con_IdCurrEduCls,
      SysCommentCRUD.IdCurrEduClsCache,
      '=',
    );
    let arrvSysCommentObjLst: Array<clsvSysCommentEN> = [];
    try {
      this.recCount = await vSysComment_GetRecCountByCondCache(
        objvSysCommentCond,
        SysCommentCRUD.IdCurrEduClsCache,
      );
      if (this.recCount == 0) {
      ...
      arrvSysCommentObjLst = await vSysComment_GetSubObjLstCache(
        objvSysCommentCond,
        SysCommentCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      ...
      arrvSysCommentObjLst = arrvSysCommentObjLst.sort(this.SortFunExportExcel);
  ...
  public async DelRecord(strCommentId: string) {
    ...
    try {
      ...
      if (returnInt > 0) {
        SysComment_ReFreshCache(SysCommentCRUD.IdCurrEduClsCache);
  ...
  public async SelectRecord(strCommentId: string) {
    ...
    try {
      const objSysCommentEN = await vSysComment_GetObjByCommentIdAsync(strCommentId);
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_vSysCommentCache();
  }
    ...
    try {
      ...
      if (this.commentTypeName_q != '') {
        strWhereCond += Format(
          ...
          this.commentTypeName_q,
        );
      }
    } catch (objException) {
      const strMsg: string = Format(
        '(errid:WiTsCs0017)在组合查询条件(CombineSysCommentCondition)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
  ...
  public async CombinevSysCommentConditionObj(): Promise<clsvSysCommentEN> {
    ...
    try {
      if (this.userId_q != '') {
        ...
        objvSysCommentCond.SetCondFldValue(clsvSysCommentEN.con_UserId, this.userId_q, 'like');
      }
      if (this.commentTypeName_q != '') {
        strWhereCond += Format(
          " And {0} like '% {1}%'",
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0018)在组合查询条件对象(CombineSysCommentConditionObj)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
    objvSysCommentCond.whereCond = strWhereCond;
    return objvSysCommentCond;
  }
  ...
  public async CombinevSysCommentConditionObj4ExportExcel(): Promise<clsvSysCommentEN> {
    ...
    //例如 1 = 1 && UserName = '张三'
    let strWhereCond = ' 1 = 1 ';
    const objvSysCommentCond = new clsvSysCommentENEx();
    //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
    try {
      if (this.userId_q != '') {
        strWhereCond += Format(
      ...
      const strMsg: string = Format(
        '(errid:WiTsCs0019)在组合导出Excel条件对象(CombineSysCommentConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
   ...
   **/
  public async BindTab_vSysComment(
  ...
  public async ExtendFldFuncMap(
    arrvSysCommentExObjLst: Array<clsvSysCommentENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ) {
   ...
   **/
  public async BindGv_vSysCommentCache(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_vSysCommentCache.name;
    if (SysCommentCRUD.sortvSysCommentBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortvSysCommentBy)为空,请检查!(In BindGv_vSysCommentCache)',
      );
    ...
    const objvSysCommentCond = await this.CombinevSysCommentConditionObj();
    const strWhereCond = JSON.stringify(objvSysCommentCond);
    let intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrvSysCommentExObjLst: Array<clsvSysCommentENEx> = [];
    try {
      this.recCount = await vSysComment_GetRecCountByCondCache(
        objvSysCommentCond,
        SysCommentCRUD.IdCurrEduClsCache,
      ...
      if (SysCommentCRUD.sortFunStatic != undefined) {
        strSortFun = SysCommentCRUD.sortFunStatic(SysCommentCRUD.ascOrDesc4SortFun);
      }
      ...
      arrvSysCommentExObjLst = await vSysCommentEx_GetObjExLstByPagerCache(
        objPagerPara,
        SysCommentCRUD.IdCurrEduClsCache,
      );
    } catch (e) {
    ...
    try {
      await this.BindTab_vSysComment(divList, arrvSysCommentExObjLst);
      //console.log("完成BindGv_vSysCommentCache!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
   ...
   * @returns 返回两个对象比较的结果
   **/
  public SortFunExportExcel(a: clsvSysCommentEN, b: clsvSysCommentEN): number {
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      objAnchorElement,
      SysCommentCRUD.ascOrDesc4SortFun,
    ...
    SysCommentCRUD.sortFunStatic = sortFun;
    await this.BindGv_vSysCommentCache(this.divList);
  }
  ...
  public async DelMultiRecord(arrCommentId: Array<string>) {
    ...
    try {
      ...
      if (returnInt > 0) {
        for (const strCommentId of arrCommentId) {
          SysComment_ReFreshCache(SysCommentCRUD.IdCurrEduClsCache);
        }
        const strInfo = `删除${this.thisTabName}记录成功,共删除${returnInt}条记录!`;
        //显示信息框
   ...
   **/
  public ShowSysCommentObj(divContainer: HTMLDivElement, objSysComment: clsSysCommentEN) {
    if (divContainer == null) {
  ...
  public get commentTypeName_q(): string {
    const strValue = GetInputValueInDivObj(this.divQuery, 'txtCommentTypeName_q');
  ...
  public set commentTypeName_q(value: string) {
    SetInputValueInDivObj(this.divQuery, 'txtCommentTypeName_q', value);
  }
  /**
