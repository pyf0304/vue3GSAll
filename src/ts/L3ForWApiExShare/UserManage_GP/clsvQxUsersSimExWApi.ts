...
import { vQxUsersSim_GetObjLstAsync,vQxUsersSim_SortFunByKey } from "@/ts/L3ForWApi/UserManage_GP/clsvQxUsersSimWApi";
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, vQxUsersSimEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function vQxUsersSimEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsvQxUsersSimENEx>> {
...
const arrvQxUsersSimExObjLst = arrvQxUsersSimObjLst.map(vQxUsersSimEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsvQxUsersSimEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
arrvQxUsersSimSel = arrvQxUsersSimSel.sort(vQxUsersSimEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, vQxUsersSimEx_ConstructorName, strThisFuncName);
console.error(strMsg);
...
*/
export  function vQxUsersSimEx_FuncMapByFldName(strFldName: string, objvQxUsersSimEx: clsvQxUsersSimENEx)
{
