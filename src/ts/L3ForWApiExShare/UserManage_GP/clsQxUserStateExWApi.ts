...
import { QxUserState_GetObjLstCache,QxUserState_GetObjLstAsync,QxUserState_SortFunByKey } from "@/ts/L3ForWApi/UserManage/clsQxUserStateWApi";
import { stuPagerPara } from "@/ts/PubFun/stuPagerPara";
import { IsNullOrEmpty,Format } from "@/ts/PubFun/clsString";
import { clsQxUserStateEN } from "@/ts/L0Entity/UserManage/clsQxUserStateEN";
import { clsQxUserStateENEx } from "@/ts/L0Entity/UserManage/clsQxUserStateENEx";
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxUserStateEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function QxUserStateEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQxUserStateENEx>> {
...
const arrQxUserStateExObjLst = arrQxUserStateObjLst.map(QxUserStateEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsQxUserStateEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const objQxUserStateCond = new clsQxUserStateENEx();
...
arrQxUserStateSel = arrQxUserStateSel.sort(QxUserStateEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, qxUserStateEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function QxUserStateEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsQxUserStateENEx>> {
...
*/
export  function QxUserStateEx_FuncMapByFldName(strFldName: string, objQxUserStateEx: clsQxUserStateENEx)
{
