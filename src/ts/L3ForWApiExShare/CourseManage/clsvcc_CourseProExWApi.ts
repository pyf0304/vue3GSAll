...
import {
  vcc_CoursePro_GetObjLstAsync,
  vcc_CoursePro_SortFunByKey,
  vcc_CoursePro_FilterFunByKey,
} from '@/ts/L3ForWApi/CourseManage/clsvcc_CourseProWApi';
import { clsvcc_CourseProENEx } from '@/ts/L0Entity/CourseManage/clsvcc_CourseProENEx';
 ...
 **/
export function vcc_CourseProEx_CopyToEx(
  objvcc_CourseProENS: clsvcc_CourseProEN,
): clsvcc_CourseProENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vcc_CourseProEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vcc_CourseProEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvcc_CourseProENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvcc_CourseProExObjLst) {
      await vcc_CourseProEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvcc_CoursePro_Sel = arrvcc_CoursePro_Sel.sort(
        vcc_CourseProEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function vcc_CourseProEx_FuncMapByFldName(
  strFldName: string,
  objvcc_CourseProEx: clsvcc_CourseProENEx,
) {
  const strThisFuncName = vcc_CourseProEx_FuncMapByFldName.name;
  console.log(objvcc_CourseProEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
