...
import {
  ...
  TeacherInfoEx_GetIdTeacherByTeacherId,
} from '@/ts/L3ForWApiExShare/BaseInfo/clsTeacherInfoExWApi';
import { vStudentInfoEx_GetObjExLstByPagerAsync } from '@/ts/L3ForWApiExShare/UserManage_GP/clsvStudentInfoExWApi';
import { StudentInfoEx_GetIdStudentInfoByStuId } from '@/ts/L3ForWApiExShare/UserManage_GP/clsStudentInfoExWApi';
...
export async function CurrEduClsEx_Bind_EduClsTeaList(
  ...
  try {
    const strWhereCond = ` teacherId='${userStore.getUserId}' and ${clsvCurrEduClsTeacherEN.con_UserType} ='${userStore.userType}' order by updDate Desc`;
    ...
    // let strhtml = '';
    if (
      arrvCurrEduClsTeacherObjLst.find((x) => x.idCurrEduCls == clsPubLocalStorage.idCurrEduCls) ==
      null
    ) {
      const objvCurrEduClsTeacher = arrvCurrEduClsTeacherObjLst[0];

      SetSpanHtmlInDivObj(divLayout, strEduClsNameId, objvCurrEduClsTeacher.eduClsName);
      //存入session
      clsPubLocalStorage.idCurrEduCls = objvCurrEduClsTeacher.idCurrEduCls;
      clsPubLocalStorage.eduClsTypeId = objvCurrEduClsTeacher.eduClsTypeId;
      clsPubLocalStorage.eduClsName = objvCurrEduClsTeacher.eduClsName;
    } else {
      const strEduClsName = await currEduClsStore.getEduClsName(clsPubLocalStorage.idCurrEduCls);
      SetSpanHtmlInDivObj(divLayout, strEduClsNameId, strEduClsName);
    }
...
export async function CurrEduClsEx_Bind_EduClsStuList(
  ...
  try {
    ...
    if (
      arrvCurrEduClsStuObjLst.find((x) => x.idCurrEduCls == clsPubLocalStorage.idCurrEduCls) == null
    ) {
      const objvCurrEduClsStu = arrvCurrEduClsStuObjLst[0];
      SetSpanHtmlInDivObj(divLayout, strEduClsNameId, objvCurrEduClsStu.eduClsName);
      //存入session
