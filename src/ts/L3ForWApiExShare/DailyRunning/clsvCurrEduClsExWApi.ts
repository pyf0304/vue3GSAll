...
import {
  vCurrEduCls_GetObjLstAsync,
  vCurrEduCls_SortFunByKey,
  vCurrEduCls_FilterFunByKey,
 ...
 * @returns 对象列表
 */
export async function vCurrEduClsEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvCurrEduClsENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvCurrEduClsExObjLst) {
      await vCurrEduClsEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvCurrEduCls_Sel = arrvCurrEduCls_Sel.sort(
        vCurrEduClsEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function vCurrEduClsEx_FuncMapByFldName(
  strFldName: string,
  objvCurrEduClsEx: clsvCurrEduClsENEx,
) {
