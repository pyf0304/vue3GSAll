 ...
 * @returns 对象列表
 */
export async function XzUniZoneEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsXzUniZoneENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrXzUniZoneObjLst = await XzUniZone_GetObjLstCache();
  ...
  const objXzUniZoneCond = new clsXzUniZoneENEx();
