...
import {
  ...
  gs_TeachingDate_GetObjLstAsync,
  gs_TeachingDate_SortFunByKey,
  gs_TeachingDate_FilterFunByKey,
...
export async function gs_TeachingDateEx_GetObjExLstByPagerCache(
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrgs_TeachingDateExObjLst) {
      await gs_TeachingDateEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrgs_TeachingDate_Sel = arrgs_TeachingDate_Sel.filter(
 ...
 */
export function gs_TeachingDateEx_FuncMapByFldName(
  strFldName: string,
  objgs_TeachingDateEx: clsgs_TeachingDateENEx,
) {
  const strThisFuncName = gs_TeachingDateEx_FuncMapByFldName.name;
  console.log(objgs_TeachingDateEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
