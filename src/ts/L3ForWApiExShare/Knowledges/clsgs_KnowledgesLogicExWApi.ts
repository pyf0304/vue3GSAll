...
import {
  ...
  gs_KnowledgesLogic_GetObjLstCache,
 ...
 * @returns 对象列表
 */
export async function gs_KnowledgesLogicEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strKnowledgeGraphId: string,
): Promise<Array<clsgs_KnowledgesLogicENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrgs_KnowledgesLogicObjLst = await gs_KnowledgesLogic_GetObjLstCache(strKnowledgeGraphId);
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrgs_KnowledgesLogic_Sel = arrgs_KnowledgesLogic_Sel.filter(
...
export function gs_KnowledgesLogicEx_FuncMapByFldName(
  ...
  console.log(objgs_KnowledgesLogicEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
