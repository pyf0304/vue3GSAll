...
import { cc_KnowledgeModules_GetObjLstCache,cc_KnowledgeModules_GetObjLstAsync,cc_KnowledgeModules_SortFunByKey,cc_KnowledgeModules_FilterFunByKey } from "@/ts/L3ForWApi/Knowledges/clscc_KnowledgeModulesWApi";
 ...
 **/
export  function cc_KnowledgeModulesEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export  function cc_KnowledgeModulesEx_CopyToEx(objcc_KnowledgeModulesENS:clscc_KnowledgeModulesEN ): clscc_KnowledgeModulesENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, cc_KnowledgeModulesEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function cc_KnowledgeModulesEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clscc_KnowledgeModulesENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrcc_KnowledgeModulesObjLst = await cc_KnowledgeModules_GetObjLstCache();
const arrcc_KnowledgeModulesExObjLst = arrcc_KnowledgeModulesObjLst.map(cc_KnowledgeModulesEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clscc_KnowledgeModulesEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const objcc_KnowledgeModulesCond = new clscc_KnowledgeModulesENEx();
...
arrcc_KnowledgeModulesSel = arrcc_KnowledgeModulesSel.sort(cc_KnowledgeModulesEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, cc_KnowledgeModulesEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function cc_KnowledgeModulesEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clscc_KnowledgeModulesENEx>> {
const strThisFuncName = "GetObjExLstByPagerAsync";
const arrcc_KnowledgeModulesObjLst = await cc_KnowledgeModules_GetObjLstAsync(objPagerPara.whereCond);
const arrcc_KnowledgeModulesExObjLst = arrcc_KnowledgeModulesObjLst.map(cc_KnowledgeModulesEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clscc_KnowledgeModulesEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
 ...
 **/
export  async function cc_KnowledgeModulesEx_FuncMapCourseName(objcc_KnowledgeModules:clscc_KnowledgeModulesENEx )
{
...
{
const strMsg = Format("(errid:Watl000445)函数映射表对象数据出错,{0}.(in {1}.{2})", e, cc_KnowledgeModulesEx_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
*/
export  function cc_KnowledgeModulesEx_FuncMapByFldName(strFldName: string, objcc_KnowledgeModulesEx: clscc_KnowledgeModulesENEx)
{
 ...
 * @param objcc_KnowledgeModulesS:源对象
 **/
export  async function cc_KnowledgeModulesEx_FuncMapKeyCourseName(objcc_KnowledgeModules:clscc_KnowledgeModulesENEx ): Promise<Array<string>>
{
