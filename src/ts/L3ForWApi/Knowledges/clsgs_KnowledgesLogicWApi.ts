 ...
 * @returns 对象
*/
export  async function gs_KnowledgesLogic_GetObjBymIdCache(lngmId:number,strCourseId:string, bolTryAsyncOnce = true) {
const strThisFuncName = "GetObjBymIdCache";

if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空!(In clsgs_KnowledgesLogicWApi.GetObjBymIdCache)");
console.error(strMsg);
 throw (strMsg);
}
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
try
{
const arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicObjLstCache.filter(x => 
 x.mId == lngmId );
let objgs_KnowledgesLogic: clsgs_KnowledgesLogicEN;
if (arrgs_KnowledgesLogicSel.length > 0)
{
objgs_KnowledgesLogic = arrgs_KnowledgesLogicSel[0];
return objgs_KnowledgesLogic;
}
else
{
...
{
const objgs_KnowledgesLogicConst = await gs_KnowledgesLogic_GetObjBymIdAsync(lngmId);
if (objgs_KnowledgesLogicConst != null)
{
gs_KnowledgesLogic_ReFreshThisCache(strCourseId);
return objgs_KnowledgesLogicConst;
}
}
return null;
}
 ...
 * @returns 对象
*/
export  async function gs_KnowledgesLogic_GetObjBymIdlocalStorage(lngmId: number) {
const strThisFuncName = "GetObjBymIdlocalStorage";

if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空!(In clsgs_KnowledgesLogicWApi.GetObjBymIdlocalStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
const objgs_KnowledgesLogicCache: clsgs_KnowledgesLogicEN = JSON.parse(strTempObj);
return objgs_KnowledgesLogicCache;
}
try
{
 ...
 * @returns 对象
*/
export  async function gs_KnowledgesLogic_UpdateObjInLstCache(objgs_KnowledgesLogic: clsgs_KnowledgesLogicEN,strCourseId: string) {
const strThisFuncName = "UpdateObjInLstCache";
try
{
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
const obj = arrgs_KnowledgesLogicObjLstCache.find(x => x.courseKnowledgeId == objgs_KnowledgesLogic.courseKnowledgeId && x.knowledgeGraphId == objgs_KnowledgesLogic.knowledgeGraphId);
if (obj != null)
{
...
{
arrgs_KnowledgesLogicObjLstCache.push(objgs_KnowledgesLogic);
}
}
catch (e) {
...
{
  const strMsg = Format("参数:[strCourseIdClassfy]不能为空!(In clsgs_KnowledgesLogicWApi.func)");
console.error(strMsg);
 throw (strMsg);
}
if (strCourseIdClassfy.length != 8)
{
const strMsg = Format("缓存分类变量:[strCourseIdClassfy]的长度:[{0}]不正确!(clsgs_KnowledgesLogicWApi.func)", strCourseIdClassfy.length);
console.error(strMsg);
throw (strMsg);
}
...
const objgs_KnowledgesLogic = await gs_KnowledgesLogic_GetObjBymIdCache(lngmId , strCourseIdClassfy);
 ...
 * @returns 返回一个关键字值列表
*/
export  async function gs_KnowledgesLogic_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string, strCourseIdClassfy: string): Promise<Array<number>>
{
//const strThisFuncName = "funcKey";

if (IsNullOrEmpty(strCourseIdClassfy) == true)
{
  const strMsg = Format("参数:[strCourseIdClassfy]不能为空!(In clsgs_KnowledgesLogicWApi.funcKey)");
console.error(strMsg);
 throw (strMsg);
}
if (strCourseIdClassfy.length != 8)
{
const strMsg = Format("缓存分类变量:[strCourseIdClassfy]的长度:[{0}]不正确!(clsgs_KnowledgesLogicWApi.funcKey)", strCourseIdClassfy.length);
console.error(strMsg);
throw (strMsg);
}
...
const arrgs_KnowledgesLogic = await gs_KnowledgesLogic_GetObjLstCache(strCourseIdClassfy);
if (arrgs_KnowledgesLogic == null) return [];
let arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogic;
...
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
break;
case enumComparisonOp.Like_03:
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
break;
case enumComparisonOp.In_04:
arrValues = strInValue.split(',');
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
break;
}
break;
case "boolean":
...
{
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strInFldName) == strInValue);
}
break;
case "number":
...
if (arrgs_KnowledgesLogicSel.length == 0) return [];
return arrgs_KnowledgesLogicSel.map(x=>x.mId);
}
 ...
 **/
export  async function gs_KnowledgesLogic_GetObjLstClientCache(strCourseId: string)
{
...
{
//缓存存在,直接返回
const arrgs_KnowledgesLogicExObjLstCache: Array<clsgs_KnowledgesLogicEN> = CacheHelper.Get(strKey);
const arrgs_KnowledgesLogicObjLstT = gs_KnowledgesLogic_GetObjLstByJSONObjLst(arrgs_KnowledgesLogicExObjLstCache);
return arrgs_KnowledgesLogicObjLstT;
}
try
{
 ...
 * @returns 从本地缓存中获取的对象列表
 **/
export  async function gs_KnowledgesLogic_GetObjLstlocalStorage(strCourseId: string)
{
...
{
strWhereCond = Format("{0}='{1}'",clsgs_KnowledgesLogicEN.con_CourseId, strCourseId);
}
const strKey = Format("{0}_{1}", clsgs_KnowledgesLogicEN._CurrTabName, strCourseId);
if (IsNullOrEmpty(clsgs_KnowledgesLogicEN.CacheAddiCondition) == false)
{
...
const arrgs_KnowledgesLogicExObjLstCache: Array<clsgs_KnowledgesLogicEN> = JSON.parse(strTempObjLst);
const arrgs_KnowledgesLogicObjLstT = gs_KnowledgesLogic_GetObjLstByJSONObjLst(arrgs_KnowledgesLogicExObjLstCache);
return arrgs_KnowledgesLogicObjLstT;
}
try
 ...
 * @returns 从本地缓存中获取的对象列表
 **/
export  async function gs_KnowledgesLogic_GetObjLstlocalStoragePureCache(strCourseId: string)
{
//初始化列表缓存
const strKey = Format("{0}_{1}", clsgs_KnowledgesLogicEN._CurrTabName, strCourseId);
if (strKey == "")
...
//缓存存在,直接返回
const strTempObjLst:string = localStorage.getItem(strKey) as string;
const arrgs_KnowledgesLogicObjLstCache: Array<clsgs_KnowledgesLogicEN> = JSON.parse(strTempObjLst);
 ...
 * @returns 从本地缓存中获取的对象列表
 **/
export  async function gs_KnowledgesLogic_GetObjLstsessionStorage(strCourseId: string)
{
 ...
 * @returns 从本地缓存中获取的对象列表
 **/
export  async function gs_KnowledgesLogic_GetObjLstsessionStoragePureCache(strCourseId: string)
{
//初始化列表缓存
const strKey = Format("{0}_{1}", clsgs_KnowledgesLogicEN._CurrTabName, strCourseId);
if (strKey == "")
 ...
 * @returns 从本地缓存中获取的对象列表
 **/
export  async function gs_KnowledgesLogic_GetObjLstCache(strCourseId: string): Promise<Array<clsgs_KnowledgesLogicEN>> 
{
...
{
  const strMsg = Format("参数:[strCourseId]不能为空！(In clsgs_KnowledgesLogicWApi.gs_KnowledgesLogic_GetObjLstCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strCourseId.length != 8)
{
const strMsg = Format("缓存分类变量:[strCourseId]的长度:[{0}]不正确！(clsgs_KnowledgesLogicWApi.gs_KnowledgesLogic_GetObjLstCache)", strCourseId.length);
console.error(strMsg);
throw (strMsg);
}
let arrgs_KnowledgesLogicObjLstCache;
switch (clsgs_KnowledgesLogicEN.CacheModeId)
{
case "04"://sessionStorage
arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstsessionStorage(strCourseId);
break;
case "03"://localStorage
arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstlocalStorage(strCourseId);
break;
case "02"://ClientCache
arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstClientCache(strCourseId);
break;
default:
arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstClientCache(strCourseId);
break;
 ...
 * @returns 从本地缓存中获取的对象列表
 **/
export  async function gs_KnowledgesLogic_GetObjLstPureCache(strCourseId: string)
{
...
{
case "04"://sessionStorage
arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstsessionStoragePureCache(strCourseId);
break;
case "03"://localStorage
arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstlocalStoragePureCache(strCourseId);
break;
case "02"://ClientCache
arrgs_KnowledgesLogicObjLstCache = null;
break;
default:
arrgs_KnowledgesLogicObjLstCache = null;
break;
 ...
 * @returns 对象列表子集
*/
export  async function gs_KnowledgesLogic_GetSubObjLstCache(objgs_KnowledgesLogicCond: clsgs_KnowledgesLogicEN ,strCourseId: string) {
const strThisFuncName = "GetSubObjLstCache";
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
let arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicObjLstCache;
if (objgs_KnowledgesLogicCond.sfFldComparisonOp == null || objgs_KnowledgesLogicCond.sfFldComparisonOp == "") return arrgs_KnowledgesLogicSel;
const dicFldComparisonOp: { [index: string]: string } = JSON.parse(objgs_KnowledgesLogicCond.sfFldComparisonOp);
//console.log("clsgs_KnowledgesLogicWApi->GetSubObjLstCache->dicFldComparisonOp:");
//console.log(dicFldComparisonOp);
try {
const sstrKeys = GetObjKeys(objgs_KnowledgesLogicCond);
//console.log(sstrKeys);
for (const strKey of sstrKeys) {
if (Object.prototype.hasOwnProperty.call(dicFldComparisonOp, strKey) == false) continue;
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strKey) != null);
const strComparisonOp = dicFldComparisonOp[strKey];
const strValue = objgs_KnowledgesLogicCond.GetFldValue(strKey);
...
if (strComparisonOp == "=") {
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strKey).toString() == strValue.toString());
}
else if (strComparisonOp == "like") {
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strKey).toString().indexOf(strValue.toString()) != -1);
}
else if (strComparisonOp == "length greater") {
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
}
...
if (strValue == null) continue;
if (strComparisonOp == "=") {
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => x.GetFldValue(strKey) == strValue);
}
break;
case "number":
...
return arrgs_KnowledgesLogicSel;
}
catch (e) {
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]缓存对象列表中获取子集对象不成功!(in {2}.{3})", e, JSON.stringify( objgs_KnowledgesLogicCond), gs_KnowledgesLogic_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 对象列表
*/
export  async function gs_KnowledgesLogic_GetObjLstBymIdLstCache(arrmIdLst: Array<number> ,strCourseId: string) {
const strThisFuncName = "GetObjLstBymIdLstCache";
try
{
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
const arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicObjLstCache.filter(x => arrmIdLst.indexOf(x.mId)>-1);
return arrgs_KnowledgesLogicSel;
}
catch (e)
{
 ...
 * @returns 对象列表
*/
export  async function gs_KnowledgesLogic_GetObjLstByPagerCache(objPagerPara: stuPagerPara ,strCourseId: string) {
const strThisFuncName = "GetObjLstByPagerCache";
if (objPagerPara.pageIndex == 0) return new Array<clsgs_KnowledgesLogicEN>();
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
if (arrgs_KnowledgesLogicObjLstCache.length == 0) return arrgs_KnowledgesLogicObjLstCache;
let arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicObjLstCache;
const objCond = JSON.parse(objPagerPara.whereCond);
const objgs_KnowledgesLogicCond = new clsgs_KnowledgesLogicEN();
ObjectAssign(objgs_KnowledgesLogicCond, objCond);
let dicFldComparisonOp: { [index: string]: string } = {};
if (objCond.sfFldComparisonOp != "")
{
dicFldComparisonOp = JSON.parse(objCond.sfFldComparisonOp);
}
//console.log("clsgs_KnowledgesLogicWApi->GetObjLstByPagerCache->dicFldComparisonOp:");
//console.log(dicFldComparisonOp);
try {
...
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.filter(x => arrValues.indexOf(x.GetFldValue(strKey).toString()) != -1);
}
break;
case "boolean":
if (strValue == null) continue;
...
if (arrgs_KnowledgesLogicSel.length == 0) return arrgs_KnowledgesLogicSel;
...
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.sort(gs_KnowledgesLogic_SortFunByKey(strSortFld, strSortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.sort(objPagerPara.sortFun);
}
arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicSel.slice(intStart, intEnd);     
return arrgs_KnowledgesLogicSel;
}
catch (e) {
 ...
 * @returns 对象列表子集
*/
export  async function gs_KnowledgesLogic_IsExistRecordCache(objgs_KnowledgesLogicCond: clsgs_KnowledgesLogicEN,strCourseId: string) {
const strThisFuncName = "IsExistRecordCache";
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
if (arrgs_KnowledgesLogicObjLstCache == null) return false;
let arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicObjLstCache;
if (objgs_KnowledgesLogicCond.sfFldComparisonOp == null || objgs_KnowledgesLogicCond.sfFldComparisonOp == "") return arrgs_KnowledgesLogicSel.length>0?true:false;
const dicFldComparisonOp: { [index: string]: string } = JSON.parse(objgs_KnowledgesLogicCond.sfFldComparisonOp);
//console.log("clsgs_KnowledgesLogicWApi->GetSubObjLstCache->dicFldComparisonOp:");
//console.log(dicFldComparisonOp);
try {
...
const strMsg = Format("根据条件:[{0}]判断是否存在不成功!(in {1}.{2})", JSON.stringify( objgs_KnowledgesLogicCond), gs_KnowledgesLogic_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
return false;
 ...
 * @returns 对象
*/
export  async function gs_KnowledgesLogic_IsExistCache(lngmId:number,strCourseId:string) {
const strThisFuncName = "IsExistCache";
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
if (arrgs_KnowledgesLogicObjLstCache == null) return false;
try
{
const arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicObjLstCache.filter(x => x.mId == lngmId);
if (arrgs_KnowledgesLogicSel.length > 0)
{
return true;
}
 ...
 * @param objgs_KnowledgesLogicCond:条件对象
 * @returns 对象列表记录数
*/
export  async function gs_KnowledgesLogic_GetRecCountByCondCache(objgs_KnowledgesLogicCond: clsgs_KnowledgesLogicEN ,strCourseId: string) {
const strThisFuncName = "GetRecCountByCondCache";
const arrgs_KnowledgesLogicObjLstCache = await gs_KnowledgesLogic_GetObjLstCache(strCourseId);
if (arrgs_KnowledgesLogicObjLstCache == null) return 0;
let arrgs_KnowledgesLogicSel = arrgs_KnowledgesLogicObjLstCache;
if (objgs_KnowledgesLogicCond.sfFldComparisonOp == null || objgs_KnowledgesLogicCond.sfFldComparisonOp == "") return arrgs_KnowledgesLogicSel.length;
const dicFldComparisonOp: { [index: string]: string } = JSON.parse(objgs_KnowledgesLogicCond.sfFldComparisonOp);
//console.log("clsgs_KnowledgesLogicWApi->GetSubObjLstCache->dicFldComparisonOp:");
//console.log(dicFldComparisonOp);
try {
...
return arrgs_KnowledgesLogicSel.length;
}
catch (e) {
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]从缓存对象列表中获取记录数不成功!(in {2}.{3})", e, JSON.stringify( objgs_KnowledgesLogicCond), gs_KnowledgesLogic_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
return 0;
...
if (IsNullOrEmpty(strCourseId) == true)
{
  const strMsg = Format("参数:[strCourseId]不能为空!(In clsgs_KnowledgesLogicWApi.gs_KnowledgesLogic_ReFreshThisCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strCourseId.length != 8)
{
const strMsg = Format("缓存分类变量:[strCourseId]的长度:[{0}]不正确!(clsgs_KnowledgesLogicWApi.gs_KnowledgesLogic_ReFreshThisCache)", strCourseId.length);
console.error(strMsg);
throw (strMsg);
}
...
{
 throw new Error("(errid:Watl000413)字段[知识点标题(courseKnowledgeTitle)]的长度不能超过100(In 知识点逻辑(gs_KnowledgesLogic))!值:$(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle)(clsgs_KnowledgesLogicBL:CheckPropertyNew)");
}
...
{
 throw new Error("(errid:Watl000414)字段[知识点标题(courseKnowledgeTitle)]的值:[$(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle)], 非法,应该为字符型(In 知识点逻辑(gs_KnowledgesLogic))!(clsgs_KnowledgesLogicBL:CheckPropertyNew0)");
}
...
if (IsNullOrEmpty(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle) == false && GetStrLen(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle) > 100)
{
 throw new Error("(errid:Watl000416)字段[知识点标题(courseKnowledgeTitle)]的长度不能超过100(In 知识点逻辑(gs_KnowledgesLogic))!值:$(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle)(clsgs_KnowledgesLogicBL:CheckProperty4Update)");
}
if (IsNullOrEmpty(pobjgs_KnowledgesLogicEN.knowledgeGraphId) == false && GetStrLen(pobjgs_KnowledgesLogicEN.knowledgeGraphId) > 10)
{
...
if (IsNullOrEmpty(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle) == false && undefined !== pobjgs_KnowledgesLogicEN.courseKnowledgeTitle && tzDataType.isString(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle) === false)
{
 throw new Error("(errid:Watl000417)字段[知识点标题(courseKnowledgeTitle)]的值:[$(pobjgs_KnowledgesLogicEN.courseKnowledgeTitle)], 非法,应该为字符型(In 知识点逻辑(gs_KnowledgesLogic))!(clsgs_KnowledgesLogicBL:CheckProperty4Update)");
}
if (null != pobjgs_KnowledgesLogicEN.xPosition && undefined !== pobjgs_KnowledgesLogicEN.xPosition && tzDataType.isNumber(pobjgs_KnowledgesLogicEN.xPosition) === false)
{
...
if (Object.prototype.hasOwnProperty.call(objgs_KnowledgesLogicCond.dicFldComparisonOp, clsgs_KnowledgesLogicEN.con_CourseKnowledgeTitle) == true)
{
const strComparisonOpCourseKnowledgeTitle:string = objgs_KnowledgesLogicCond.dicFldComparisonOp[clsgs_KnowledgesLogicEN.con_CourseKnowledgeTitle];
strWhereCond += Format(" And {0} {2} '{1}'", clsgs_KnowledgesLogicEN.con_CourseKnowledgeTitle, objgs_KnowledgesLogicCond.courseKnowledgeTitle, strComparisonOpCourseKnowledgeTitle);
}
