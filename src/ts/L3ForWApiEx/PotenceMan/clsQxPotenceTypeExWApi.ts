...
import { QxPotenceType_GetObjLstCache,QxPotenceType_GetObjLstAsync,QxPotenceType_SortFunByKey } from "@/ts/L3ForWApi/PotenceMan/clsQxPotenceTypeWApi";
...
import { QxFuncModule_func,QxFuncModule_funcKey } from "@/ts/L3ForWApi/PrjManage_GP/clsQxFuncModuleWApi";
import { clsQxFuncModuleEN } from "@/ts/L0Entity/PrjManage_GP/clsQxFuncModuleEN";
import { QxPrjMenus_func,QxPrjMenus_funcKey } from "@/ts/L3ForWApi/MenuManage_GP/clsQxPrjMenusWApi";
 ...
 **/
export  function QxPotenceTypeEx_CopyToEx(objQxPotenceTypeENS:clsQxPotenceTypeEN ): clsQxPotenceTypeENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceTypeEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function QxPotenceTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQxPotenceTypeENEx>> {
...
const arrQxPotenceTypeExObjLst = arrQxPotenceTypeObjLst.map(QxPotenceTypeEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsQxPotenceTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
arrQxPotenceTypeSel = arrQxPotenceTypeSel.sort(QxPotenceTypeEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, qxPotenceTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function QxPotenceTypeEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsQxPotenceTypeENEx>> {
 ...
 **/
export  async function QxPotenceTypeEx_FuncMapFuncModuleName(objQxPotenceType:clsQxPotenceTypeENEx )
{
...
{
const strMsg = Format("(errid:Watl000342)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
if (IsNullOrEmpty(objQxPotenceType.menuName) == true){
 ...
 const QxPrjMenusMenuName = await QxPrjMenus_func(clsQxPrjMenusEN.con_MenuId, clsQxPrjMenusEN.con_MenuName, QxPrjMenusMenuId );
 objQxPotenceType.menuName = QxPrjMenusMenuName;
}
}
catch (e)
{
const strMsg = Format("(errid:Watl000403)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
{
const strMsg = Format("(errid:Watl000476)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
*/
export  function QxPotenceTypeEx_FuncMapByFldName(strFldName: string, objQxPotenceTypeEx: clsQxPotenceTypeENEx)
{
 ...
 * @param objQxPotenceTypeS:源对象
 **/
export  async function QxPotenceTypeEx_FuncMapKeyFuncModuleName(objQxPotenceType:clsQxPotenceTypeENEx ): Promise<Array<string>>
{
...
if (IsNullOrEmpty(objQxPotenceType.funcModuleName) == true) return [];
 ...
 const arrFuncModuleId = await QxFuncModule_funcKey(clsQxFuncModuleEN.con_FuncModuleName, QxFuncModuleFuncModuleName , enumComparisonOp.Like_03);
 ...
 * @param objQxPotenceTypeS:源对象
 **/
export  async function QxPotenceTypeEx_FuncMapKeyMenuName(objQxPotenceType:clsQxPotenceTypeENEx ): Promise<Array<string>>
{
...
if (IsNullOrEmpty(objQxPotenceType.menuName) == true) return [];
 ...
 const arrMenuId = await QxPrjMenus_funcKey(clsQxPrjMenusEN.con_MenuName, QxPrjMenusMenuName , enumComparisonOp.Like_03);
 return arrMenuId;
}
catch (e)
{
 ...
 * @param objQxPotenceTypeS:源对象
 **/
export  async function QxPotenceTypeEx_FuncMapKeyDateTimeSim(objQxPotenceType:clsQxPotenceTypeENEx ): Promise<Array<string>>
{
