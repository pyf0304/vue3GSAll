...
import { vCurrEduClsTeacher_GetObjLstCache,vCurrEduClsTeacher_GetObjLstAsync,vCurrEduClsTeacher_SortFunByKey } from "@/ts/L3ForWApi/DailyRunning/clsvCurrEduClsTeacherWApi";
 ...
 * @returns 对象列表
*/
export  async function vCurrEduClsTeacherEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara , strIdCurrEduCls:string):Promise<Array<clsvCurrEduClsTeacherENEx>> {
...
const arrvCurrEduClsTeacherExObjLst = arrvCurrEduClsTeacherObjLst.map(vCurrEduClsTeacherEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsvCurrEduClsTeacherEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
arrvCurrEduClsTeacherSel = arrvCurrEduClsTeacherSel.sort(vCurrEduClsTeacherEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const arrvCurrEduClsTeacherObjLst = await vCurrEduClsTeacher_GetObjLstAsync(objPagerPara.whereCond);
const arrvCurrEduClsTeacherExObjLst = arrvCurrEduClsTeacherObjLst.map(vCurrEduClsTeacherEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsvCurrEduClsTeacherEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
