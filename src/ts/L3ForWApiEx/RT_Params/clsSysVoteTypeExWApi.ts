...
import {
  SysVoteType_GetObjLstCache,
  SysVoteType_GetObjLstAsync,
  SysVoteType_SortFunByKey,
  SysVoteType_FilterFunByKey,
} from '@/ts/L3ForWApi/RT_Params/clsSysVoteTypeWApi';
import { clsSysVoteTypeEN } from '@/ts/L0Entity/RT_Params/clsSysVoteTypeEN';
import { clsSysVoteTypeENEx } from '@/ts/L0Entity/RT_Params/clsSysVoteTypeENEx';
 ...
 * @returns 对象列表
 */
export async function SysVoteTypeEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSysVoteTypeENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrSysVoteTypeObjLst = await SysVoteType_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrSysVoteTypeExObjLst) {
      await SysVoteTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrSysVoteType_Sel = arrSysVoteType_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrSysVoteType_Sel = arrSysVoteType_Sel.sort(
        SysVoteTypeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function SysVoteTypeEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSysVoteTypeENEx>> {
 ...
 */
export function SysVoteTypeEx_FuncMapByFldName(
  strFldName: string,
  objSysVoteTypeEx: clsSysVoteTypeENEx,
) {
  const strThisFuncName = SysVoteTypeEx_FuncMapByFldName.name;
  console.log(objSysVoteTypeEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
