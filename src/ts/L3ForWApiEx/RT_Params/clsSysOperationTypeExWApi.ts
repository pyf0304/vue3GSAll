...
import {
  SysOperationType_GetObjLstCache,
  SysOperationType_GetObjLstAsync,
  SysOperationType_SortFunByKey,
  SysOperationType_FilterFunByKey,
} from '@/ts/L3ForWApi/RT_Params/clsSysOperationTypeWApi';
import { clsSysOperationTypeEN } from '@/ts/L0Entity/RT_Params/clsSysOperationTypeEN';
import { clsSysOperationTypeENEx } from '@/ts/L0Entity/RT_Params/clsSysOperationTypeENEx';
 ...
 **/
export function SysOperationTypeEx_CopyToEx(
  objSysOperationTypeENS: clsSysOperationTypeEN,
): clsSysOperationTypeENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      sysOperationTypeEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function SysOperationTypeEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSysOperationTypeENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrSysOperationTypeObjLst = await SysOperationType_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrSysOperationTypeExObjLst) {
      await SysOperationTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrSysOperationType_Sel = arrSysOperationType_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrSysOperationType_Sel = arrSysOperationType_Sel.sort(
        SysOperationTypeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function SysOperationTypeEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSysOperationTypeENEx>> {
 ...
 */
export function SysOperationTypeEx_FuncMapByFldName(
  strFldName: string,
  objSysOperationTypeEx: clsSysOperationTypeENEx,
) {
  const strThisFuncName = SysOperationTypeEx_FuncMapByFldName.name;
  console.log(objSysOperationTypeEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
