...
//import $ from "jquery";
import {
  vgs_TeachingDate_GetObjLstCache,
  vgs_TeachingDate_GetObjLstAsync,
  vgs_TeachingDate_SortFunByKey,
  vgs_TeachingDate_FilterFunByKey,
 ...
 * @returns 对象列表
 */
export async function vgs_TeachingDateEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strIdCurrEducls: string,
): Promise<Array<clsvgs_TeachingDateENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrvgs_TeachingDateObjLst = await vgs_TeachingDate_GetObjLstCache(strIdCurrEducls);
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvgs_TeachingDateExObjLst) {
      await vgs_TeachingDateEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrvgs_TeachingDate_Sel = arrvgs_TeachingDate_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvgs_TeachingDate_Sel = arrvgs_TeachingDate_Sel.sort(
        vgs_TeachingDateEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function vgs_TeachingDateEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvgs_TeachingDateENEx>> {
 ...
 */
export function vgs_TeachingDateEx_FuncMapByFldName(
  strFldName: string,
  objvgs_TeachingDateEx: clsvgs_TeachingDateENEx,
) {
  const strThisFuncName = vgs_TeachingDateEx_FuncMapByFldName.name;
  console.log(objvgs_TeachingDateEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
