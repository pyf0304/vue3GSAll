...
//import $ from "jquery";
import {
  vDiscussionTopics_GetObjLstCache,
  vDiscussionTopics_GetObjLstAsync,
  vDiscussionTopics_SortFunByKey,
  vDiscussionTopics_FilterFunByKey,
 ...
 **/
export function vDiscussionTopicsEx_CopyToEx(
  objvDiscussionTopicsENS: clsvDiscussionTopicsEN,
): clsvDiscussionTopicsENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vDiscussionTopicsEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vDiscussionTopicsEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strIdCurrEducls: string,
): Promise<Array<clsvDiscussionTopicsENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrvDiscussionTopicsObjLst = await vDiscussionTopics_GetObjLstCache(strIdCurrEducls);
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvDiscussionTopicsExObjLst) {
      await vDiscussionTopicsEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrvDiscussionTopics_Sel = arrvDiscussionTopics_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvDiscussionTopics_Sel = arrvDiscussionTopics_Sel.sort(
        vDiscussionTopicsEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function vDiscussionTopicsEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvDiscussionTopicsENEx>> {
 ...
 */
export function vDiscussionTopicsEx_FuncMapByFldName(
  strFldName: string,
  objvDiscussionTopicsEx: clsvDiscussionTopicsENEx,
) {
  const strThisFuncName = vDiscussionTopicsEx_FuncMapByFldName.name;
  console.log(objvDiscussionTopicsEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
