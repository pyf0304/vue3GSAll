...
import {
  gs_UserRelaPosition_FilterFunByKey,
  gs_UserRelaPosition_GetObjLstAsync,
  gs_UserRelaPosition_GetObjLstCache,
  gs_UserRelaPosition_SortFunByKey,
...
import { clsgs_UserRelaPositionENEx } from '@/ts/L0Entity/GradEduTools/clsgs_UserRelaPositionENEx';
 ...
 **/
export function gs_UserRelaPositionEx_GetWebApiUrl(
 ...
 **/
export function gs_UserRelaPositionEx_CopyToEx(
  objgs_UserRelaPositionENS: clsgs_UserRelaPositionEN,
): clsgs_UserRelaPositionENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      gs_UserRelaPositionEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function gs_UserRelaPositionEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strIdCurrEducls: string,
): Promise<Array<clsgs_UserRelaPositionENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrgs_UserRelaPositionObjLst = await gs_UserRelaPosition_GetObjLstCache(strIdCurrEducls);
  const arrgs_UserRelaPositionExObjLst = arrgs_UserRelaPositionObjLst.map(
    gs_UserRelaPositionEx_CopyToEx,
  );
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrgs_UserRelaPositionExObjLst) {
      await gs_UserRelaPositionEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  if (arrgs_UserRelaPositionExObjLst.length == 0) return arrgs_UserRelaPositionExObjLst;
  let arrgs_UserRelaPosition_Sel: Array<clsgs_UserRelaPositionENEx> =
    arrgs_UserRelaPositionExObjLst;
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrgs_UserRelaPosition_Sel = arrgs_UserRelaPosition_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrgs_UserRelaPosition_Sel = arrgs_UserRelaPosition_Sel.sort(
        gs_UserRelaPositionEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function gs_UserRelaPositionEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsgs_UserRelaPositionENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrgs_UserRelaPositionExObjLst = await gs_UserRelaPosition_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
 ...
 */
export function gs_UserRelaPositionEx_FuncMapByFldName(
  strFldName: string,
  objgs_UserRelaPositionEx: clsgs_UserRelaPositionENEx,
) {
  const strThisFuncName = gs_UserRelaPositionEx_FuncMapByFldName.name;
  console.log(objgs_UserRelaPositionEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
...
export async function gs_UserRelaPositionEx_AddNewRecordAsync(strJSON: string): Promise<boolean> {
  ...
  //strIdCurrEduclsstrJSON = JSON.stringify(arrgs_UserRelaPosition);
  //strJson = eval(strJSON);

  //strJson = JSON.parse(strJSON);
  //varJSON = JSON.stringify(strJson);

  const strUrl = gs_UserRelaPositionEx_GetWebApiUrl(gs_UserRelaPositionEx_Controller, strAction);
  try {
    ...
    if (error.statusText == 'error') {
      const strInfo = Format(
        ...
        strUrl,
        gs_UserRelaPositionEx_Controller,
        strThisFuncName,
      );
