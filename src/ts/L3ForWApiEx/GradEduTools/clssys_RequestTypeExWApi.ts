...
//import $ from "jquery";
import {
  sys_RequestType_GetObjLstCache,
  sys_RequestType_GetObjLstAsync,
  sys_RequestType_SortFunByKey,
  sys_RequestType_FilterFunByKey,
} from '@/ts/L3ForWApi/GradEduTools/clssys_RequestTypeWApi';
...
import { clssys_RequestTypeENEx } from '@/ts/L0Entity/GradEduTools/clssys_RequestTypeENEx';
 ...
 **/
export function sys_RequestTypeEx_CopyToEx(
  objsys_RequestTypeENS: clssys_RequestTypeEN,
): clssys_RequestTypeENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      sys_RequestTypeEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function sys_RequestTypeEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clssys_RequestTypeENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrsys_RequestTypeObjLst = await sys_RequestType_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrsys_RequestTypeExObjLst) {
      await sys_RequestTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrsys_RequestType_Sel = arrsys_RequestType_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrsys_RequestType_Sel = arrsys_RequestType_Sel.sort(
        sys_RequestTypeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function sys_RequestTypeEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clssys_RequestTypeENEx>> {
 ...
 */
export function sys_RequestTypeEx_FuncMapByFldName(
  strFldName: string,
  objsys_RequestTypeEx: clssys_RequestTypeENEx,
) {
  const strThisFuncName = sys_RequestTypeEx_FuncMapByFldName.name;
  console.log(objsys_RequestTypeEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
