...
import {
  ...
  ViewpointVer_SortFunByKey,
  ViewpointVer_FilterFunByKey,
} from '@/ts/L3ForWApi/GradEduTopic/clsViewpointVerWApi';
import { clsViewpointVerENEx } from '@/ts/L0Entity/GradEduTopic/clsViewpointVerENEx';
 ...
 **/
export function ViewpointVerEx_CopyToEx(
  objViewpointVerENS: clsViewpointVerEN,
): clsViewpointVerENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      viewpointVerEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function ViewpointVerEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsViewpointVerENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrViewpointVerExObjLst) {
      await ViewpointVerEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrViewpointVer_Sel = arrViewpointVer_Sel.sort(
        ViewpointVerEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function ViewpointVerEx_FuncMapByFldName(
  strFldName: string,
  objViewpointVerEx: clsViewpointVerENEx,
) {
  const strThisFuncName = ViewpointVerEx_FuncMapByFldName.name;
  console.log(objViewpointVerEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
