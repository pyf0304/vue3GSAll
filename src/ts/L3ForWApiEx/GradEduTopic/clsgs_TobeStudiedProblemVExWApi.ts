 ...
 **/
export function gs_TobeStudiedProblemVEx_GetWebApiUrl(
 ...
 **/
export function gs_TobeStudiedProblemVEx_CopyToEx(
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      gs_TobeStudiedProblemVEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function gs_TobeStudiedProblemVEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsgs_TobeStudiedProblemVerENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrgs_TobeStudiedProblemVExObjLst = await gs_TobeStudiedProblemVer_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrgs_TobeStudiedProblemVExObjLst) {
      await gs_TobeStudiedProblemVEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  let arrgs_TobeStudiedProblemVer_Sel: Array<clsgs_TobeStudiedProblemVerENEx> =
    arrgs_TobeStudiedProblemVExObjLst;
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrgs_TobeStudiedProblemVer_Sel = arrgs_TobeStudiedProblemVer_Sel.sort(
        gs_TobeStudiedProblemVEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function gs_TobeStudiedProblemVEx_FuncMapByFldName(
  strFldName: string,
  objgs_TobeStudiedProblemVEx: clsgs_TobeStudiedProblemVerENEx,
) {
  const strThisFuncName = gs_TobeStudiedProblemVEx_FuncMapByFldName.name;
  console.log(objgs_TobeStudiedProblemVEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
