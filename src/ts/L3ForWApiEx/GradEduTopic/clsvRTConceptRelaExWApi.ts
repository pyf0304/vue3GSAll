...
//import $ from "jquery";
import {
  vRTConceptRela_FilterFunByKey,
  vRTConceptRela_GetObjLstCache,
  vRTConceptRela_GetObjLstAsync,
  vRTConceptRela_SortFunByKey,
 ...
 **/
export function vRTConceptRelaEx_CopyToEx(
  objvRTConceptRelaENS: clsvRTConceptRelaEN,
): clsvRTConceptRelaENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vRTConceptRelaEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vRTConceptRelaEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvRTConceptRelaENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrvRTConceptRelaObjLst = await vRTConceptRela_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvRTConceptRelaExObjLst) {
      await vRTConceptRelaEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrvRTConceptRela_Sel = arrvRTConceptRela_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvRTConceptRela_Sel = arrvRTConceptRela_Sel.sort(
        vRTConceptRelaEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function vRTConceptRelaEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvRTConceptRelaENEx>> {
 ...
 */
export function vRTConceptRelaEx_FuncMapByFldName(
  strFldName: string,
  objvRTConceptRelaEx: clsvRTConceptRelaENEx,
) {
  const strThisFuncName = vRTConceptRelaEx_FuncMapByFldName.name;
  console.log(objvRTConceptRelaEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
