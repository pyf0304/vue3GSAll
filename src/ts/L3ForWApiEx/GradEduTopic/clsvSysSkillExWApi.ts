...
//import $ from "jquery";
import {
  vSysSkill_FilterFunByKey,
  vSysSkill_GetObjLstCache,
  vSysSkill_GetObjLstAsync,
  vSysSkill_SortFunByKey,
 ...
 * @returns 对象列表
 */
export async function vSysSkillEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strIdCurrEducls: string,
): Promise<Array<clsvSysSkillENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrvSysSkillObjLst = await vSysSkill_GetObjLstCache(strIdCurrEducls);
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvSysSkillExObjLst) {
      await vSysSkillEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrvSysSkill_Sel = arrvSysSkill_Sel.filter(
 ...
 * @returns 对象列表
 */
export async function vSysSkillEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvSysSkillENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrvSysSkillObjLst = await vSysSkill_GetObjLstAsync(objPagerPara.whereCond);
  const arrvSysSkillExObjLst = arrvSysSkillObjLst.map(vSysSkillEx_CopyToEx);
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvSysSkillExObjLst) {
