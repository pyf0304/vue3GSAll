...
import {
  vConceptCitation_FilterFunByKey,
  vConceptCitation_GetObjLstAsync,
  vConceptCitation_SortFunByKey,
} from '@/ts/L3ForWApi/GradEduTopic/clsvConceptCitationWApi';
import { clsvConceptCitationENEx } from '@/ts/L0Entity/GradEduTopic/clsvConceptCitationENEx';
 ...
 **/
export function vConceptCitationEx_CopyToEx(
  objvConceptCitationENS: clsvConceptCitationEN,
): clsvConceptCitationENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vConceptCitationEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vConceptCitationEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvConceptCitationENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvConceptCitationExObjLst) {
      await vConceptCitationEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvConceptCitation_Sel = arrvConceptCitation_Sel.sort(
        vConceptCitationEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function vConceptCitationEx_FuncMapByFldName(
  strFldName: string,
  objvConceptCitationEx: clsvConceptCitationENEx,
) {
  const strThisFuncName = vConceptCitationEx_FuncMapByFldName.name;
  console.log(objvConceptCitationEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
