...
//import $ from "jquery";
import {
  SysSocialRelations_FilterFunByKey,
  SysSocialRelations_GetObjLstCache,
  SysSocialRelations_GetObjLstAsync,
  SysSocialRelations_SortFunByKey,
...
import { clsSysSocialRelationsENEx } from '@/ts/L0Entity/GradEduTopic/clsSysSocialRelationsENEx';
 ...
 **/
export function SysSocialRelationsEx_GetWebApiUrl(
 ...
 **/
export function SysSocialRelationsEx_CopyToEx(
  objSysSocialRelationsENS: clsSysSocialRelationsEN,
): clsSysSocialRelationsENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      sysSocialRelationsEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function SysSocialRelationsEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strIdCurrEducls: string,
): Promise<Array<clsSysSocialRelationsENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrSysSocialRelationsObjLst = await SysSocialRelations_GetObjLstCache(strIdCurrEducls);
  const arrSysSocialRelationsExObjLst = arrSysSocialRelationsObjLst.map(
    SysSocialRelationsEx_CopyToEx,
  );
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrSysSocialRelationsExObjLst) {
      await SysSocialRelationsEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrSysSocialRelations_Sel = arrSysSocialRelations_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrSysSocialRelations_Sel = arrSysSocialRelations_Sel.sort(
        SysSocialRelationsEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function SysSocialRelationsEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSysSocialRelationsENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrSysSocialRelationsExObjLst = await SysSocialRelations_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
 ...
 */
export function SysSocialRelationsEx_FuncMapByFldName(
  strFldName: string,
  objSysSocialRelationsEx: clsSysSocialRelationsENEx,
) {
  const strThisFuncName = SysSocialRelationsEx_FuncMapByFldName.name;
  console.log(objSysSocialRelationsEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
