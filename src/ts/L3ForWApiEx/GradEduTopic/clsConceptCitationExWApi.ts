...
import { clsConceptCitationENEx } from '@/ts/L0Entity/GradEduTopic/clsConceptCitationENEx';
import {
  ConceptCitation_FilterFunByKey,
  ConceptCitation_GetObjLstAsync,
  ConceptCitation_SortFunByKey,
 ...
 **/
export function ConceptCitationEx_CopyToEx(
  objConceptCitationENS: clsConceptCitationEN,
): clsConceptCitationENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      conceptCitationEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function ConceptCitationEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsConceptCitationENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrConceptCitationExObjLst) {
      await ConceptCitationEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrConceptCitation_Sel = arrConceptCitation_Sel.sort(
        ConceptCitationEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function ConceptCitationEx_FuncMapByFldName(
  strFldName: string,
  objConceptCitationEx: clsConceptCitationENEx,
) {
  const strThisFuncName = ConceptCitationEx_FuncMapByFldName.name;
  console.log(objConceptCitationEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
