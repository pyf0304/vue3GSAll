 ...
 **/
export function SysSocialRelationsVEx_GetWebApiUrl(
 ...
 **/
export function SysSocialRelationsVEx_CopyToEx(
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      sysSocialRelationsVEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function SysSocialRelationsVEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSysSocialRelationsVerENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrSysSocialRelationsVExObjLst = await SysSocialRelationsVer_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrSysSocialRelationsVExObjLst) {
      await SysSocialRelationsVEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  let arrSysSocialRelationsVer_Sel: Array<clsSysSocialRelationsVerENEx> =
    arrSysSocialRelationsVExObjLst;
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrSysSocialRelationsVer_Sel = arrSysSocialRelationsVer_Sel.sort(
        SysSocialRelationsVEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function SysSocialRelationsVEx_FuncMapByFldName(
  strFldName: string,
  objSysSocialRelationsVEx: clsSysSocialRelationsVerENEx,
) {
  const strThisFuncName = SysSocialRelationsVEx_FuncMapByFldName.name;
  console.log(objSysSocialRelationsVEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
