...
import {
  vRTResearchResult_GetObjLstAsync,
  vRTResearchResult_GetObjLstCache,
  vRTResearchResult_SortFunByKey,
 ...
 **/
export function vRTResearchResultEx_CopyToEx(
  objvRTResearchResultENS: clsvRTResearchResultEN,
): clsvRTResearchResultENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vRTResearchResultEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vRTResearchResultEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvRTResearchResultENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrvRTResearchResultExObjLst = await vRTResearchResult_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvRTResearchResultExObjLst) {
      await vRTResearchResultEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvRTResearchResult_Sel = arrvRTResearchResult_Sel.sort(
        vRTResearchResultEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function vRTResearchResultEx_FuncMapByFldName(
  strFldName: string,
  objvRTResearchResultEx: clsvRTResearchResultENEx,
) {
  const strThisFuncName = vRTResearchResultEx_FuncMapByFldName.name;
  console.log(objvRTResearchResultEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
 ...
 * @returns 对象列表
 */
export async function vRTResearchResultEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvRTResearchResultENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrvRTResearchResultObjLst = await vRTResearchResult_GetObjLstCache();
  const arrvRTResearchResultExObjLst = arrvRTResearchResultObjLst.map(vRTResearchResultEx_CopyToEx);
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (
    IsNullOrEmpty(objSortInfo.SortFld) == false &&
    clsvRTResearchResultEN.AttributeName.indexOf(objSortInfo.SortFld) == -1
  ) {
    for (const objInFor of arrvRTResearchResultExObjLst) {
      await vRTResearchResultEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  ...
  if (arrvRTResearchResultExObjLst.length == 0) return arrvRTResearchResultExObjLst;
  let arrvRTResearchResultSel: Array<clsvRTResearchResultENEx> = arrvRTResearchResultExObjLst;
