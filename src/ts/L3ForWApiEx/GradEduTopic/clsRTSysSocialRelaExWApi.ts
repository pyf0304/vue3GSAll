...
//import $ from "jquery";
import {
  RTSysSocialRela_FilterFunByKey,
  RTSysSocialRela_GetObjLstCache,
  RTSysSocialRela_GetObjLstAsync,
  RTSysSocialRela_SortFunByKey,
...
import { clsRTSysSocialRelaENEx } from '@/ts/L0Entity/GradEduTopic/clsRTSysSocialRelaENEx';
 ...
 **/
export function RTSysSocialRelaEx_CopyToEx(
  objRTSysSocialRelaENS: clsRTSysSocialRelaEN,
): clsRTSysSocialRelaENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      rTSysSocialRelaEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function RTSysSocialRelaEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsRTSysSocialRelaENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrRTSysSocialRelaObjLst = await RTSysSocialRela_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrRTSysSocialRelaExObjLst) {
      await RTSysSocialRelaEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrRTSysSocialRela_Sel = arrRTSysSocialRela_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrRTSysSocialRela_Sel = arrRTSysSocialRela_Sel.sort(
        RTSysSocialRelaEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function RTSysSocialRelaEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsRTSysSocialRelaENEx>> {
 ...
 */
export function RTSysSocialRelaEx_FuncMapByFldName(
  strFldName: string,
  objRTSysSocialRelaEx: clsRTSysSocialRelaENEx,
) {
  const strThisFuncName = RTSysSocialRelaEx_FuncMapByFldName.name;
  console.log(objRTSysSocialRelaEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
