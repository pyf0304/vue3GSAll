...
//import $ from "jquery";
import {
  ke_Super_FilterFunByKey,
  ke_Super_GetObjLstCache,
  ke_Super_GetObjLstAsync,
  ke_Super_SortFunByKey,
 ...
 * @returns 对象列表
 */
export async function ke_SuperEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clske_SuperENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrke_SuperObjLst = await ke_Super_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrke_SuperExObjLst) {
      await ke_SuperEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrke_Super_Sel = arrke_Super_Sel.filter(
 ...
 * @returns 对象列表
 */
export async function ke_SuperEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clske_SuperENEx>> {
