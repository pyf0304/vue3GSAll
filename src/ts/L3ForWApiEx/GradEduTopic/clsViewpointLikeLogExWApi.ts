...
import {
  ViewpointLikeLog_FilterFunByKey,
  ViewpointLikeLog_GetObjLstAsync,
  ViewpointLikeLog_SortFunByKey,
} from '@/ts/L3ForWApi/GradEduTopic/clsViewpointLikeLogWApi';
import { clsViewpointLikeLogENEx } from '@/ts/L0Entity/GradEduTopic/clsViewpointLikeLogENEx';
 ...
 **/
export function ViewpointLikeLogEx_CopyToEx(
  objViewpointLikeLogENS: clsViewpointLikeLogEN,
): clsViewpointLikeLogENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      viewpointLikeLogEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function ViewpointLikeLogEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsViewpointLikeLogENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrViewpointLikeLogExObjLst) {
      await ViewpointLikeLogEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrViewpointLikeLog_Sel = arrViewpointLikeLog_Sel.sort(
        ViewpointLikeLogEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function ViewpointLikeLogEx_FuncMapByFldName(
  strFldName: string,
  objViewpointLikeLogEx: clsViewpointLikeLogENEx,
) {
  const strThisFuncName = ViewpointLikeLogEx_FuncMapByFldName.name;
  console.log(objViewpointLikeLogEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
