...
import {
  cc_ExcellentType_GetObjLstCache,
  cc_ExcellentType_GetObjLstAsync,
  cc_ExcellentType_SortFunByKey,
  cc_ExcellentType_FilterFunByKey,
...
export async function cc_ExcellentTypeEx_GetObjExLstByPagerCache(
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrcc_ExcellentTypeExObjLst) {
      await cc_ExcellentTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
...
export function cc_ExcellentTypeEx_FuncMapByFldName(
  ...
  console.log(objcc_ExcellentTypeEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
