...
import {
  XzAdminClsType_GetObjLstCache,
  XzAdminClsType_GetObjLstAsync,
  XzAdminClsType_SortFunByKey,
  XzAdminClsType_FilterFunByKey,
} from '@/ts/L3ForWApi/PeopleManage/clsXzAdminClsTypeWApi';
 ...
 * @returns 对象列表
 */
export async function XzAdminClsTypeEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsXzAdminClsTypeENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrXzAdminClsTypeObjLst = await XzAdminClsType_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrXzAdminClsTypeExObjLst) {
      await XzAdminClsTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrXzAdminClsType_Sel = arrXzAdminClsType_Sel.filter(
...
export function XzAdminClsTypeEx_FuncMapByFldName(
  ...
  console.log(objXzAdminClsTypeEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
