...
import {
  qa_Paper_GetObjLstCache,
  qa_Paper_GetObjLstAsync,
  qa_Paper_SortFunByKey,
  qa_Paper_FilterFunByKey,
...
import { clsqa_PaperENEx } from '@/ts/L0Entity/InteractManage/clsqa_PaperENEx';
 ...
 * @returns 对象列表
 */
export async function qa_PaperEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strIdCurrEducls: string,
): Promise<Array<clsqa_PaperENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrqa_PaperObjLst = await qa_Paper_GetObjLstCache(strIdCurrEducls);
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrqa_PaperExObjLst) {
      await qa_PaperEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrqa_Paper_Sel = arrqa_Paper_Sel.filter(
 ...
 * @returns 对象列表
 */
export async function qa_PaperEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsqa_PaperENEx>> {
