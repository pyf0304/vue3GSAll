...
import { cc_WorkStuRelation_GetObjLstAsync,cc_WorkStuRelation_SortFunByKey } from "@/ts/L3ForWApi/InteractManage/clscc_WorkStuRelationWApi";
 ...
 **/
export  function cc_WorkStuRelationEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export  function cc_WorkStuRelationEx_CopyToEx(objcc_WorkStuRelationENS:clscc_WorkStuRelationEN ): clscc_WorkStuRelationENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, cc_WorkStuRelationEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function cc_WorkStuRelationEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clscc_WorkStuRelationENEx>> {
const strThisFuncName = "GetObjExLstByPagerAsync";
const arrcc_WorkStuRelationObjLst = await cc_WorkStuRelation_GetObjLstAsync(objPagerPara.whereCond);
const arrcc_WorkStuRelationExObjLst = arrcc_WorkStuRelationObjLst.map(cc_WorkStuRelationEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clscc_WorkStuRelationEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
arrcc_WorkStuRelationSel = arrcc_WorkStuRelationSel.sort(cc_WorkStuRelationEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, cc_WorkStuRelationEx_ConstructorName, strThisFuncName);
console.error(strMsg);
...
*/
export  function cc_WorkStuRelationEx_FuncMapByFldName(strFldName: string, objcc_WorkStuRelationEx: clscc_WorkStuRelationENEx)
{
