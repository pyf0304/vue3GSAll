...
import {
  qa_AnswerVer_GetObjLstCache,
  qa_AnswerVer_GetObjLstAsync,
  qa_AnswerVer_SortFunByKey,
  qa_AnswerVer_FilterFunByKey,
 ...
 **/
export function qa_AnswerVerEx_CopyToEx(
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      qa_AnswerVerEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function qa_AnswerVerEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsqa_AnswerVerENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrqa_AnswerVerObjLst = await qa_AnswerVer_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrqa_AnswerVerExObjLst) {
      await qa_AnswerVerEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrqa_AnswerVer_Sel = arrqa_AnswerVer_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrqa_AnswerVer_Sel = arrqa_AnswerVer_Sel.sort(
        qa_AnswerVerEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function qa_AnswerVerEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsqa_AnswerVerENEx>> {
 ...
 */
export function qa_AnswerVerEx_FuncMapByFldName(
  strFldName: string,
  objqa_AnswerVerEx: clsqa_AnswerVerENEx,
) {
  const strThisFuncName = qa_AnswerVerEx_FuncMapByFldName.name;
  console.log(objqa_AnswerVerEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
