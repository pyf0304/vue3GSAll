...
import {
  vgs_PConceptRela_GetObjLstAsync,
  vgs_PConceptRela_SortFunByKey,
  vgs_PConceptRela_FilterFunByKey,
} from '@/ts/L3ForWApi/GradEduPaper/clsvgs_PConceptRelaWApi';
import { clsvgs_PConceptRelaENEx } from '@/ts/L0Entity/GradEduPaper/clsvgs_PConceptRelaENEx';
 ...
 **/
export function vgs_PConceptRelaEx_CopyToEx(
  objvgs_PConceptRelaENS: clsvgs_PConceptRelaEN,
): clsvgs_PConceptRelaENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vgs_PConceptRelaEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vgs_PConceptRelaEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvgs_PConceptRelaENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvgs_PConceptRelaExObjLst) {
      await vgs_PConceptRelaEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvgs_PConceptRela_Sel = arrvgs_PConceptRela_Sel.sort(
        vgs_PConceptRelaEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function vgs_PConceptRelaEx_FuncMapByFldName(
  strFldName: string,
  objvgs_PConceptRelaEx: clsvgs_PConceptRelaENEx,
) {
  const strThisFuncName = vgs_PConceptRelaEx_FuncMapByFldName.name;
  console.log(objvgs_PConceptRelaEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
