...
import {
  gs_PViewpointRela_GetObjLstAsync,
  gs_PViewpointRela_SortFunByKey,
  gs_PViewpointRela_FilterFunByKey,
} from '@/ts/L3ForWApi/GradEduPaper/clsgs_PViewpointRelaWApi';
import { clsgs_PViewpointRelaENEx } from '@/ts/L0Entity/GradEduPaper/clsgs_PViewpointRelaENEx';
 ...
 **/
export function gs_PViewpointRelaEx_CopyToEx(
  objgs_PViewpointRelaENS: clsgs_PViewpointRelaEN,
): clsgs_PViewpointRelaENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      gs_PViewpointRelaEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function gs_PViewpointRelaEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsgs_PViewpointRelaENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrgs_PViewpointRelaExObjLst) {
      await gs_PViewpointRelaEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrgs_PViewpointRela_Sel = arrgs_PViewpointRela_Sel.sort(
        gs_PViewpointRelaEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function gs_PViewpointRelaEx_FuncMapByFldName(
  strFldName: string,
  objgs_PViewpointRelaEx: clsgs_PViewpointRelaENEx,
) {
  const strThisFuncName = gs_PViewpointRelaEx_FuncMapByFldName.name;
  console.log(objgs_PViewpointRelaEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
