...
import {
  PaperAppraise_GetObjLstCache,
  PaperAppraise_GetObjLstAsync,
  PaperAppraise_SortFunByKey,
  PaperAppraise_FilterFunByKey,
} from '@/ts/L3ForWApi/GradEduPaper/clsPaperAppraiseWApi';
import { stuPagerPara } from '@/ts/PubFun/stuPagerPara';

import { IsNullOrEmpty, Format } from '@/ts/PubFun/clsString';

import { clsPaperAppraiseENEx } from '@/ts/L0Entity/GradEduPaper/clsPaperAppraiseENEx';
 ...
 **/
export function PaperAppraiseEx_CopyToEx(
  objPaperAppraiseENS: clsPaperAppraiseEN,
): clsPaperAppraiseENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      paperAppraiseEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function PaperAppraiseEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsPaperAppraiseENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrPaperAppraiseObjLst = await PaperAppraise_GetObjLstCache();
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrPaperAppraiseExObjLst) {
      await PaperAppraiseEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrPaperAppraise_Sel = arrPaperAppraise_Sel.filter(
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrPaperAppraise_Sel = arrPaperAppraise_Sel.sort(
        PaperAppraiseEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function PaperAppraiseEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsPaperAppraiseENEx>> {
 ...
 */
export function PaperAppraiseEx_FuncMapByFldName(
  strFldName: string,
  objPaperAppraiseEx: clsPaperAppraiseENEx,
) {
  const strThisFuncName = PaperAppraiseEx_FuncMapByFldName.name;
  console.log(objPaperAppraiseEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
