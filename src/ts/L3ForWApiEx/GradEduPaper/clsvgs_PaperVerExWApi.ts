...
import {
  vgs_PaperVer_GetObjLstAsync,
  vgs_PaperVer_SortFunByKey,
  vgs_PaperVer_FilterFunByKey,
} from '@/ts/L3ForWApi/GradEduPaper/clsvgs_PaperVerWApi';
import { clsvgs_PaperVerENEx } from '@/ts/L0Entity/GradEduPaper/clsvgs_PaperVerENEx';
 ...
 **/
export function vgs_PaperVerEx_CopyToEx(
  objvgs_PaperVerENS: clsvgs_PaperVerEN,
): clsvgs_PaperVerENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vgs_PaperVerEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vgs_PaperVerEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvgs_PaperVerENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvgs_PaperVerExObjLst) {
      await vgs_PaperVerEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvgs_PaperVer_Sel = arrvgs_PaperVer_Sel.sort(
        vgs_PaperVerEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function vgs_PaperVerEx_FuncMapByFldName(
  strFldName: string,
  objvgs_PaperVerEx: clsvgs_PaperVerENEx,
) {
  const strThisFuncName = vgs_PaperVerEx_FuncMapByFldName.name;
  console.log(objvgs_PaperVerEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
