...
import {
  vgs_UserConfig_FilterFunByKey,
  vgs_UserConfig_GetObjLstAsync,
  vgs_UserConfig_GetObjLstCache,
  vgs_UserConfig_SortFunByKey,
 ...
 **/
export function vgs_UserConfigEx_CopyToEx(
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vgs_UserConfigEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vgs_UserConfigEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvgs_UserConfigENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrvgs_UserConfigObjLst = await vgs_UserConfig_GetObjLstCache();
  const arrvgs_UserConfigExObjLst = arrvgs_UserConfigObjLst.map(vgs_UserConfigEx_CopyToEx);
  const objSortInfo = GetSortExpressInfo(objPagerPara);
  if (
    IsNullOrEmpty(objSortInfo.SortFld) == false &&
    clsvgs_UserConfigEN.AttributeName.indexOf(objSortInfo.SortFld) == -1
  ) {
    for (const objInFor of arrvgs_UserConfigExObjLst) {
      await vgs_UserConfigEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  if (arrvgs_UserConfigExObjLst.length == 0) return arrvgs_UserConfigExObjLst;
  let arrvgs_UserConfigSel: Array<clsvgs_UserConfigENEx> = arrvgs_UserConfigExObjLst;
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvgs_UserConfigSel = arrvgs_UserConfigSel.sort(
        vgs_UserConfigEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 * @returns 对象列表
 */
export async function vgs_UserConfigEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvgs_UserConfigENEx>> {
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      const strSortFld = sstrSplit[0];
      if (sstrSplit.length > 1) strSortType = sstrSplit[1];
      arrvgs_UserConfig_Sel = arrvgs_UserConfig_Sel.sort(
        vgs_UserConfigEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空，就调用排序函数
 ...
 */
export function vgs_UserConfigEx_FuncMapByFldName(
  strFldName: string,
  objvgs_UserConfigEx: clsvgs_UserConfigENEx,
) {
  const strThisFuncName = vgs_UserConfigEx_FuncMapByFldName.name;
  console.log(objvgs_UserConfigEx);
  let strMsg = '';
  //如果是本表中字段，不需要映射
