...
export class  clsRTViewpointEN extends clsGeneralTab
{
public static CacheAddiCondition = "";//缓存附加条件,作为向后台调取数据的附加条件
public static CacheModeId = "03"; //localStorage
public static IsUseDelSign = false; //使用删除标志,记录不能删除,仅设置删除标志
public static WhereFormat = ""; //条件格式串
public static _CurrTabName= "RTViewpoint"; //当前表名,与该类相关的表名
public static _KeyFldName= "TopicId,SubViewpointId"; //当前表中的关键字名称,与该类相关的表中关键字名
public static mintAttributeCount = 9;
public static AttributeName = ["topicId", "subViewpointId", "proposePeople", "gsKnowledgeTypeId", "classificationId", "idCurrEduCls", "updDate", "updUser", "memo"];
//以下是属性变量
...
default:
strMsg = `字段名:[${strFldName}]在表对象:[RTViewpoint]中不存在!`;
...
break;
default:
strMsg = `字段名:[${strFldName}]在表对象:[RTViewpoint]中不存在!(in ${this.constructor.name}.${strThisFuncName})`;
console.error(strMsg);
break;
}
}
