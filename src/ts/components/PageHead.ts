...
import { ResearchTopicEx_Bind_TopicList } from '@/ts/L3ForWApiEx/GradEduTopic/clsResearchTopicExWApi';
...
export class PageHead {
  ...
  public async PageLoad() {
    ...
    try {
      if (userStore.getUserId != '') {
        ...
        // 判断角色
        // 管理员
        const isShowEduCls = PageHead.GetPropValue('isShowEduCls');
        if (isShowEduCls == 'true') {
          ...
          //老师
            ...
            userStore.getRoleId == enumQxRoles.Regular_Teacher_00620002 ||
            userStore.getRoleId == enumQxRoles.K_12_Teacher_00620013
          ) {
            await CurrEduClsEx_Bind_EduClsTeaList(
              PageHead.divLayout,
              PageHead.EduCls_Click,
        ...
        const isShowTopic = PageHead.GetPropValue('isShowTopic');
        if (isShowTopic == 'true') {
          setTimeout(() => {
            this.Bind_TopicList();
          }, 200);
  ...
  public async Bind_TopicList() {
    const paperId = PageHead.GetPropValue('paperId');
    await ResearchTopicEx_Bind_TopicList(
      PageHead.divLayout,
      PageHead.Topic_Click,
      'spnTopicName_Head',
      paperId,
    );
  }
  ...
  public static async EduCls_Click(
    ...
    SetSpanHtmlInDivObj(PageHead.divLayout, 'spnEduClsName_Head', strEduName);
  ...
  public static async Topic_Click(strTopicId: string, strTopicName: string) {
    //$("#hidIdCurrEduCls").val(strkey);
    // $('#spnEduClsName_Head').html(strEduName);
    SetSpanHtmlInDivObj(PageHead.divLayout, 'spnTopicName_Head', strTopicName);
