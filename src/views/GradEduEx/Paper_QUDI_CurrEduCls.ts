...
export class Paper_QUDI_CurrEduCls extends PaperCRUD {
  ...
  */
  ...
  public async BindGv_Paper(divList: HTMLDivElement) {
    ...
    const strIdCurrEducls = clsPubLocalStorage.idCurrEduCls;
    if (strIdCurrEducls == '') {
      const strMsg = `当前教学班Id为空！请检查`;
      console.error(strMsg);
      alert(strMsg);
      return;
    }
    strWhereCond += ` and paperId in (select paperId from PaperEduClsRela where ${clsPaperEduClsRelaEN.con_IdCurrEduCls}= '${strIdCurrEducls}')`;
    ...
    try {
      ...
      if (userStore.userType == UserTypeMap.middle_School) {
        //私有空间
        await PaperEx_BindTab_Paper4MSReading(divList, arrPaperExObjLst, this);
      } else {
 ...
 */
  ...
  public async btnAddDownload_Click(strPaperId: string) {
    ...
    try {
      let returnBool = await PaperDownloadLog_AddNewRecordAsync(objPaperDownloadLogEN);
  ...
  public get thisDivList(): HTMLDivElement {
    ...
    const divList = GetDivObjInDivObj(divLayOut, 'divList_CurrEduCls');
    return divList;
  }
}
