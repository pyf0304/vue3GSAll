...
export class Pdf_QA {
  ...
  public async Showdiv_Questions() {
    ...
    const strWhereCond1 = `1=1 and qaPaperId='${GetInputValueInDivObj(divName, 'hidqa_PaperId')}'`;
    ...
    try {
      ...
      const strQuestionsId = Pdf_QA.questionsId;
      if (strQuestionsId != '') {
  ...
  public async Showdiv_PdfQuestions() {
    ...
    try {
      arrqa_QuestionsObjLst = await qa_Questions_GetObjLstAsync(strWhereCond);
      ...
      for (let i = 0; i < arrqa_QuestionsObjLst.length; i++) {
        ...
        const pdfPage = document.getElementsByClassName('page');
  ...
  */
  public async btnUpdateQuestions_Click(strQuestionsId: string) {
    ...
    Pdf_QA.questionsId = strQuestionsId;
    try {
  ...
  */
  public async UpdateRecordSave1() {
    ...
    //
    const objqa_QuestionsEN: clsqa_QuestionsEN = new clsqa_QuestionsEN();
    objqa_QuestionsEN.SetQuestionsId(Pdf_QA.questionsId);
    this.PutDataToqa_QuestionsClass(objqa_QuestionsEN);
  ...
  public async AddNewqa_Push() {
    try {
      ...
      for (let i = 0; i < strUses.length; i++) {
        ...
        objqa_PushEN.SetQuestionsId(Pdf_QA.questionsId);
  ...
  */
  public PutDataToqa_QuestionsClass(pobjqa_QuestionsEN: clsqa_QuestionsEN) {
    ...
    pobjqa_QuestionsEN.SetPdfPageNumIn(GetInputValueInDivObj(divName, 'page_cache'));
    pobjqa_QuestionsEN.SetPdfPageTop(GetInputValueInDivObjN(divName, 'page_top'));
    pobjqa_QuestionsEN.SetPdfZoom(GetInputValueInDivObj(divName, 'pdf_zoom'));

    let pdfDiv_top = GetInputValueInDivObj(divName, 'pdfDiv_top');
    pdfDiv_top = pdfDiv_top.substring(0, pdfDiv_top.length - 2);
    let pdfDiv_left = GetInputValueInDivObj(divName, 'pdfDiv_left');
  ...
  */
  ...
  public async SynAnswer() {
    ...
    //添加记录的同时并记录论文读写的教师评价数
    strWhereCond = ` questionsId='${Pdf_QA.questionsId}'`;
  ...
  public async btnAddVote_Click(strAnswerId: string, strUserId: string) {
    ...
    objSysVoteEN.SetPubParentId(Pdf_QA.questionsId);
    ...
    try {
      ...
      if (arrgs_TagsObjLst.length > 0) {
        ...
        for (let i = 0; i < arrgs_TagsObjLst.length; i++) {
          ...
          strhtml += `<span class="title btn-2"><a href="javascript:void(0)" onclick=btnShowTags_Click("${strTagsId}",${objgs_Tags.pdfPageNum},"${objgs_Tags.pdfContent}"); class="abstract-text">${objgs_Tags.tagsContent}</a></span>`;
          ...
          if (objgs_Tags.updUser == strUserId) {
            ...
            //删除
            strhtml += `<button title="删除标注" class="btn-danger btn btn-sm" onclick=btnDelTags_Click("${strTagsId}") href="javascript:;">${clsIcon.faTrash}</button>`;
          }
 ...
 */
  public PutDataTogs_TagsClass(pobjgs_TagsEN: clsgs_TagsEN) {
    ...
    pobjgs_TagsEN.SetPdfPageNumIn(GetInputValueInDivObj(divName, 'page_cache'));
    pobjgs_TagsEN.SetPdfPageTop(GetInputValueInDivObjN(divName, 'page_top'));
    pobjgs_TagsEN.SetPdfZoom(GetInputValueInDivObj(divName, 'pdf_zoom'));
  ...
  */
  ...
  public set pdfPageNum(value: number) {
    ...
    SetInputValueInDivObj(divName, 'hidPdfPageNum', value);
  }
  /*
   * Pdf页码
   */
  public get pdfPageNum(): number {
    ...
    return GetInputValueInDivObjN(divName, 'hidPdfPageNum');
  }
  ...
  public getDivName(): HTMLDivElement | null {
    ...
    Pdf_QA.divEdit = Pdf_QA.EditObj.$refs.refDivEdit;
    if (Pdf_QA.divEdit == null) {
      if (Pdf_QA.times4TestShowDialog < 2) {
        Pdf_QA.times4TestShowDialog++;
        setTimeout(() => {
          this.getDivName();
        }, 100);
      } else {
      ...
      Pdf_QA.times4TestShowDialog = 0;
    }
    return Pdf_QA.divEdit;
  }
}
