import $ from 'jquery';
import { AddObjectiveFactEx } from './AddObjectiveFactEx';
...
import { TopicObjective_GetObjLstAsync } from '@/ts/L3ForWApi/GradEduTopic/clsTopicObjectiveWApi';
import {
...
export class AddObjectiveFactEx_List extends TopicObjectiveCRUD implements IShowList {
  ...
  BindGv(strType: string) {
    console.log(strType);
    this.BindGv_vTopicObjectiveCache(this.thisDivList);
  }
...
*/
  public async PageLoad() {
    ...
    // 在此处放置用户代码以初始化页面
    try {
      const objPageEdit = new AddObjectiveFactEx('', this);
      ...
      if (userStore.getUserId != '') {
        ...
        if (GetInputValueInDivObj(objDivLayOut, 'hidObjectiveId') != '') {
          await objPageEdit.UpdateRecord(GetInputValueInDivObj(objDivLayOut, 'hidObjectiveId'));
          HideDivInDivObj(objLayOut, 'divLoading');
        } else {
          objPageEdit.AddNewRecord();
          let configTypeId;
          const strhidObjectiveTypeId = GetInputValueInDivObj(objDivLayOut, 'hidObjectiveTypeId');
...
*/
  public async btnDelRecord_Click() {
    try {
      ...
      arrTopicObjectiveObjLst = await TopicObjective_GetObjLstAsync(strWhereCond);
      //查询是否有提交的数据
