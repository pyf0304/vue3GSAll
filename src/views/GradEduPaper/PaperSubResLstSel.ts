import { Ref } from 'vue';
import $ from 'jquery';
import { PaperSubResEx_GetObjExLstByPagerAsync } from '@/ts/L3ForWApiEx/GradEduPaper/clsPaperSubResExWApi';
import { PaperResRelaEx_CopyToEx } from '@/ts/L3ForWApiEx/GradEduPaper/clsPaperResRelaExWApi';
...
import {
  ...
  PaperSubRes_GetRecCountByCondAsync,
...
*/
export class PaperSubResLstSel extends PaperSubResCRUD {
  ...
  public static objPageEdit: PaperSubResLstSel;
   ...
   */
  //论文列表
  public mstrListDivPaperSubRes = 'divDataLst';

  //主题自研论文关系
  public mstrListDivRtOriginalPaperSubResRela = 'divDataLst4RtOriginalPaperSubResRela';

  //主题引用论文关系
  //public mstrListDivRtPaperSubResRela: string = "divRtPaperSubResRelaDataLst";
  public mstrListDivRtCitedPaperSubResRela = 'divRtCitedPaperSubResRelaDataLst';
  /**
  ...
  constructor(objShowList: IShowList, divLayout: HTMLDivElement) {
    ...
    PaperSubResLstSel.objPageEdit = this;
  }
  /**
 ...
 **/
  ...
  public static btn_Click(strCommandName: string, strKeyId: string, divLayout: HTMLDivElement) {
    ...
    // const objPage: PaperSubResLstSel = PaperSubResLstSel.objPageEdit;
...
*/
  public async PageLoad() {
    ...
    try {
      ...
      if (userStore.getUserId != '') {
        //1、为下拉框设置数据源,绑定列表数据
        this.hidSortPaperResRelaBy = 'updDate Desc';
        ...
        await this.BindGv_vPaperSubRes();
  ...
  //   */
  // public ShowDialog_PaperSubRes(strOp:string) {
  //   //检查相关控件是否存在

  //   CheckControlExist(divName, 'div', 'divPaperSubResList');
  //   CheckControlExist(divName, 'h4', 'myModalLabel');
  //   if (strOp === 'Add') {
  //     $('#divPaperSubResList').html('添加记录');
  //     //ViewInfo_EditEx.GetMaxStrId('#txtViewId');
  //   } else if (strOp === 'Update') {
  //     $('#divPaperSubResList').html('修改记录');
  //   } else if (strOp === 'Detail') {
  //     $('#btnSubmit_ViewInfo').hide();
  //     $('#myModalLabel').html('详细信息');
  //   }
  //   ShowDialog('#divPaperSubResList');
  // }
  ...
  //   CheckControlExist(divName, 'div', 'divPaperSubResList');
  //   HideDialog('#divPaperSubResList');
  // }
  /**
   * 显示对话框
   * (AutoGCLib.Vue_ViewScript_EditCS_TS4TypeScript:Gen_Vue_TS_ShowDialog)
   **/
  public async ShowDialog_divPaperSubResList(strOp: string): Promise<boolean> {
    const strThisFuncName = this.ShowDialog_divPaperSubResList.name;
    if (PaperSubResLstSel.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      await PaperSubResLstSel.EditObj.showDialog();
    }
    this.divEdit = PaperSubResLstSel.EditObj.$refs.refDivLayout;
    if (this.divEdit == null) {
      if (PaperSubResLstSel.times4TestShowDialog < 2) {
        PaperSubResLstSel.times4TestShowDialog++;
        setTimeout(() => {
          this.ShowDialog_divPaperSubResList(strOp);
        }, 100);
      } else {
        const strMsg = Format(
      ...
      PaperSubResLstSel.times4TestShowDialog = 0;
   ...
   **/
  public HideDialog_divPaperSubResList() {
    if (PaperSubResLstSel.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      PaperSubResLstSel.EditObj.hideDialog();
    }
  }
  public async btnAddPaperSubResRela_Click() {
    //this.opType = "Add";
    try {
      const divName4Pager = 'divPager';
      const bolIsSuccess = await this.ShowDialog_divPaperSubResList(this.opType);
      if (bolIsSuccess == false) return;

      // 为编辑区绑定下拉框
      await this.BindDdl4EditRegion();
      this.SetEventFunc();
      // this.ShowDialog_PaperSubRes('');
  ...
  public async SetEventFunc() {
    // 在此处放置用户代码以初始化页面
    try {
      const objDiv0 = document.getElementById('divPaperSubResList');
      ...
      btnQuery.addEventListener('click', (e: any) => {
        console.log(e);
        this.btnPaperSubResQuery_Click();
      });
    } catch (e: any) {
  ...
  public async BindDdl4EditRegion() {
    ...
    // await clsDropDownList.BindDdl_UsersInvPaperSubRes_Cache('ddlUserId_q'); //用户查询区域
  }
  /// <summary>
  /// 在当前界面中，导入编辑区域
  ...
  public async AddDPV_Edit(divName4Edit: string) {
    const strUrl = './PaperSubResLstSel';
  ...
  //相关论文
  public async liPaperSubResClick() {
    const objLayOut = this.getDivName(enumDivType.LayOut_01);
    if (objLayOut == null) return;
    try {
      //文献类型；
      await PaperSubResType_BindDdl_PaperSubResTypeIdInDivCache(objLayOut, 'ddlLiteratureTypeId_q');
  ...
  public async btnParticipateQA_Click(strKeyId: string) {
    try {
      ...
      const objPaperResRelaEN = await PaperResRela_GetObjByKeyLstAsync(paperId, paperSubResId);
      if (objPaperResRelaEN != null) {
        const strPaperSubResId = objPaperResRelaEN.paperId;
  ...
  */
  public async CombinevPaperSubResCondition(): Promise<string> {
    const divName = this.divEdit;
    //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
    //例如 1 = 1 && userName = '张三'
    let strWhereCond = ' 1 = 1 ';

    const strTopicId: string = this.paperId;
    //论文条件教学班
    ...
    try {
      ...
      //排除获取已存在的关系数据
      strWhereCond += ` And paperId not in (select paperId from PaperResRela where paperId = '${strTopicId}')`;
    } catch (objException) {
      const strMsg = `(errid:WiTsCs0002)在组合查询条件(CombinePaperSubResCondition)时出错!请联系管理员!${objException}`;
      throw strMsg;
    }
    return strWhereCond;
  }
...
*/
  public async BindGv_vPaperSubRes() {
    const objDivLayOut = this.divEdit;
    if (objDivLayOut == null) return;
    const strListDiv = GetDivObjInDivObj(objDivLayOut, this.mstrListDivPaperSubRes);
    ...
    //}
    //论文
    PaperSubResCRUD.sortPaperSubResBy = 'updDate Desc';

    const strWhereCond = await this.CombinevPaperSubResCondition();
    const intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrPaperSubResExObjLst: Array<clsPaperSubResENEx> = [];
    try {
      this.recCount = await PaperSubRes_GetRecCountByCondAsync(strWhereCond);
      const objPagerPara: stuPagerPara = {
        pageIndex: intCurrPageIndex,
        pageSize: this.pageSize,
        whereCond: strWhereCond,
        orderBy: PaperSubResCRUD.sortPaperSubResBy,
        sortFun: (x, y) => {
          x = x;
          y = y;
          return 0;
      ...
      arrPaperSubResExObjLst = await PaperSubResEx_GetObjExLstByPagerAsync(objPagerPara);
    } catch (e: any) {
      console.error('catch(e)=');
      console.error(e);
      const strMsg = `根1据条件获取相应的记录对象的列表不成功,${e}.`;
    ...
    try {
      this.BindTab_vPaperSubRes(strListDiv, arrPaperSubResExObjLst);
      console.log('完成BindGv_vPaperSubRes!');

      if (GetInputValueInDivObj(objDivLayOut, 'hidstrType') == '03') {
        this.BindTab_vPaperSubResQA(strListDiv, arrPaperSubResExObjLst);
        console.log('完成BindGv_vQAPaperSubRes!');
      }
    } catch (e: any) {
      console.error('catch(e)=');
      console.error(e);
  ...
  /* 显示vPaperSubRes对象的所有属性值
   (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_BindTab)
   <param name = "divContainer">显示容器</param>
   <param name = "arrPaperSubResObjLst">需要绑定的对象列表</param>
 */
  public async BindTab_vPaperSubRes(
    divContainer: HTMLDivElement,
    arrPaperSubResExObjLst: Array<clsPaperSubResENEx>,
  ) {
    const strThisFuncName = this.BindTab_vPaperSubRes.name;
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      // },
      {
        ...
        funcName: (strKeyId: string, strText: string) => {
          ...
          //btn1.setAttribute("onclick", `btnPaperSubResRecordInTab_Click("${strKeyId}");`);
          (function (strKeyId1) {
            btn1.onclick = function () {
              PaperSubResLstSel.btnReturnClick(strKeyId1);
            };
          })(strKeyId);
  ...
  public static btnReturnClick(strKeyId: string) {
    // PaperSubResLstSel.objPageEdit.HideDialog_PaperSubRes();
    PaperSubResLstSel.objPageEdit.iShowList.BindGvCache(clsPaperResRelaEN._CurrTabName, strKeyId);
    console.log('btnReturnClick(strKeyId:string) ');
  }
  public async BindTab_vPaperSubResQA(
    divContainer: HTMLDivElement,
    arrPaperSubResExObjLst: Array<clsPaperSubResENEx>,
  ) {
    const strThisFuncName = this.BindTab_vPaperSubResQA.name;
    const objLayOut = this.divEdit;
    if (objLayOut == null) return;
    if (divContainer == null) {
      const strMsg = `所给层divContainer不存在！(in ${strThisFuncName})`;
    ...
    const arrDataColumn: Array<clsDataColumn> = [
      ...
      },
      {
        ...
        funcName: (strKeyId: string, strText: string) => {
          ...
          btn1.innerText = strText;
          btn1.className = 'btn btn-outline-info btn-sm';
          btn1.setAttribute('onclick', `btnPaperSubResQARecordInTab_Click("${strKeyId}");`);
          return btn1;
        },
      },
    ];
    ...
    //BindTab(o, arrPaperSubResExObjLst, arrDataColumn, "paperId");
    const divName4Pager = 'divPager';
    this.objPager.recCount = this.recCount;
    this.objPager.pageSize = this.pageSize;
  ...
  //查询论列表
  public async btnPaperSubResQuery_Click() {
    await this.BindGv_vPaperSubRes();
  }
  //确定选择的论文 并添加到关系表中
  public btnPaperSubResRecordInTab_Click(strkeyId: string) {
    const divName = this.divEdit;
    //存放Id
    // SetHidPaperSubResId(divName, strkeyId);

    //获取论文条件 02代表 小组成员论文数据；
    const strType = GetInputValueInDivObj(divName, 'hidstrType');
    if (strType == '01') {
      //执行添加关系方法
      //this.AddNewRecordSavePaperSubResRela();//所有论文
    } else {
      //小组成员论文；
   ...
   */
  public set hidSortvPaperSubResBy(value: string) {
    const divName = this.divEdit;
    SetInputValueInDivObj(divName, 'hidSortvPaperSubResBy', value);
  }
  /*
   * 设置排序字段
   */
  public get hidSortvPaperSubResBy(): string {
    const divName = this.divEdit;
    return GetInputValueInDivObj(divName, 'hidSortvPaperSubResBy');
  }
...
*/
   ...
   */
  public set hidSortPaperResRelaBy(value: string) {
    const divName = this.divEdit;
    SetInputValueInDivObj(divName, 'hidSortPaperResRelaBy', value);
  }
  /*
   * 设置排序字段
   */
  public get hidSortPaperResRelaBy(): string {
    const divName = this.divEdit;
    return GetInputValueInDivObj(divName, 'hidSortPaperResRelaBy');
  }

  /*
   * 获取当前页序号  -------论文
   */
  public get CurrPageIndexPaperSubRes(): number {
    const divName = this.divEdit;
    return GetInputValueInDivObjN(divName, 'hidCurrPageIndexPaperSubRes');
  }
  /*
   * 设置当前页序号-------论文
   */
  public set CurrPageIndexPaperSubRes(value: number) {
    const divName = this.divEdit;
    SetInputValueInDivObj(divName, 'hidCurrPageIndexPaperSubRes', value);
  }

  /* 
   论文
  */
  public async SortByPaperSubRes(strSortByFld: string) {
    if (PaperSubResCRUD.sortPaperSubResBy.indexOf(strSortByFld) >= 0) {
      if (PaperSubResCRUD.sortPaperSubResBy.indexOf('Asc') >= 0) {
        PaperSubResCRUD.sortPaperSubResBy = `${strSortByFld} Desc`;
      } else {
        PaperSubResCRUD.sortPaperSubResBy = `${strSortByFld} Asc`;
      }
    } else {
      PaperSubResCRUD.sortPaperSubResBy = `${strSortByFld} Asc`;
    }
  ...
  //小组论文写作 公开
  public async btnIsPublicPaperSubRes_Click(strKeyId: number) {
    if (strKeyId == 0) {
      alert('请选择需要提交的记录！');
      return;
    }
    this.PublicPaperSubResRecord_pyf(strKeyId);
  }

  public async PublicPaperSubResRecord_pyf(strPaperSubResId: number) {
    //this.keyId = strPaperSubResId;

    try {
      const objPaperSubResEN = await PaperSubRes_GetObjByPaperSubResIdAsync(strPaperSubResId);
      if (objPaperSubResEN == null) return;
      // //通过判断数据用户是否是当前登录用户；
      if (objPaperSubResEN.updUserId == userStore.getUserId) {
        //判断数据是否已审核
      } else {
  ...
  public get paperId(): string {
    return PaperSubResLstSel.GetPropValue('paperId');
  }
}
