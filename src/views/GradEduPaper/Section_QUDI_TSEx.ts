import { Ref } from 'vue';
import $ from 'jquery';
import { Section_EditEx } from '../GradEduPaper/Section_EditEx';
...
import {
  vSection_GetObjLstByPagerAsync,
  vSection_GetRecCountByCondAsync,
...
export class Section_QUDI_TSEx extends SectionCRUD implements IShowList {
  public static SectionQUDITsRef: Ref<any>;
  ...
  public static btn_Click(strCommandName: string, strKeyId: string, divLayout: HTMLDivElement) {
    ...
    if (SectionCRUD.objPageCRUD == null) {
      SectionCRUD.objPageCRUD = new Section_QUDI_TSEx(divLayout);
      objPage = <Section_QUDI_TSEx>SectionCRUD.objPageCRUD;
    } else {
      objPage = <Section_QUDI_TSEx>SectionCRUD.objPageCRUD;
    }
    let objPageEdit;
    ...
    switch (strCommandName) {
      ...
      case 'AddNewRecordWithMaxId': //添加记录使用最大关键字
        objPageEdit = new Section_EditEx('Section_EditEx', objPage);
        objPageEdit.paperId = objPage.paperId;
      ...
      case 'ReOrder': //重序记录
        ...
        break;
      default:
        AccessBtnClickDefault(strCommandName, 'Section_QUDI_TSExEx.btn_Click');
...
*/
  public async PageLoad() {
    ...
    try {
      ...
      console.log('SectionCRUD_EditEx.strPaperId:', Section_QUDI_TSEx.strPaperId);
      ...
      if (strUserId != '') {
        ...
        await vSection_GetRecCountByCondAsync(strWhereCond).then((jsonData) => {
 ...
 */
  public async BindGv_vSection(divList: HTMLDivElement) {
    ...
    if (divList == null) {
      divList = GetDivObjInDivObj(objLayOut, 'divList');
    }
    ...
    try {
      ...
      arrvSectionObjLst = await vSection_GetObjLstByPagerAsync(objPagerPara);
    } catch (e: any) {
    ...
    //}
    try {
      this.BindTab_vSection(divList, arrvSectionObjLst);
  ...
  public HideDialog_Section() {
    if (Section_QUDI_TSEx.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      Section_QUDI_TSEx.SectionQUDITsRef.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_Section(strOp: string): Promise<boolean> {
    ...
    if (Section_QUDI_TSEx.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (Section_QUDI_TSEx.SectionQUDITsRef == null) {
        const strMsg = Format(
          '当前编辑区的SectionQUDITsRef为空，请检查！(in {0}.{1})',
      ...
      await Section_QUDI_TSEx.SectionQUDITsRef.value.showDialog();
    }
    this.divLayout = Section_QUDI_TSEx.SectionQUDITsRef.value.$refs.refDivLayout;
    if (this.divLayout == null) {
      if (Section_QUDI_TSEx.times4TestShowDialog < 2) {
        Section_QUDI_TSEx.times4TestShowDialog++;
        setTimeout(() => {
      ...
      Section_QUDI_TSEx.times4TestShowDialog = 0;
    }
  ...
  public get paperId(): string {
    return SectionCRUD.GetPropValue('paperId');
  }
}
