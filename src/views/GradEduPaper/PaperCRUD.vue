<template>
  <div id="divLayout" ref="refDivLayout" class="divComContainer">
        ...
        <tr>
          ...
          <td class="text-left">
            <span class="form-control form-control-sm" style="width: 200px">
              <input
                id="chkIsQuotethesis_q"
                name="chkIsQuotethesis_q"
                type="checkbox"
                Text="是否引用论文"
              /><label for="chkIsQuotethesis_q">是否引用论文</label></span
            >
          </td>
          <td class="text-right">
            <label
      ...
      <ul class="nav">
        ...
        <li class="nav-item ml-3">
          <div class="btn-group" role="group" aria-label="Basic example">
            <select
              id="ddlMajorDirectionId_SetValue_SetFldValue"
              name="ddlMajorDirectionId_SetValue_SetFldValue"
              class="form-control form-control-sm"
              style="width: 60px"
            ></select>
            <button
              id="btnSetMajorDirectionId"
              name="btnSetMajorDirectionId"
              class="btn btn-outline-info btn-sm text-nowrap"
              @click="btn_Click('', '')"
              >设置研究方向</button
            >
    ...
    <div id="divList" ref="refDivList" class="div_List">
      ...
      <input id="hidSortPaperBy" type="hidden" />
    </div>
    <!--编辑层-->
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      onMounted(() => {
        const objPage = new PaperCRUDEx(refDivLayout.value);
      ...
      function btn_Click(strCommandName: string, strKeyId: string) {
        switch (strCommandName) {
          ...
          case 'UpdateRecordInTab':
            PaperCRUD.EditObj = Paper_EditRef.value;
            Paper_Edit.EditObj = Paper_EditRef.value;
            break;
          default:
            break;
        }
        PaperCRUDEx.btn_Click(strCommandName, strKeyId, refDivLayout.value);
      }
      return {
        strTitle,
        btn_Click,
