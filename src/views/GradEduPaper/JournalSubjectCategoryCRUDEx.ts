//import * as QQ from "q";
import $ from 'jquery';
import { JournalSubjectCategory_EditEx } from './JournalSubjectCategory_EditEx';
import { JournalSubjectCategoryEx_ImportDataFromCsv } from '@/ts/L3ForWApiEx/GradEduPaper/clsJournalSubjectCategoryExWApi';
import { JournalSubjectCategoryCRUD } from '@/viewsBase/GradEduPaper/JournalSubjectCategoryCRUD';
import { JournalSubjectCategory_ReFreshCache } from '@/ts/L3ForWApi/GradEduPaper/clsJournalSubjectCategoryWApi';
import {
...
export class JournalSubjectCategoryCRUDEx extends JournalSubjectCategoryCRUD implements IShowList {
  ...
  BindGvCache(strType: string, strPara: string) {
    ...
    switch (strType) {
      case 'JournalSubjectCategory':
        this.BindGv_JournalSubjectCategoryCache(this.thisDivList);
        break;
      default:
  ...
  public static btn_Click(strCommandName: string, strKeyId: string, divLayout: HTMLDivElement) {
    const objPage: JournalSubjectCategoryCRUDEx = new JournalSubjectCategoryCRUDEx(divLayout);
    const objPageEdit: JournalSubjectCategory_EditEx = new JournalSubjectCategory_EditEx(
      '',
      objPage,
    );
    ...
    switch (strCommandName) {
      ...
      case 'ReOrder': //重序记录
        ...
        break;
      default:
        AccessBtnClickDefault(strCommandName, 'JournalSubjectCategoryCRUDExEx.btn_Click');
  ...
  public async btnImportData_Click() {
    ...
    try {
      ...
      if (returnInt > 0) {
        ...
        await this.BindGv_JournalSubjectCategoryCache(this.thisDivList);
